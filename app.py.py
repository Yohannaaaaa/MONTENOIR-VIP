...
def tarot_lucky_wheel(data):
    ...
    if users[key].get("lastWheelDate") == today:
        emit("tarot_wheel_result", {"ok": False, "msg": "Şans çarkını günde sadece 1 kez çevirebilirsin."})
        return

    rewards = [10, 10, 10, 20, 20, 20, 30, 30, 50, 50, 100, 200, 300]
    weights = [20, 20, 20, 15, 15, 15, 10, 10, 6, 6, 4, 2, 1]
...
<style>
body{
    margin:0;min-height:100vh;font-family:Arial,sans-serif;color:#d4af37;
    background:radial-gradient(circle at top,#2a1a05 0%,#050505 48%,#000 100%);
    overflow-x:hidden;
}
.tarotCardsBg{position:fixed;inset:0;z-index:0;pointer-events:none;opacity:.16;background:radial-gradient(circle at 20% 30%,rgba(212,175,55,.35),transparent 13%),radial-gradient(circle at 80% 20%,rgba(212,175,55,.25),transparent 12%),radial-gradient(circle at 40% 80%,rgba(212,175,55,.22),transparent 15%),repeating-linear-gradient(45deg,rgba(212,175,55,.06) 0 1px,transparent 1px 48px);}
.tarotCardsBg:after{content:"☾  THE MOON   ✦   THE STAR   ✦   THE LOVERS   ✦   THE HIGH PRIESTESS   ☽";position:absolute;top:34%;left:-12%;width:130%;font-size:74px;font-weight:900;letter-spacing:18px;color:#d4af37;opacity:.18;transform:rotate(-8deg);font-family:Georgia,serif;} .tarotCardsBg:before{content:"🂠  🂡  🂢  🂣  🂤";position:absolute;bottom:12%;left:5%;font-size:150px;letter-spacing:40px;color:#d4af37;opacity:.08;filter:blur(.4px);}
.stars{position:fixed;inset:0;z-index:0;pointer-events:none;background-image:radial-gradient(#d4af37 1px,transparent 1px);background-size:55px 55px;opacity:.18;animation:stars 25s linear infinite;}
@keyframes stars{from{background-position:0 0}to{background-position:400px 700px}}
.fog{position:fixed;inset:-20%;z-index:0;pointer-events:none;background:radial-gradient(circle,rgba(255,255,255,.08),transparent 35%);filter:blur(35px);opacity:.35;animation:fog 12s ease-in-out infinite alternate;}
...
h1{font-family:Georgia,'Times New Roman',serif;font-size:52px;text-align:left;text-shadow:0 0 18px #d4af37,0 0 50px #8a6a00;margin:20px 0 5px;letter-spacing:4px}.sub{text-align:left;color:#f4df94;letter-spacing:3px;font-family:Georgia,'Times New Roman',serif}.topbar{display:flex;justify-content:flex-start;gap:10px;flex-wrap:wrap;margin:20px 0}
.btn,.service{background:linear-gradient(145deg,#000,#160f02,#000);color:#d4af37;border:1px solid #d4af37;border-radius:18px;padding:13px 18px;font-weight:900;cursor:pointer;box-shadow:0 0 12px rgba(212,175,55,.55),inset 0 0 14px rgba(212,175,55,.12);}
...
.panel{background:rgba(0,0,0,.72);border:1px solid rgba(212,175,55,.75);border-radius:24px;padding:20px;text-align:left;box-shadow:0 0 25px rgba(212,175,55,.25);font-family:Georgia,'Times New Roman',serif;}
.panel h2{text-align:center;color:#ffd978}.service{width:100%;margin:6px 0;text-align:left}input,textarea,select{width:95%;padding:12px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:#f8e7a0;margin:6px 0;}textarea{height:90px}.resultBox{white-space:pre-wrap;color:#f8e7a0;line-height:1.5}.adminItem{border:1px solid #d4af37;border-radius:14px;padding:12px;margin:8px 0;background:rgba(255,255,255,.04)}.candle{font-size:36px;animation:candle 1.4s ease-in-out infinite alternate}@keyframes candle{from{filter:drop-shadow(0 0 4px #d4af37)}to{filter:drop-shadow(0 0 20px #ffcc55)}}
.profileLine{display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap}
.profileLine img{width:58px;height:58px;border-radius:50%;object-fit:cover;border:2px solid #d4af37;box-shadow:0 0 14px #d4af37}
.levelBadge{display:inline-block;border:1px solid #d4af37;border-radius:999px;padding:6px 12px;margin-left:6px;background:rgba(212,175,55,.12);box-shadow:0 0 12px rgba(212,175,55,.45)}
.chatBox{height:170px;overflow:auto;background:#050505;border:1px solid #d4af37;border-radius:14px;padding:10px;color:#f8e7a0;margin-bottom:8px}
.payBox{border:1px dashed #d4af37;border-radius:14px;padding:10px;margin:8px 0;background:rgba(212,175,55,.05)}

.ownerName{
    color:#d4af37!important;
    background:linear-gradient(90deg,#d4af37,#000,#ffd700,#d4af37);
    -webkit-background-clip:text;
    color:transparent!important;
    text-shadow:0 0 10px #d4af37,0 0 28px #000,0 0 38px #ffd700;
    font-weight:900;
    letter-spacing:2px;
}
.ownerBadge{
...
.wheel{
    width:260px;height:260px;border-radius:50%;margin:15px auto;
    border:6px solid #d4af37;
    background:conic-gradient(#2a1b00 0deg 45deg,#000 45deg 90deg,#3b2600 90deg 135deg,#050505 135deg 180deg,#2a1b00 180deg 225deg,#000 225deg 270deg,#3b2600 270deg 315deg,#050505 315deg 360deg);
    box-shadow:0 0 24px #d4af37,inset 0 0 30px rgba(212,175,55,.25);
    display:flex;align-items:center;justify-content:center;
    position:relative;
    transition:transform 4s cubic-bezier(.12,.74,.12,1);
}
.wheel:before{content:"50  ·  100  ·  200  ·  30  ·  10  ·  20  ·  300";position:absolute;width:210px;text-align:center;color:#ffd700;font-weight:900;font-size:18px;line-height:32px;text-shadow:0 0 8px #d4af37;}
.wheel:after{content:"🔮";font-size:46px;z-index:2;background:#000;border:2px solid #d4af37;border-radius:50%;padding:18px;box-shadow:0 0 18px #d4af37;}
.wheelPointer{font-size:34px;color:#ffd700;text-shadow:0 0 12px #d4af37;margin-bottom:-12px}

</style></head>
<body>
<div class="tarotCardsBg"></div><div class="stars"></div><div class="fog"></div>
<div class="wrap">
<h1>🚪 ✦ 🔮 TAROT & RITUEL ✦</h1>
<div class="sub">Siyah kadife · Altın sır · Mum ışığı</div>
<div class="candle">🕯️ 🕯️ 🕯️</div>
<div class="topbar">
<button class="btn" onclick="location.href='/'">♠ Codenames'e Dön</button>
<button class="btn" onclick="loadProfile()">👤 Profili Yenile</button>
<button class="btn" onclick="spinWheel()">🎡 Şans Çarkı</button>
<button class="btn" onclick="loadAdminRequests()">👑 Admin Talep Paneli</button>
</div>
...
<div class="panel" style="text-align:center">
  <div class="profileLine">
    ...
    <div>
      <b>👤 <span id="userName">-</span><span id="ownerBadge"></span></b><br>
      🪙 <span id="chips">-</span> jeton
      <span id="levelBadge" class="levelBadge">-</span>
    </div>
  </div>
  <p style="font-size:12px;color:#f4df94">Codenames hesabın otomatik kullanılır. Tarot tarafında yeniden giriş yok.</p>
</div>

<div class="grid">
<div class="panel"><h2>🔮 Tarot Açılımları</h2>
<button class="service" onclick="selectService('genel_bakim')">Genel Bakım · 30 dk · 1500 🪙</button>
<button class="service" onclick="selectService('ask_acilimi')">Aşk Açılımı · 20 dk · 1000 🪙</button>
<button class="service" onclick="selectService('tek_soru')">Tek Soru · 5 dk · 300 🪙</button>
<button class="service" onclick="selectService('uc_soru')">3 Soru · 10 dk · 700 🪙</button>
<button class="service" onclick="selectService('ai_otomatik_bakim')">Otomatik AI Bakımı · 100 🪙</button>
</div>

<div class="panel"><h2>🕯️ Ritüeller</h2>
<button class="service" onclick="selectService('rituel_ask_iliski')">Aşk ve İlişki · 800 🪙</button>
<button class="service" onclick="selectService('rituel_ozguven_cekim')">Öz Güven ve Çekim Gücü · 800 🪙</button>
<button class="service" onclick="selectService('rituel_sans_bolluk')">Şans ve Bolluk · 800 🪙</button>
<button class="service" onclick="selectService('rituel_kariyer_basari')">Kariyer ve Başarı · 800 🪙</button>
<button class="service" onclick="selectService('rituel_negatif_enerji')">Negatif Enerjiden Arınma · 800 🪙</button>
<button class="service" onclick="selectService('rituel_kisisel_niyet')">Kişisel Niyet · 1500 🪙</button>
</div>

<div class="panel"><h2>💳 Gerçek Ödeme</h2>
<div class="payBox">Stripe · hazırlanacak</div>
<div class="payBox">PayPal · hazırlanacak</div>
<div class="payBox">Google Pay · hazırlanacak</div>
<hr>
<h2>🪙 Paketler</h2>
<div class='payBox'>🪙 200 Jeton — <b>4,99 £</b></div><div class='payBox'>🪙 500 Jeton — <b>9,99 £</b></div><div class='payBox'>🪙 1200 Jeton — <b>19,99 £</b></div><div class='payBox'>🪙 3000 Jeton — <b>39,99 £</b></div><div class='payBox'>🪙 8000 Jeton — <b>89,99 £</b></div>
</div>
</div>

<div class="panel"><h2>📩 Talep Formu</h2>
<b>Seçilen hizmet:</b> <span id="selectedLabel">Henüz seçilmedi</span>
<input id="clientName" placeholder="İsmin">
<input id="motherName" placeholder="Anne adı">
<input id="birthDate" placeholder="Doğum tarihi">
<textarea id="question" placeholder="Sorunu / niyetini yaz"></textarea>
<input id="photoNote" placeholder="Fotoğraf / özel bilgi notu">
<button class="btn" onclick="submitTarot()">Jetonla Satın Al ve Gönder</button>
</div>

<div class="grid">
<div class="panel"><h2>✨ Sonuç</h2><div id="resultBox" class="resultBox">Henüz sonuç yok.</div><div id="pdfBox"></div></div>
<div class="panel"><h2>💬 Canlı Sohbet</h2><div id="tarotChatBox" class="chatBox"></div><input id="tarotChatInput" placeholder="Bakımcıya mesaj yaz"><button class="btn" onclick="sendTarotChat()">Gönder</button></div>
</div>

<div class="panel"><h2>👑 Admin Talep Paneli</h2><div id="adminRequests">Yohanna / admin hesabıyla talepleri görebilirsin.</div>
<hr>
<h3>👑 Owner Üyelik Ver</h3>
<input id="adminTargetUser" placeholder="Kullanıcı adı">
...
<button class="btn" onclick="ownerSetLevel()">Üyelik Ver</button>
...
<script>
const socket=io();
let account=localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||'';
let selectedService='';
function defaultAvatarData(avatar){
    const isMan = avatar === 'man.png';
    const bg = isMan ? '#111111' : '#ff4fd8';
    const emoji = isMan ? '👤' : '👩';
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><circle cx="60" cy="60" r="56" fill="${bg}" stroke="#d4af37" stroke-width="6"/><text x="60" y="78" font-size="58" text-anchor="middle">${emoji}</text></svg>`;
    return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
}
function levelName(chips){
    chips=parseInt(chips||0);
    if(chips>=15000) return '💎 Elmas';
    if(chips>=5000) return '🥇 Altın';
    if(chips>=1000) return '🥈 Gümüş';
    return '🥉 Bronz';
}
function saveSharedProfile(p){
    ...
    localStorage.setItem('codenamesChips_'+p.username, String(p.chips||1000));
}
function renderProfile(p){
    const isOwner = (p.username||'').toLowerCase()==='yohanna';
    userName.innerHTML=p.username;
    userName.className=isOwner?'ownerName':'';
    ownerBadge.innerHTML=isOwner?'<span class="ownerBadge">👑 OWNER</span>':'';
    chips.innerHTML=p.chips;
    levelBadge.innerHTML=p.membershipLabel || levelName(p.chips);
    avatarImg.src=p.avatarData || defaultAvatarData(p.avatar||'woman.png');
    saveSharedProfile(p);
}
function loadProfile(){
    account=localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||account||'';
    if(!account){alert('Önce Codenames tarafında giriş yap.');location.href='/';return}
    socket.emit('tarot_get_profile',{account});
}
function selectService(s){
    selectedService=s;
    const labels={genel_bakim:'Genel Bakım · 1500 jeton',ask_acilimi:'Aşk Açılımı · 1000 jeton',tek_soru:'Tek Soru · 300 jeton',uc_soru:'3 Soru · 700 jeton',ai_otomatik_bakim:'Otomatik AI Bakımı · 100 jeton',rituel_ask_iliski:'Aşk ve İlişki Ritüeli · 800 jeton',rituel_ozguven_cekim:'Öz Güven ve Çekim Gücü Ritüeli · 800 jeton',rituel_sans_bolluk:'Şans ve Bolluk Ritüeli · 800 jeton',rituel_kariyer_basari:'Kariyer ve Başarı Ritüeli · 800 jeton',rituel_negatif_enerji:'Negatif Enerjiden Arınma Ritüeli · 800 jeton',rituel_kisisel_niyet:'Kişisel Niyet Ritüeli · 1500 jeton'};
    selectedLabel.innerHTML=labels[s]||s;
}
function submitTarot(){
    if(!account){alert('Giriş gerekli.');return}
    if(!selectedService){alert('Önce hizmet seç.');return}
    socket.emit('tarot_submit_request',{account,service:selectedService,name:clientName.value,motherName:motherName.value,birthDate:birthDate.value,question:question.value,photoNote:photoNote.value});
}
...
function sendTarotChat(){
    let msg=tarotChatInput.value.trim();
    if(!msg)return;
    socket.emit('tarot_chat',{account,msg});
    tarotChatInput.value='';
}
...
socket.on('tarot_wheel_result',d=>{if(!d.ok){alert(d.msg);return}renderProfile(d.profile);wheelInfo.innerHTML='Bugünkü ödülün: +'+d.reward+' jeton';alert('Şans çarkı: +'+d.reward+' jeton');});
...
def owner_set_membership(data):
    ...
    if owner.lower() != "yohanna":
        emit("owner_set_membership_result", {"ok": False, "msg": "Bu işlemi sadece Yohanna yapabilir."})
        return
    users = load_users()
    key = find_user_key(users, target)
    if not key:
        emit("owner_set_membership_result", {"ok": False, "msg": "Kullanıcı bulunamadı."})
        return
    labels = {"bronze": "🥉 Bronz", "silver": "🥈 Gümüş", "gold": "🥇 Altın", "diamond": "💎 Elmas"}
    users[key]["membershipLevel"] = level
    users[key]["membershipLabel"] = labels.get(level, level)
    save_users(users)
    emit("owner_set_membership_result", {"ok": True, "msg": f"{key} için üyelik verildi: {labels.get(level, level)}"})


# =========================
# OWNER PANEL - ONLY YOHANNA
# =========================
OWNER_USERNAME = "yohanna"
SITE_SETTINGS_FILE = os.path.join(os.environ.get("DATA_DIR", "."), "site_settings.json")

def is_owner_name(name):
    return (name or "").strip().lower() == OWNER_USERNAME
...
def load_site_settings():
    try:
        ...
        return {"maintenance": False, "registrations": True, "wheel": True, "tarot": True, "ritual": True, "wheelDailyLimit": 1}
...
def owner_get_panel(data):
    ...
    emit("owner_panel_data", {"ok": True, "users": [private_profile(k, v) for k, v in users.items()], "requests": reqs[:100], "settings": load_site_settings(), "stats": stats, "payments": {"stripe": "Bugün 0 £ · Bu ay 0 £ · Toplam 0 £", "paypal": "Son işlemler / iadeler hazırlanacak"}})
...
def owner_set_membership(data):
    ...
    users[key]["membershipLevel"] = "" if level == "none" else level
    users[key]["membershipLabel"] = labels.get(level, "")
    users[key]["membershipDuration"] = data.get("duration", "forever")
    save_users(users)
...
def owner_update_request(data):
    ...
    for r in reqs:
        if str(r.get("id")) == str(data.get("id")):
            ...
            r["result"] = data.get("result", r.get("result", ""))
            save_tarot_requests(reqs)
...
def owner_update_settings(data):
    ...
    emit("owner_action_result", {"ok": True, "msg": "Site ayarları güncellendi."})
...
#tarotTopBtn.tarotTopBtn{left:15px!important;right:auto!important;top:65px!important;}
.name-green{color:#00ff66!important;text-shadow:0 0 10px #00ff66,0 0 22px #00ff66!important;}
</style></head><body>
<div class="vipCasinoMarks"><span class="m1">A♠</span><span class="m2">K♥</span><span class="m3">Q♣</span><span class="m4">J♦</span><span class="m5">♠♥♣♦</span><span class="m6">A K Q J</span></div><div id="profileChip" class="vipProfileChip" onclick="openProfile()">👤 Profil</div><button id="tarotTopBtn" class="tarotTopBtn" onclick="window.location.href='/tarot'">🚪 ✦ 🔮 TAROT & RITUEL ✦</button><div class="topLeftFixed"><button onclick="startTimer()">⏱ Süre</button><button onclick="newGame()">🎲 Yeni Oyun</button><button onclick="goLobby()">🚪 Lobi</button><button class="closeTableBtn" onclick="closeCurrentTable()">❌ Masayı Kapat</button><button id="ownerPanelBtn" style="display:none;" onclick="openOwnerPanel()">👑 Owner Panel</button><button onclick="openAuth()">👤 Üyelik</button><button onclick="openProfile()">🏆 Profil</button><button onclick="openRanking()">📊 Classement</button><button onclick="openSettings()">⚙️ Ayarlar</button><button onclick="openWords()">📚 Kelimeler</button><button onclick="openBet()">🎰 Bahis</button><button onclick="openShop()">🪙 Jeton Al</button></div>
...
function updateProfileChip(){
    if(currentAccount){
        ...
        const genderCls = ((currentProfile && currentProfile.avatar)==='man.png') ? 'genderMaleName' : 'genderFemaleName';
        profileChip.innerHTML='<img src="'+avatarSrc+'" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid #d4af37;box-shadow:0 0 10px #d4af37;"> <span class="'+genderCls+'">'+currentAccount+'</span>'+vip+' <span>🪙 '+currentChips+'</span>';
    }else{
...
function refreshOwnerButton(){ if(document.getElementById('ownerPanelBtn')) ownerPanelBtn.style.display = isOwnerAccount() ? 'inline-block' : 'none'; }
...
function ownerUserAction(action){ socket.emit('owner_manage_user',{owner:currentAccount,target:ownerTarget(),action}); }
function ownerChip(amount){ socket.emit('owner_manage_user',{owner:currentAccount,target:ownerTarget(),action: amount>0?'add_chips':'remove_chips',amount:Math.abs(amount)}); }
function ownerSetMembership(){ socket.emit('owner_set_membership',{owner:currentAccount,target:ownerTarget(),level:ownerMembershipLevel.value,duration:ownerMembershipDuration.value}); }
...
socket.on('owner_panel_data',d=>{if(!d.ok){alert(d.msg);return}ownerStats.innerHTML='Toplam kullanıcı: '+d.stats.totalUsers+'<br>Online kullanıcı: '+d.stats.onlineUsers+'<br>Bugünkü kayıt: '+d.stats.todayUsers+'<br>Satılan jeton: '+d.stats.soldChips+'<br>Tamamlanan tarot: '+d.stats.completedTarot+'<br>Tamamlanan ritüel: '+d.stats.completedRitual;ownerUserList.innerHTML='<h3>👥 Kullanıcılar</h3>'+d.users.map(u=>'<div class="profileCard"><b>'+u.username+'</b> · 🪙 '+u.chips+' · '+(u.membershipLabel||'Üyelik yok')+'</div>').join('');if(d.settings){setMaintenance.checked=!!d.settings.maintenance;setRegistrations.checked=!!d.settings.registrations;setWheel.checked=!!d.settings.wheel;setTarot.checked=!!d.settings.tarot;setRitual.checked=!!d.settings.ritual;ownerWheelLimit.value=String(d.settings.wheel?d.settings.wheelDailyLimit||1:0);}});
