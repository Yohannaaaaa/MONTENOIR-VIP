...
# =========================
# MONTENOIR CORE ECONOMY / XP / SOCIAL SYSTEMS
# =========================
...
def calc_level(xp):
    ...
    return level, next_level_xp, max(0, min(progress, 100))

def ensure_user_defaults(data):
    ...
    data.setdefault("ownedFrames", [])
    data.setdefault("ownedNameColors", [])
    data.setdefault("ownedBadges", [])
    data.setdefault("activeFrame", data.get("avatarFrame", "none"))
    data.setdefault("activeNameColor", data.get("nameColor", "default"))
    data.setdefault("openedChests", 0)
    data.setdefault("tournaments", [])
    data.setdefault("premiumTarotRequests", [])
    return data

def add_xp_to_user(users, key, amount, reason=""):
    if not key or key not in users:
        return
    ensure_user_defaults(users[key])
    users[key]["xp"] = int(users[key].get("xp", 0)) + int(amount)
    lvl, nxt, prog = calc_level(users[key]["xp"])
    users[key]["level"] = lvl
    unlock_achievements(users, key)

def unlock_achievements(users, key):
    if key not in users:
        return
    u = ensure_user_defaults(users[key])
    ach = set(u.get("achievements", []))
    if int(u.get("games", 0)) >= 1:
        ach.add("🎮 İlk Oyun")
    if int(u.get("wins", 0)) >= 1:
        ach.add("🏆 İlk Zafer")
    if int(u.get("wins", 0)) >= 10:
        ach.add("🔥 10 Oyun Kazandı")
    if int(u.get("games", 0)) >= 100:
        ach.add("🎮 100 Oyun Oynadı")
    if int(u.get("openedChests", 0)) >= 1:
        ach.add("🎁 İlk Sandık")
    if len(u.get("premiumTarotRequests", [])) >= 1:
        ach.add("🔮 Tarot Ustası")
    if int(u.get("level", 1)) >= 10:
        ach.add("⭐ Seviye 10")
    u["achievements"] = sorted(list(ach))

def profile_payload(username, data):
    ensure_user_defaults(data)
    lvl, nxt, prog = calc_level(data.get("xp", 0))
    data["level"] = lvl
    return {
        "username": username,
        ...
        "friendRequests": data.get("friendRequests", []),
        "ownedFrames": data.get("ownedFrames", []),
        "ownedNameColors": data.get("ownedNameColors", []),
        "ownedBadges": data.get("ownedBadges", []),
        "activeFrame": data.get("activeFrame", data.get("avatarFrame", "none")),
        "activeNameColor": data.get("activeNameColor", data.get("nameColor", "default")),
    }

def is_owner_name(name):
    return (name or "").strip().lower() in ["yohanna", "owner", "admin"]
...
def public_profile(username, data):
    ...
    p = {
        ...
        "xp": int(data.get("xp", 0)),
        "level": calc_level(data.get("xp", 0))[0],
        "nextLevelXp": calc_level(data.get("xp", 0))[1],
        "xpProgress": calc_level(data.get("xp", 0))[2],
        "achievements": data.get("achievements", []),
        "friends": data.get("friends", []),
        "activeFrame": data.get("activeFrame", data.get("avatarFrame", "none")),
        "activeNameColor": data.get("activeNameColor", data.get("nameColor", "default"))
...
def save_player_to_user(player):
    ...
    if username in users:
        ...
        users[username]["xp"] = int(player.get("xp", users[username].get("xp", 0)))
        users[username]["level"] = calc_level(users[username].get("xp", 0))[0]
        users[username]["achievements"] = users[username].get("achievements", [])
        save_users(users)
...
def tournaments_page():
    content = r"""
<div class='grid'>
<div class='item'><h2>👑 Haftalık Codenames Turnuvası</h2><p>Giriş: 100 jeton</p><p>Ödül: 5.000 jeton</p><button class='btn' onclick="joinTournament('weekly-codenames')">Katıl</button></div>
<div class='item'><h2>♠️ Poker Turnuvası</h2><p>Yakında</p></div>
</div><div id='result'></div>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
function joinTournament(tournament){fetch('/api/join_tournament',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),tournament})}).then(r=>r.json()).then(d=>{result.innerText=d.msg;});}
</script>
"""
...
def profile_shop_page():
    content = r"""
<p>Satın alma için giriş yapmış kullanıcı adın tarayıcıdan alınır.</p>
<div class='grid'>
<div class='item'>🟡 Altın Çerçeve<br>1000 jeton<br><button class='btn' onclick="buyCosmetic('gold-frame')">Satın Al</button></div>
<div class='item'>💎 Elmas Çerçeve<br>5000 jeton<br><button class='btn' onclick="buyCosmetic('diamond-frame')">Satın Al</button></div>
<div class='item'>🏛️ Barok Çerçeve<br>7500 jeton<br><button class='btn' onclick="buyCosmetic('baroque-frame')">Satın Al</button></div>
<div class='item'>🌈 İsim Rengi<br>3000 jeton<br><button class='btn' onclick="buyCosmetic('rainbow-name')">Satın Al</button></div>
<div class='item'>✨ Animasyonlu Profil<br>10000 jeton<br><button class='btn' onclick="buyCosmetic('animated-profile')">Satın Al</button></div>
</div>
<div id='result'></div>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
function buyCosmetic(item){fetch('/api/buy_cosmetic',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),item})}).then(r=>r.json()).then(d=>{result.innerText=d.msg||JSON.stringify(d);});}
</script>
"""
...
def chests_page():
    content = r"""
<div class='grid'>
<div class='item'>🥉 Bronz Sandık<br>100 jeton<br><button class='btn' onclick="openChest('bronze')">Aç</button></div>
<div class='item'>🥈 Gümüş Sandık<br>300 jeton<br><button class='btn' onclick="openChest('silver')">Aç</button></div>
<div class='item'>🥇 Altın Sandık<br>800 jeton<br><button class='btn' onclick="openChest('gold')">Aç</button></div>
<div class='item'>💎 Elmas Sandık<br>2000 jeton<br><button class='btn' onclick="openChest('diamond')">Aç</button></div>
</div>
<div id='result'></div>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
function openChest(chest){fetch('/api/open_chest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),chest})}).then(r=>r.json()).then(d=>{result.innerText=d.msg||JSON.stringify(d);});}
</script>
"""
...
def achievements_page():
    content = r"""
<p>Başarımlar oyun, sandık, turnuva ve tarot kullanımına göre otomatik açılır.</p>
<div id='achievements'>Yükleniyor...</div>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
fetch('/api/profile?u='+encodeURIComponent(user())).then(r=>r.json()).then(d=>{
 if(!d.ok){achievements.innerText=d.msg;return}
 let a=d.profile.achievements||[];
 achievements.innerHTML=a.length?a.map(x=>'<div class="item">'+x+'</div>').join(''):'Henüz başarım yok.';
});
</script>
"""
...
def friends_page():
    content = r"""
<input id='target' placeholder='Arkadaş kullanıcı adı' style='padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'>
<button class='btn' onclick='addFriend()'>Arkadaş Ekle</button>
<input id='msgTarget' placeholder='Mesaj gönderilecek kullanıcı' style='padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'>
<input id='msgText' placeholder='Mesaj' style='padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'>
<button class='btn' onclick='sendMsg()'>Özel Mesaj Gönder</button>
<div id='friendsBox'></div><div id='result'></div>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
function load(){fetch('/api/profile?u='+encodeURIComponent(user())).then(r=>r.json()).then(d=>{if(!d.ok){friendsBox.innerText=d.msg;return}friendsBox.innerHTML='<h3>Arkadaşlar</h3>'+((d.profile.friends||[]).join('<br>')||'Yok')+'<h3>İstekler</h3>'+((d.profile.friendRequests||[]).map(x=>x+' <button onclick="acceptFriend(\''+x+'\')">Kabul Et</button>').join('<br>')||'Yok');});}
function addFriend(){fetch('/api/friend_request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),target:target.value})}).then(r=>r.json()).then(d=>{result.innerText=d.msg;load();});}
function acceptFriend(rq){fetch('/api/accept_friend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),requester:rq})}).then(r=>r.json()).then(d=>{result.innerText=d.msg;load();});}
function sendMsg(){fetch('/api/send_private_message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),target:msgTarget.value,msg:msgText.value})}).then(r=>r.json()).then(d=>{result.innerText=d.msg;});}
load();
</script>
"""
...
def ai_tarot_premium_page():
    content = r"""
<p>Premium yorum: 1500 jeton</p>
<input id='birthDate' placeholder='Doğum tarihi' style='padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'>
<textarea id='question' placeholder='Sorunu yaz' style='width:90%;height:90px;padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'></textarea>
<input id='photoNote' placeholder='Fotoğraf / enerji notu' style='padding:12px;border-radius:12px;background:#060606;color:#f6d777;border:1px solid #d4af37'>
<button class='btn' onclick='premiumTarot()'>Premium Yorumu Al</button>
<pre id='result' style='white-space:pre-wrap;text-align:left;color:#fff0a8'></pre>
<script>
function user(){return localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||''}
function premiumTarot(){fetch('/api/premium_tarot',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({u:user(),birthDate:birthDate.value,question:question.value,photoNote:photoNote.value})}).then(r=>r.json()).then(d=>{result.innerText=d.msg;});}
</script>
"""
...
# =========================
# MONTENOIR FUNCTIONAL SYSTEMS V1
# =========================

def require_user_from_request():
    username = (request.args.get("u") or request.form.get("u") or request.json.get("u") if request.is_json else "").strip()
    if not username:
        return None, None, {"ok": False, "msg": "Giriş gerekli."}
    users = load_users()
    key = find_user_key(users, username)
    if not key:
        key = ensure_user_account(username)
        users = load_users()
    ensure_user_defaults(users[key])
...
def api_profile():
    ...
    unlock_achievements(users, key)
    save_users(users)
    return {"ok": True, "profile": profile_payload(key, users[key])}
...
def api_open_chest():
    ...
    rewards = {
        "bronze": [("chips", 50), ("chips", 100), ("badge", "🥉 Bronz Şans")],
    ...
    msg = ""
    if kind == "chips":
        users[key]["chips"] = int(users[key].get("chips", 0)) + int(value)
        msg = f"🪙 {value} jeton kazandın!"
    elif kind == "frame":
        users[key].setdefault("ownedFrames", [])
        if value not in users[key]["ownedFrames"]:
            users[key]["ownedFrames"].append(value)
        msg = f"🎨 {value} çerçeve kazandın!"
    elif kind == "color":
        users[key].setdefault("ownedNameColors", [])
        if value not in users[key]["ownedNameColors"]:
            users[key]["ownedNameColors"].append(value)
        msg = f"🌈 {value} isim rengi kazandın!"
    elif kind == "badge":
        users[key].setdefault("ownedBadges", [])
        if value not in users[key]["ownedBadges"]:
            users[key]["ownedBadges"].append(value)
        msg = f"🎖️ {value} rozeti kazandın!"
    users[key]["openedChests"] = int(users[key].get("openedChests", 0)) + 1
    add_xp_to_user(users, key, 25, "chest")
    save_users(users)
    return {"ok": True, "msg": msg, "profile": profile_payload(key, users[key])}
...
def api_friend_request():
    ...
    tkey = find_user_key(users, target)
    if not key or not tkey:
        return {"ok": False, "msg": "Kullanıcı bulunamadı."}
    ensure_user_defaults(users[key]); ensure_user_defaults(users[tkey])
    if key == tkey:
        return {"ok": False, "msg": "Kendini ekleyemezsin."}
    if key in users[tkey].get("friends", []):
        return {"ok": False, "msg": "Zaten arkadaşsınız."}
    if key not in users[tkey].setdefault("friendRequests", []):
        users[tkey]["friendRequests"].append(key)
    save_users(users)
...
def api_accept_friend():
    ...
    users = load_users()
    key = find_user_key(users, u)
    rkey = find_user_key(users, requester)
    if not key or not rkey:
        return {"ok": False, "msg": "Kullanıcı bulunamadı."}
    ensure_user_defaults(users[key]); ensure_user_defaults(users[rkey])
    if rkey in users[key].get("friendRequests", []):
        users[key]["friendRequests"].remove(rkey)
    if rkey not in users[key]["friends"]:
        users[key]["friends"].append(rkey)
    if key not in users[rkey]["friends"]:
        users[rkey]["friends"].append(key)
    save_users(users)
...
def api_send_private_message():
    ...
    key = find_user_key(users, u)
    tkey = find_user_key(users, target)
    if not key or not tkey or not msg:
        return {"ok": False, "msg": "Eksik bilgi."}
    ensure_user_defaults(users[tkey])
...
def api_join_tournament():
    ...
    ensure_user_defaults(users[key])
    fee = 100
    if not is_owner_name(key) and int(users[key].get("chips", 0)) < fee:
        return {"ok": False, "msg": "Yeterli jeton yok."}
    if tournament in users[key].get("tournaments", []):
        return {"ok": False, "msg": "Zaten kayıtlısın."}
    if not is_owner_name(key):
        users[key]["chips"] = int(users[key].get("chips", 0)) - fee
    users[key].setdefault("tournaments", []).append(tournament)
    add_xp_to_user(users, key, 50, "tournament")
    save_users(users)
    return {"ok": True, "msg": "Turnuvaya katıldın. 100 jeton kesildi.", "profile": profile_payload(key, users[key])}
...
def api_buy_cosmetic():
    ...
    ensure_user_defaults(users[key])
    items = {
        "gold-frame": ("ownedFrames", "gold", 1000),
        "diamond-frame": ("ownedFrames", "diamond", 5000),
        "baroque-frame": ("ownedFrames", "baroque", 7500),
        "rainbow-name": ("ownedNameColors", "rainbow", 3000),
    ...
    if value not in users[key][group]:
        users[key][group].append(value)
    add_xp_to_user(users, key, 20, "cosmetic")
    save_users(users)
    return {"ok": True, "msg": "Satın alındı.", "profile": profile_payload(key, users[key])}
...
def api_premium_tarot():
    ...
    if not is_owner_name(key):
        users[key]["chips"] = int(users[key].get("chips", 0)) - price
    result = (
        "🤖 AI Tarot Premium Yorumu\\n\\n"
        f"Doğum tarihi: {birth or '-'}\\n"
        f"Soru: {question or '-'}\\n"
        f"Fotoğraf notu: {photo or '-'}\\n\\n"
        "Kartların enerjisi güçlü bir dönüşüm, karar ve içsel netleşme dönemini gösteriyor. "
        "Şu anda geçmişte kalan bir döngüyü kapatma ve daha kontrollü, daha bilinçli bir seçim yapma ihtiyacı öne çıkıyor. "
        "Duygusal tarafta sezgilerine güvenmen, acele karar vermemen ve niyetini netleştirmen gerekiyor."
    )
    req = {"time": int(time.time()), "question": question, "birthDate": birth, "photoNote": photo, "result": result}
    users[key].setdefault("premiumTarotRequests", []).insert(0, req)
    add_xp_to_user(users, key, 75, "premium_tarot")
    save_users(users)
    return {"ok": True, "msg": result, "profile": profile_payload(key, users[key])}
...
@socketio.on("request_profile")
def montenoir_request_profile_v1(data):
    account = (data.get("account") or data.get("username") or "").strip()
    users = load_users()
    key = find_user_key(users, account)
    if not key:
        emit("profile_data", {"ok": False, "msg": "Kullanıcı bulunamadı."})
        return
    ensure_user_defaults(users[key])
    unlock_achievements(users, key)
    save_users(users)
    emit("profile_data", {"ok": True, "profile": private_profile(key, users[key]), **profile_payload(key, users[key])})
    emit("profile_loaded", {"ok": True, "profile": private_profile(key, users[key]), **profile_payload(key, users[key])})
