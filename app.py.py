...
import random, string, os, json, hashlib, time
...
def pdata(code):
    r = rooms[code]
    return {'players': r['players'], 'game': r['game'], 'stats': r['stats'], 'locks': r['locks'], 'bets': r['bets'], 'micStates': r.get('micStates', {}), 'room': code}
...
<div id="authModal" class="modal"><div class="modalContent"><button class="closeBtn" onclick="closeModals()">X</button><h2>👤 Üyelik</h2><h3>📝 Kayıt Ol</h3><input id="regUsername" placeholder="Kullanıcı adı"><input id="regEmail" type="email" placeholder="Email"><input id="regPassword" type="password" placeholder="Şifre"><button onclick="registerAccount()">Kayıt Ol</button><hr><h3>🔐 Giriş Yap</h3><input id="loginUsername" placeholder="Kullanıcı adı"><input id="loginPassword" type="password" placeholder="Şifre"><button onclick="loginAccount()">Giriş Yap</button><hr><button onclick="logoutAccount()">🚪 Çıkış Yap</button><p id="authStatus">Henüz giriş yapılmadı.</p></div></div>
...
function playerAvatarSrc(p){
    if(p.avatarData) return p.avatarData;
    return '/static/' + (p.avatar || 'woman.png');
}
...
function registerAccount(){
    let u=regUsername.value.trim(), e=regEmail.value.trim(), p=regPassword.value;
    if(!u||!e||!p){alert('Kullanıcı adı, email ve şifre yaz.');return}
...
function uploadAvatar(){
    ...
    reader.onload = function(){
        socket.emit('upload_avatar',{room:roomCode,avatarData:reader.result});
    };
...
function renderPlayers(players,locks){
    ...
    players.forEach(p=>{
        ...
        if(p.team==='blue'){
            blueLobby.innerHTML+=`<div>${av}<br><span class="${mh.nameClass}">${p.name}</span>${crown}</div>`;
            bluePlayers.innerHTML+=`${av} <span class="${mh.nameClass}">${p.name}</span>${crown} — ${roleLabel(p.role)} — 🪙 ${chips}<br>`;
        }else if(p.team==='red'){
            redLobby.innerHTML+=`<div>${av}<br><span class="${mh.nameClass}">${p.name}</span>${crown}</div>`;
            redPlayers.innerHTML+=`${av} <span class="${mh.nameClass}">${p.name}</span>${crown} — ${roleLabel(p.role)} — 🪙 ${chips}<br>`;
        }else{
            spectatorLobby.innerHTML+=`<div>${av}<br><span class="${mh.nameClass}">${p.name}</span>${crown}</div>`;
...
socket.on('connect',()=>{
    mySid=socket.id;
    restoreLocalFields();
    let a=localStorage.getItem('codenamesAccount');
    if(a){
        playerName.value=a;
        currentAccount=a;
        authStatus.innerHTML='Compte mémorisé : '+a+' (reconnecte-toi si besoin)';
    }
...
def register_account(data):
    ...
    save_users(users)
    emit('register_result', {'ok': True, 'profile': public_profile(username, users[username])})
...
def login_account(data):
    ...
    users = load_users()
    if username not in users or users[username].get('password_hash') != hash_password(password):
        emit('login_result', {'ok': False, 'msg': 'Kullanıcı adı veya şifre yanlış.'})
        return

    emit('login_result', {'ok': True, 'profile': public_profile(username, users[username])})
...
def create_room(data):
    ...
    rooms[code] = {'players': [], 'game': new_game('default'), 'stats': {'blueWins':0,'redWins':0,'history':[],'wordHistory':[],'betHistory':[],'gameNo':0}, 'locks': {'blue':False,'red':False}, 'password': password, 'adminSid': request.sid, 'bets': {}, 'micStates': {}, 'category': 'default'}
...
def join_room_code(data):
    ...
    join_room(code); emit('room_joined', {'room': code}); emit('players_update', {'players': rooms[code]['players'], 'locks': rooms[code]['locks'], 'micStates': rooms[code].get('micStates', {})})
...
def sit(data):
    ...
    if old:
        old.update({'sid':request.sid,'team':data['team'],'role':data['role'],'avatar':data['avatar'],'chips':chips,'account':data.get('account')})
        ...
        player_data={'sid':request.sid,'name':data['name'],'avatar':data['avatar'],'team':data['team'],'role':data['role'],'chips':chips,'isAdmin':rooms[code]['adminSid']==request.sid,'account':data.get('account'),'avatarData':'','nameColor':'default','avatarFrame':'none'}
        acc=data.get('account')
        if acc:
            users=load_users()
            if acc in users:
...
def buy_virtual_chips(data):
    ...
    p=by_sid(code, request.sid)
    if not p: return
    p['chips']=int(p.get('chips',1000))+amount; save_player_to_user(p); emit('game_update', pdata(code), to=code)
