...
function avatarClass(f){return f==='woman.png'?'avatarImg femaleFrame':'avatarImg maleFrame'}function roleLabel(r){if(r==='player')return'Saha Ajanı';if(r==='blueSpy')return'Mavi Spymaster';if(r==='redSpy')return'Kırmızı Spymaster';if(r==='spectator')return'Seyirci';return r}function teamLabel(t){if(t==='blue')return'🔵 Mavi';if(t==='red')return'🔴 Kırmızı';return'👀 Seyirci'}function emitMicState(micOpen, speaking){
...
function renderPlayers(players,locks){
    ...
    function micHtml(p){
        ...
        const nameClass = st.speaking ? ' speakingName' : '';
        const av = `<span class="avatarWrap"><img src="/static/${p.avatar}" class="${avatarClass(p.avatar)}${speakingClass}"><span class="micBadge">${badge}</span></span>`;
        return {av,nameClass};
    }
...
def sit(data):
    ...
    join_room(code); emit('players_update', {'players':rooms[code]['players'],'locks':rooms[code]['locks'],'micStates':rooms[code].get('micStates', {})}, to=code); emit('game_update', pdata(code), to=code)
