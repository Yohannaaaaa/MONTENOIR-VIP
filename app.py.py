...
function spinDailyWheel(){
  ...
  .then(r=>r.json()).then(d=>{
    ...
    dailyWheel.style.transform="rotate("+(1440+(map[prize]||30))+"deg)";
...
function uploadAvatar(){
  ...
  fetch("/api/profile/avatar",{method:"POST",body:fd}).then(r=>r.json()).then(d=>{
    ...
    if(d.ok&&d.avatar){
      try{
        let p=JSON.parse(localStorage.getItem("montenoirProfile")||"{}");
        p.avatar=d.avatar;p.avatarData=d.avatar;
...
def api_daily_reward():
    ...
    # Normal kullanıcılar günde 1 kez çevirir. Yohanna/owner limitsiz.
    ...
    # 1000 + 2000 toplam yaklaşık %5
    prizes = [30, 50, 100, 300, 1000, 2000]
    weights = [40, 30, 18, 7, 3, 2]  # 1000+2000 = %5
