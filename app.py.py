...
def m_public(r):
    return {"code":r["code"],"started":r["started"],"turnIndex":r["turnIndex"],"players":r["players"],"owners":r["owners"],"houses":r["houses"],"hotels":r.get("hotels",{}),"mortgages":r.get("mortgages",{}),"lastLog":r["lastLog"],"lastDice":r["lastDice"]}
...
def mc(data):
    ...
    MONOPOLY_ROOMS[c]=r
    join_room(c)
    colors=["#ff3b30","#007aff","#34c759","#ffd60a","#bf5af2","#ff9500"]; color=colors[(len(r["players"]))%len(colors)]; r["players"][u]={"name":u,"token":t,"color":color,"money":START_MONEY,"position":0,"jailed":0,"bankrupt":False,"hasRolled":False}
...
body{
  margin:0;
  min-height:100vh;
  background:radial-gradient(circle at 45% 0%,#201306 0%,#050505 43%,#000 100%);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  overflow:hidden;
}
.loca{
  ...
  color:#ffd989;text-decoration:none;border:1px solid #d4af37;
  border-radius:13px;background:#000c;padding:11px 18px;font-weight:900;
  box-shadow:0 0 18px #d4af3750;
}
.screen{
  height:100vh;
  display:grid;
...
.boardWrap{
  position:relative;
  width:min(1180px,calc(100vw - 365px));
  height:calc(100vh - 190px);
  min-height:560px;
  margin-top:2px;
  filter:drop-shadow(0 38px 42px rgba(0,0,0,.95));
}
.boardImg{
  width:100%;
  height:100%;
...
.token{
  ...
  background:radial-gradient(circle at 30% 25%,#fff8,#d4af37 16%,#3b2103 58%,#020202);
  border:2px solid #ffe19a;
  box-shadow:0 0 18px #00ff6677,0 12px 16px #000,inset 0 0 10px #ffffff30;
  transition:left .22s ease,top .22s ease,transform .22s ease;
  z-index:40;
}
.token.hop{animation:hop .27s ease}
@keyframes hop{0%{transform:translateY(0) scale(1)}45%{transform:translateY(-18px) scale(1.18)}100%{transform:translateY(0) scale(1)}}
.ownerMark{
  position:absolute;width:46px;height:10px;border-radius:10px;
  border:1px solid #fff9;box-shadow:0 0 10px currentColor;z-index:30;
}
.ownerIcon{
  position:absolute;width:24px;height:24px;border-radius:50%;
  display:grid;place-items:center;font-size:15px;background:#000d;
  border:1px solid #ffe19a;z-index:31;box-shadow:0 0 9px #d4af37aa;
}
.house{
  position:absolute;
  font-size:24px;
  z-index:35;
  filter:drop-shadow(0 0 8px #d4af37) drop-shadow(0 8px 8px #000);
}
.diceBox{
  position:absolute;left:50%;top:60%;
  transform:translate(-50%,-50%);
  z-index:50;perspective:900px;
}
.realDice{
  width:92px;height:92px;position:relative;transform-style:preserve-3d;
  transform:rotateX(-18deg) rotateY(24deg);
  transition:transform .5s cubic-bezier(.2,.8,.2,1);
  filter:drop-shadow(0 23px 18px #000) drop-shadow(0 0 14px #ffffff40);
}
.realDice.rolling{animation:rollDice .8s linear infinite}
@keyframes rollDice{to{transform:rotateX(720deg) rotateY(540deg) rotateZ(360deg)}}
.diceFace{
  position:absolute;width:92px;height:92px;border-radius:18px;
  background:radial-gradient(circle at 30% 22%,#fff,transparent 26%),linear-gradient(145deg,#fff,#e8e8e8 55%,#8a8a8a);
  border:3px solid #fff;box-shadow:inset -12px -12px 22px #0004,inset 9px 9px 18px #ffffffd0;
}
...
.pip{position:absolute;width:15px;height:15px;border-radius:50%;background:radial-gradient(circle at 35% 30%,#444,#030303);box-shadow:0 2px 3px #0008}
.tl{left:19px;top:19px}.tr{right:19px;top:19px}.bl{left:19px;bottom:19px}.br{right:19px;bottom:19px}.center{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:19px;top:50%;transform:translateY(-50%)}.mr{right:19px;top:50%;transform:translateY(-50%)}
.show-1{transform:rotateX(-18deg) rotateY(24deg)}.show-2{transform:rotateX(72deg) rotateY(24deg)}.show-3{transform:rotateX(-18deg) rotateY(204deg)}.show-4{transform:rotateX(-18deg) rotateY(-66deg)}.show-5{transform:rotateX(-108deg) rotateY(24deg)}.show-6{transform:rotateX(72deg) rotateY(24deg)}
.buyPanel{
  position:absolute;left:50%;top:42%;
  transform:translate(-50%,-50%);
  width:420px;min-height:190px;
...
.buyPanel .city{color:#ff9500;font-size:25px;font-weight:900;text-transform:uppercase;margin-bottom:8px}
.buyPanel .question{font-size:22px;font-weight:800;margin-bottom:18px}
.buyPanel .price{font-size:18px;margin-bottom:22px}
.buyPanel button{width:140px;margin:0 8px}
.rollArea{
  position:absolute;left:50%;bottom:-82px;transform:translateX(-50%);
  display:flex;align-items:center;gap:18px;z-index:90;
}
.rollButtonOnBoard{
  width:330px!important;padding:18px 28px!important;border-radius:21px!important;
  border:2px solid #ffd989!important;background:linear-gradient(180deg,#ff5c4d,#b51512 58%,#5d0504)!important;
  color:#fff!important;font-size:34px!important;font-weight:900!important;letter-spacing:1px!important;
  box-shadow:0 0 25px #ff2a2a80,0 14px 22px #000c,inset 0 0 12px #ffffff30!important;
  text-shadow:0 3px 5px #000!important;
}
.rollInfo{
  width:245px;min-height:66px;border:1px solid #d4af37aa;border-radius:15px;background:#000b;
  padding:12px;color:#fff;box-shadow:0 0 16px #d4af3738;font-weight:800;
}
.rollInfo b{color:#ffd989}
.side{
  background:linear-gradient(180deg,#111e,#000e);
  border:1px solid #d4af37aa;border-radius:18px;padding:14px;
  box-shadow:0 0 30px #000,inset 0 0 14px #d4af3714;overflow:auto;
}
h2{color:#ffd989;margin:8px 0 12px}
input,select,button{
  width:100%;padding:12px;border-radius:13px;border:1px solid #d4af37;margin:6px 0;
}
input,select{background:#050505;color:#fff}
button{background:linear-gradient(180deg,#ffe08a,#a96d0d);font-weight:900;cursor:pointer;color:#160a00}
button.dark{background:linear-gradient(180deg,#1b1b1b,#050505);color:#fff}
.player{
  display:flex;align-items:center;gap:12px;border:1px solid #d4af375a;border-radius:13px;padding:10px;margin:8px 0;background:#ffffff08;
}
.player.turn{border-color:#00ff66;box-shadow:0 0 18px #00ff6680}
.picon{width:42px;height:42px;border-radius:50%;display:grid;place-items:center;font-size:24px;background:radial-gradient(circle,#d4af37,#120800);border:1px solid #ffe19a}
.moneyToast{
  position:fixed;left:18px;bottom:22px;width:245px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;
  color:#00ff66;font-weight:900;box-shadow:0 0 20px #d4af3760;z-index:200;opacity:0;transition:.25s;
}
.moneyToast.show{opacity:1}
.propertiesDock{
  position:fixed;left:50%;bottom:12px;transform:translateX(-50%);
  width:min(960px,70vw);height:118px;border:1px solid #d4af37aa;border-radius:17px;background:linear-gradient(180deg,#111e,#000e);
  display:flex;gap:12px;align-items:center;padding:16px;z-index:100;box-shadow:0 0 25px #000;
  overflow-x:auto;
}
.propertiesDockTitle{position:absolute;top:-25px;left:50%;transform:translateX(-50%);color:#ffd989;font-weight:900}
.propCard{min-width:205px;height:86px;border:1px solid #d4af37aa;border-radius:12px;background:linear-gradient(180deg,#a55a0b,#211000);overflow:hidden}
.propCardHead{height:30px;display:flex;justify-content:center;align-items:center;gap:8px;font-weight:900}
.buildSlots{display:flex;gap:7px;justify-content:center;padding:8px}
.buildSlot{width:30px;height:28px;border:1px solid #ffffff50;border-radius:6px;background:#0008;display:grid;place-items:center;font-size:19px}
.buildSlot.canBuild{cursor:pointer;box-shadow:0 0 12px #00ff6670}
.propPrice{text-align:center;color:#ffd989;font-size:12px}
.groupPanel{border:1px solid #d4af375c;border-radius:13px;padding:10px;margin-top:12px}
.groupRow{display:flex;justify-content:space-between;margin:7px 0;font-size:14px}
.log{display:none!important}
@media(max-width:1000px){
  body{overflow:auto}.screen{height:auto;grid-template-columns:1fr;padding-bottom:180px}.boardWrap{width:98vw;height:64vw;min-height:0}.side{width:auto}.propertiesDock{width:94vw}.rollArea{bottom:-92px}.rollButtonOnBoard{width:230px!important;font-size:24px!important}
}
</style>
</head>
<body>
...
<div class="screen">
 <div class="main">
  <div class="boardWrap" id="boardWrap">
   <img class="boardImg" src="/static/monopoly_board_lux_3d.png" alt="Metropoly Board">
   <div class="diceBox">
    <div id="diceCube" class="realDice show-1">
     <div class="diceFace front"><span class="pip center"></span></div>
     <div class="diceFace back"><span class="pip tl"></span><span class="pip center"></span><span class="pip br"></span></div>
     <div class="diceFace right"><span class="pip tl"></span><span class="pip tr"></span><span class="pip bl"></span><span class="pip br"></span></div>
     <div class="diceFace left"><span class="pip tl"></span><span class="pip tr"></span><span class="pip center"></span><span class="pip bl"></span><span class="pip br"></span></div>
     <div class="diceFace top"><span class="pip tl"></span><span class="pip tr"></span><span class="pip ml"></span><span class="pip mr"></span><span class="pip bl"></span><span class="pip br"></span></div>
     <div class="diceFace bottom"><span class="pip tl"></span><span class="pip br"></span></div>
    </div>
   </div>
   ...
   <div class="rollArea">
    <button class="rollButtonOnBoard" onclick="rollDice()">🎲 ZAR AT</button>
    <div id="rollInfo" class="rollInfo"><b>Kalan Atış Hakkın: 1</b><br>6 gelirsen tekrar atabilirsin!</div>
   </div>
  </div>
 </div>
 <div class="side">
  <h2>🎩 Metropoly Online</h2>
  <input id="username" placeholder="Kullanıcı adı">
  <select id="tokenSelect">
   <option value="✈️">✈️ Jet</option><option value="🎰">🎰 Casino</option><option value="🛥️">🛥️ Yacht</option><option value="🚗">🚗 Sport Car</option><option value="🎩">🎩 Hat</option><option value="🐈">🐈 Cat</option>
  </select>
  <button onclick="createRoom()">ODA OLUŞTUR</button>
  <input id="roomCodeInput" placeholder="MON-1234">
  <button class="dark" onclick="joinRoom()">ODAYA GİR</button>
  <button onclick="startGame()">OYUNU BAŞLAT</button>
  <button class="dark" onclick="endTurn()">TURU BİTİR</button>
  <h2>Oda: <span id="roomCode">-</span></h2>
  <div id="players"></div>
  <div class="groupPanel"><h2>RENK GRUPLARI</h2><div id="groupStatus"></div></div>
  <div id="housesPanel" style="display:none"></div>
  <div id="log" class="log"></div>
 </div>
</div>
<div id="moneyToast" class="moneyToast"></div>
<div id="propertiesDock" class="propertiesDock"><div class="propertiesDockTitle">EV İNŞA ET</div></div>

<script>
const socket=io();
let roomCode="",myName="",currentState=null,previousMoneyState={};
const coords=[
[89,63],[88,78],[78,79],[66,79],[56,79],[46,79],[36,79],[26,79],[12,78],
[11,66],[11,56],[11,46],[11,36],[11,27],[11,20],[11,13],[16,8],
[28,8],[39,8],[50,8],[59,8],[70,8],[80,8],[90,8],[90,20],[90,31],[90,43],[90,55]
];
...
const groupNames={green:"Yeşil",brown:"Kahverengi",orange:"Turuncu",purple:"Mor",yellow:"Sarı",blue:"Mavi",red:"Kırmızı"};
function getGroup(i){for(const [g,a] of Object.entries(groupMap)){if(a.includes(i))return g}return ""}
...
function endTurn(){socket.emit("monopoly_end_turn",{code:roomCode,username:myName})}
function hideBuy(){buyPanel.style.display="none"}
function showDice(n){diceCube.classList.remove("show-1","show-2","show-3","show-4","show-5","show-6");diceCube.classList.add("show-"+n)}
...
function logMsg(t){ if(log){log.innerHTML=t+"<br>"+log.innerHTML} }
function render(st){
 currentState=st;roomCode=st.code;roomCodeInput.value=st.code;roomCode.textContent=st.code;
 document.querySelectorAll(".token,.house,.ownerMark,.ownerIcon").forEach(e=>e.remove());
 addOwnerMarks(st);
 const names=Object.keys(st.players);players.innerHTML="";
 names.forEach((n,i)=>{const p=st.players[n],turn=i===st.turnIndex;const d=document.createElement("div");d.className="player"+(turn?" turn":"");d.innerHTML=`<div class="picon" style="box-shadow:0 0 14px ${p.color||"#d4af37"};border-color:${p.color||"#d4af37"}">${p.token}</div><div><b><span style="color:${p.color||"#d4af37"}">●</span> ${n}</b><br>${p.money}€</div><div style="margin-left:auto">🎲 ${p.hasRolled?0:1}</div>`;players.appendChild(d);addToken(p,i)});
 Object.entries(st.houses||{}).forEach(([cell,n])=>{const c=coords[+cell]||[50,50];for(let i=0;i<n;i++){const h=document.createElement("div");h.className="house";h.textContent="🏠";h.style.left=`calc(${c[0]}% - 18px + ${i*13}px)`;h.style.top=`calc(${c[1]}% - 46px)`;boardWrap.appendChild(h)}});
 Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(n){const c=coords[+cell]||[50,50];const h=document.createElement("div");h.className="house";h.textContent="🏨";h.style.left=`calc(${c[0]}% - 20px)`;h.style.top=`calc(${c[1]}% - 50px)`;boardWrap.appendChild(h)}});
 if(st.lastDice)showDice(st.lastDice);
 detectMoneyChanges(st);renderPropertiesDock(st);renderGroupStatus(st);updateRollInfo(st);
}
function addToken(p,i){const c=coords[p.position]||[89,63];const el=document.createElement("div");el.className="token";el.textContent=p.token;el.style.left=`calc(${c[0]}% - 23px + ${i*7}px)`;el.style.top=`calc(${c[1]}% - 23px + ${i*7}px)`;boardWrap.appendChild(el)}
function addOwnerMarks(st){Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const c=coords[+cell]||[50,50];const m=document.createElement("div");m.className="ownerMark";m.style.background=p.color||"#d4af37";m.style.color=p.color||"#d4af37";m.style.left=`calc(${c[0]}% - 22px)`;m.style.top=`calc(${c[1]}% + 18px)`;boardWrap.appendChild(m);const ic=document.createElement("div");ic.className="ownerIcon";ic.textContent=p.token;ic.style.left=`calc(${c[0]}% + 17px)`;ic.style.top=`calc(${c[1]}% + 10px)`;boardWrap.appendChild(ic)})}
function animateOneToken(username,from,to,state,done){const names=Object.keys(state.players);const idx=names.indexOf(username);let steps=[],cur=from,total=coords.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const el=[...document.querySelectorAll(".token")][idx];if(!el){done();return}let k=0;const timer=setInterval(()=>{if(k>=steps.length){clearInterval(timer);done();return}const c=coords[steps[k]]||[50,50];el.style.left=`calc(${c[0]}% - 23px + ${idx*7}px)`;el.style.top=`calc(${c[1]}% - 23px + ${idx*7}px)`;el.classList.remove("hop");void el.offsetWidth;el.classList.add("hop");k++},270)}
function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const diff=p.money-previousMoneyState[n];if(diff>0)showMoneyToast(n+" "+diff+"€ aldı bankadan",true);if(diff<0)showMoneyToast(n+" "+Math.abs(diff)+"€ ödedi",false)}previousMoneyState[n]=p.money})}
function updateRollInfo(st){if(!st.players[myName])return;const p=st.players[myName];const can=!p.hasRolled||st.lastDice===6;rollInfo.innerHTML=`<b>Kalan Atış Hakkın: ${can?1:0}</b><br>6 gelirsen tekrar atabilirsin!`}
function renderPropertiesDock(st){propertiesDock.innerHTML='<div class="propertiesDockTitle">EV İNŞA ET</div>';if(!st.players[myName])return;const owned=Object.entries(st.owners||{}).filter(([c,o])=>o===myName);if(!owned.length){propertiesDock.innerHTML+='<div style="color:#aaa;padding:38px 10px;font-size:13px">Henüz mülkün yok.</div>';return}owned.forEach(([cell,owner])=>{const i=+cell,houses=+(st.houses?.[cell]||0),hotel=+(st.hotels?.[cell]||0);const card=document.createElement("div");card.className="propCard";card.innerHTML=`<div class="propCardHead">${cells[i]} <span>${st.players[owner].token}</span></div><div class="buildSlots"></div><div class="propPrice">MALİYET: 50€</div>`;const slots=card.querySelector(".buildSlots");if(hotel){slots.innerHTML='<div class="buildSlot">🏨</div>'}else{for(let s=0;s<4;s++){const slot=document.createElement("div");slot.className="buildSlot "+(s===houses?"canBuild":"");slot.textContent=s<houses?"🏠":"";if(s===houses)slot.onclick=()=>socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:i});slots.appendChild(slot)}const ot=document.createElement("div");ot.className="buildSlot "+(houses>=4?"canBuild":"");ot.textContent=houses>=4?"🏨":"";if(houses>=4)ot.onclick=()=>socket.emit("monopoly_add_hotel",{code:roomCode,username:myName,cell:i});slots.appendChild(ot)}propertiesDock.appendChild(card)})}
function renderGroupStatus(st){groupStatus.innerHTML="";Object.entries(groupMap).forEach(([g,arr])=>{const count=arr.filter(i=>(st.owners||{})[i]===myName).length;groupStatus.innerHTML+=`<div class="groupRow"><span>${groupNames[g]}</span><span>${count} / ${arr.length}</span></div>`})}
socket.on("monopoly_room_state",render);
socket.on("monopoly_error",d=>alert(d.msg));
socket.on("monopoly_animate_move",d=>{diceCube.classList.add("rolling");setTimeout(()=>{diceCube.classList.remove("rolling");showDice(d.dice);if(currentState){animateOneToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]?.position===d.to){buyCity.textContent=cells[d.to];buyPrice.textContent="Fiyat: "+prices[d.to]+" €";buyPanel.style.display="block"}})}else{render(d.state)}},850)});
