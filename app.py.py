...
@app.route("/monopoly")
def monopoly_page():
    return r"""<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>METROPOLY NEW BOARD</title><script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script><style>
*{box-sizing:border-box}body{margin:0;overflow:hidden;background:#050505;color:white;font-family:Arial}.top{position:fixed;left:12px;right:12px;top:10px;height:54px;z-index:100;display:flex;gap:8px;align-items:center;padding:7px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#171717,#030303);box-shadow:0 0 22px #000}.top input,.top select,.top button{height:38px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:white;padding:8px 10px;font-weight:900}.top button{background:linear-gradient(#ffe08a,#b87913);color:#160800;cursor:pointer}.loca{color:#ffd989;text-decoration:none;font-weight:900;border:1px solid #d4af37;border-radius:12px;padding:9px 14px;background:#000}.room{color:#ffd989;font-weight:900}#wrap{position:fixed;left:0;right:0;top:72px;bottom:0;overflow:hidden;background:#000}#board{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(100vw,calc((100vh - 72px)*1.777));height:auto;pointer-events:none}.tok{position:absolute;width:42px;height:42px;border-radius:50%;display:grid;place-items:center;transform:translate(-50%,-50%);font-size:27px;background:#0008;border:2px solid currentColor;box-shadow:0 0 16px currentColor,0 8px 12px #000;z-index:30}.owner{position:absolute;width:42px;height:7px;border-radius:8px;transform:translate(-50%,-50%);box-shadow:0 0 8px currentColor;z-index:24}.build{position:absolute;transform:translate(-50%,-50%);font-size:18px;z-index:26;white-space:nowrap}.roll{position:absolute;left:50%;top:47%;transform:translate(-50%,-50%);z-index:80;width:210px;height:58px;border-radius:20px;border:2px solid #ffd989;background:linear-gradient(#f7c863,#b87913 55%,#4d2700);color:#180900;font-size:26px;font-weight:900;box-shadow:0 0 26px #d4af3788,0 12px 24px #000;cursor:pointer}.dice{position:absolute;left:50%;top:37%;width:92px;height:92px;transform-style:preserve-3d;transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg);z-index:70;filter:drop-shadow(0 18px 14px #000);pointer-events:none}.dice.rolling{animation:diceRoll 1.15s cubic-bezier(.14,.82,.31,1) both}@keyframes diceRoll{0%{left:46%;top:32%;transform:translate(-50%,-50%) rotateX(0) rotateY(0) rotateZ(0) scale(.82)}25%{left:50%;top:39%;transform:translate(-50%,-50%) rotateX(280deg) rotateY(360deg) rotateZ(180deg) scale(1.08)}55%{left:53%;top:43%;transform:translate(-50%,-50%) rotateX(640deg) rotateY(760deg) rotateZ(380deg)}77%{left:49%;top:36%;transform:translate(-50%,-50%) rotateX(880deg) rotateY(1000deg) rotateZ(540deg) scale(.95)}100%{left:50%;top:37%;transform:translate(-50%,-50%) rotateX(1080deg) rotateY(1260deg) rotateZ(720deg) scale(1)}}.face{position:absolute;width:92px;height:92px;border-radius:9px;background:linear-gradient(145deg,#fff,#e7e7e7 55%,#888);border:3px solid #fff}.front{transform:translateZ(46px)}.back{transform:rotateY(180deg) translateZ(46px)}.right{transform:rotateY(90deg) translateZ(46px)}.leftf{transform:rotateY(-90deg) translateZ(46px)}.topf{transform:rotateX(90deg) translateZ(46px)}.bottomf{transform:rotateX(-90deg) translateZ(46px)}.pip{position:absolute;width:14px;height:14px;border-radius:50%;background:#111}.tl{left:19px;top:19px}.tr{right:19px;top:19px}.bl{left:19px;bottom:19px}.br{right:19px;bottom:19px}.cc{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:19px;top:50%;transform:translateY(-50%)}.mr{right:19px;top:50%;transform:translateY(-50%)}.show1{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg)}.show2{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}.show3{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(208deg)}.show4{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(-62deg)}.show5{transform:translate(-50%,-50%) rotateX(-108deg) rotateY(28deg)}.show6{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}.hot{position:absolute;z-index:90;border-radius:12px;cursor:pointer}.hot:hover{outline:2px solid #fff8}.panel{position:fixed;left:50%;top:44%;transform:translate(-50%,-50%);z-index:150;width:410px;max-width:92vw;padding:22px;border-radius:22px;background:#000e;border:1px solid #d4af37;box-shadow:0 0 30px #d4af3777;text-align:center;display:none}.panel button{border:1px solid #d4af37;border-radius:12px;padding:12px 18px;margin:8px;font-weight:900;cursor:pointer;background:linear-gradient(#ffe08a,#b87913);color:#150800}.dark{background:linear-gradient(#cf4d42,#6d120b)!important;color:white!important}.players{position:fixed;right:12px;top:74px;width:260px;max-height:40vh;z-index:95;border:1px solid #d4af37aa;border-radius:16px;background:#000c;padding:10px;overflow:auto}.player{display:flex;align-items:center;gap:8px;border:1px solid #d4af3755;border-radius:12px;margin:6px 0;padding:8px;background:#111c}.turn{border-color:#00ff66!important}.dot{width:16px;height:16px;border-radius:50%;border:2px solid #fff9}.money{color:#ffd989}.toast,.chatlog{position:fixed;left:16px;bottom:18px;z-index:160;width:320px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.show{opacity:1}textarea{width:100%;height:110px;border:1px solid #d4af37;border-radius:14px;background:#070707;color:white;padding:12px;font-size:15px}
</style></head><body><div class="top"><a class="loca" href="/">▮ LOCA</a><input id="username" placeholder="Kullanıcı adı"><select id="tokenSelect"><option value="✈️">✈️ Avion</option><option value="🎰">🎰 Machine</option><option value="🛥️">🛥️ Bateau</option><option value="🚗">🚗 Voiture</option><option value="🎩">🎩 Chapeau</option><option value="🐈">🐈 Chat noir</option></select><button onclick="createRoom()">ODA OLUŞTUR</button><input id="roomCodeInput" placeholder="MON-1234"><button onclick="joinRoom()">ODAYA GİR</button><button onclick="startGame()">OYUNU BAŞLAT</button><button onclick="openChat()">CHAT</button><span id="topRoomLabel" class="room">Oda: -</span></div><div id="wrap"><img id="board" src="/static/monopoly_board_lux_3d.png"><div id="dice" class="dice show1"><div class="face front"><span class="pip cc"></span></div><div class="face back"><span class="pip tl"></span><span class="pip cc"></span><span class="pip br"></span></div><div class="face right"><span class="pip tl"></span><span class="pip tr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face leftf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip cc"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face topf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip ml"></span><span class="pip mr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face bottomf"><span class="pip tl"></span><span class="pip br"></span></div></div><button class="roll" onclick="rollDice()">🎲 ZAR AT</button><div id="tokens"></div><div id="owners"></div><div id="builds"></div><div id="houseBtn" class="hot" onclick="buildHouse()"></div><div id="hotelBtn" class="hot" onclick="buildHotel()"></div></div><div class="players"><b>ONLINE OYUNCULAR</b><div id="playersList"></div></div><div id="buyPanel" class="panel"><div id="buyCity" style="font-size:27px;font-weight:900;color:#ffb000">PARIS</div><div style="font-size:21px;font-weight:900;margin:12px 0">satın almak ister misin?</div><div id="buyPrice">Fiyat: 160 €</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">VAZGEÇ</button></div><div id="chatPanel" class="panel"><h3>CHAT</h3><textarea id="chatInput" placeholder="Mesaj yaz..."></textarea><button onclick="sendChat()">GÖNDER</button><button onclick="chatPanel.style.display='none'">KAPAT</button></div><div id="moneyToast" class="toast"></div><div id="chatLog" class="chatlog"></div>
<script>
const socket=io();let roomCode="",myName="",currentState=null,lastCell=null,previousMoneyState={};
const cells=["ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Amsterdam","Eaux","Moscou","?","Rome","Madrid","Impôts","Sofia","Belgrad","Gare","Istanbul","Prague","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo"];
