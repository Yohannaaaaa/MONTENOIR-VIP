...
def monopoly_owns_full_group(r,u,cell):
    arr=monopoly_get_group(cell)
    if not arr:
        return False
    owners=r.get("owners",{})
    return all(owners.get(str(c))==u for c in arr)
...
def monopoly_update_settings(data):
    ...
    if not r:
        return
    r["settings"]={
        "players": int((data or {}).get("players",4) or 4),
        "turns": (data or {}).get("turns","illimite"),
        "target": int((data or {}).get("target",7000) or 7000)
    }
    emit("monopoly_room_state",m_public(r),room=c)
...
@app.route("/monopoly")
def monopoly_page():
    return '<!DOCTYPE html>\n<html lang="tr">\n<head>\n<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width,initial-scale=1">\n<title>METROPOLY TRUE 3D QUALITY</title>\n<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>\n<script src="https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js"></script>\n<script src="https://cdn.jsdelivr.net/npm/three@0.160.0/examples/js/loaders/GLTFLoader.js"></script>\n<style>\n*{box-sizing:border-box}\nbody{margin:0;overflow:hidden;background:#050505;color:white;font-family:Arial,Helvetica,sans-serif}\n.top{position:fixed;left:12px;right:12px;top:10px;height:54px;z-index:120;display:flex;gap:8px;align-items:center;padding:7px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#171717,#030303);box-shadow:0 0 22px #000}\n.top input,.top select,.top button{height:38px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:white;padding:8px 10px;font-weight:900}\n.top button{background:linear-gradient(#ffe08a,#b87913);color:#160800;cursor:pointer}\n.loca{color:#ffd989;text-decoration:none;font-weight:900;border:1px solid #d4af37;border-radius:12px;padding:9px 14px;background:#000}\n.room{color:#ffd989;font-weight:900;white-space:nowrap}\n#stage{position:fixed;left:0;right:0;top:72px;bottom:0;background:radial-gradient(circle at 50% 10%,#2a1807,#050505 58%,#000);overflow:hidden}\n#threeLayer{position:absolute;inset:0;z-index:1}\n#overlay{position:absolute;inset:0;z-index:20;pointer-events:none}\n.tok{position:absolute;width:62px;height:62px;border-radius:50%;display:grid;place-items:center;transform:translate(-50%,-50%);font-size:36px;background:radial-gradient(circle at 28% 22%,#fff9,#fff0 30%),linear-gradient(145deg,#fff0a8,#d8a62c 42%,#8a5207 72%,#2b1600);border:3px solid #fff0a8;box-shadow:0 0 24px rgba(255,215,120,.85),0 18px 18px rgba(0,0,0,.95),inset -12px -12px 18px rgba(0,0,0,.58),inset 8px 8px 14px rgba(255,255,255,.48);z-index:45;will-change:left,top,transform;pointer-events:none}\n.tok::before{content:"";position:absolute;inset:-8px;border-radius:50%;border:3px solid currentColor;box-shadow:0 0 20px currentColor;opacity:.9}\n.tok::after{content:"";position:absolute;left:7px;right:7px;bottom:-11px;height:11px;border-radius:50%;background:rgba(0,0,0,.8);filter:blur(6px);z-index:-1}\n.tok.tap{animation:pawnTap .28s ease}\n@keyframes pawnTap{0%{filter:brightness(1)}45%{filter:brightness(1.38);transform:translate(-50%,-50%) scale(1.07)}100%{filter:brightness(1)}}\n.ownerDot{position:absolute;width:23px;height:23px;border-radius:50%;transform:translate(-50%,-50%);border:3px solid #ffe7a6;box-shadow:0 0 13px currentColor,0 0 7px rgba(255,255,255,.65),inset 3px 3px 6px rgba(255,255,255,.55),inset -4px -4px 7px rgba(0,0,0,.45);z-index:34;pointer-events:auto}\n.ownerDot::after{content:"";position:absolute;left:50%;top:50%;width:8px;height:8px;border-radius:50%;transform:translate(-50%,-50%);background:rgba(255,255,255,.78)}\n.build{position:absolute;transform:translate(-50%,-50%);font-size:22px;z-index:36;white-space:nowrap;filter:drop-shadow(0 0 5px #000);pointer-events:none}\n.dicePair{position:absolute;left:50%;top:39%;transform:translate(-50%,-50%);z-index:80;display:flex;gap:24px;pointer-events:none}\n.diceBox{width:86px;height:86px;border-radius:12px;background:radial-gradient(circle at 30% 20%,#fff6,#fff0 26%),linear-gradient(145deg,#ffdf77,#b97813 58%,#3a1f00);border:3px solid #ffe7a6;box-shadow:inset -10px -10px 18px #0008,inset 7px 7px 14px #fff5,0 18px 14px #000;position:relative}\n.dicePair.rolling .diceBox:first-child{animation:diceA 1.1s ease both}\n.dicePair.rolling .diceBox:last-child{animation:diceB 1.1s ease both}\n@keyframes diceA{0%{transform:translate(-60px,-42px) rotate(0deg) scale(.85)}30%{transform:translate(22px,16px) rotate(330deg) scale(1.08)}60%{transform:translate(-12px,-8px) rotate(620deg) scale(.98)}100%{transform:translate(0,0) rotate(720deg) scale(1)}}\n@keyframes diceB{0%{transform:translate(60px,-42px) rotate(0deg) scale(.85)}30%{transform:translate(-22px,18px) rotate(-330deg) scale(1.08)}60%{transform:translate(12px,-8px) rotate(-620deg) scale(.98)}100%{transform:translate(0,0) rotate(-720deg) scale(1)}}\n.diceDot{position:absolute;width:15px;height:15px;border-radius:50%;background:#050505;box-shadow:inset 2px 2px 3px #ffffff44}\n.tl{left:18px;top:18px}.tr{right:18px;top:18px}.bl{left:18px;bottom:18px}.br{right:18px;bottom:18px}.cc{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:18px;top:50%;transform:translateY(-50%)}.mr{right:18px;top:50%;transform:translateY(-50%)}\n.roll{position:absolute;left:50%;top:48%;transform:translate(-50%,-50%);z-index:90;width:210px;height:58px;border-radius:20px;border:2px solid #ffd989;background:linear-gradient(#f7c863,#b87913 55%,#4d2700);color:#180900;font-size:26px;font-weight:900;box-shadow:0 0 26px #d4af3788,0 12px 24px #000;cursor:pointer;pointer-events:auto}\n.buildControls{position:fixed;left:16px;top:92px;z-index:140;display:flex;flex-direction:column;gap:12px}\n.buildBtn{width:150px;height:56px;border-radius:16px;border:2px solid #ffd989;background:linear-gradient(145deg,#ffdb70,#a96b0b 58%,#3d2100);color:#1b0b00;font-size:18px;font-weight:900;box-shadow:0 0 20px rgba(212,175,55,.55),0 10px 18px #000;cursor:pointer}\n.buildBtn.hotel{background:linear-gradient(145deg,#fff0a8,#c78b16 45%,#5a2e00)}\n.buildBtn:disabled{filter:grayscale(.9) brightness(.55);cursor:not-allowed;opacity:.62}\n.buildHint{width:150px;color:#ffd989;background:#000c;border:1px solid #d4af37aa;border-radius:12px;font-size:11px;font-weight:900;padding:8px;line-height:1.25;text-align:center}\n.panel{position:fixed;left:50%;top:44%;transform:translate(-50%,-50%);z-index:160;width:410px;max-width:92vw;padding:22px;border-radius:22px;background:#000e;border:1px solid #d4af37;box-shadow:0 0 30px #d4af3777;text-align:center;display:none}\n.panel button{border:1px solid #d4af37;border-radius:12px;padding:12px 18px;margin:8px;font-weight:900;cursor:pointer;background:linear-gradient(#ffe08a,#b87913);color:#150800}\n.dark{background:linear-gradient(#cf4d42,#6d120b)!important;color:white!important}\n.players{position:fixed;right:12px;top:74px;width:270px;max-height:43vh;z-index:125;border:1px solid #d4af37aa;border-radius:16px;background:#000c;padding:10px;overflow:auto}\n.player{display:flex;align-items:center;gap:8px;border:1px solid #d4af3755;border-radius:12px;margin:6px 0;padding:8px;background:#111c}.turn{border-color:#00ff66!important;box-shadow:0 0 12px #00ff6670}.dot{width:16px;height:16px;border-radius:50%;border:2px solid #fff9}.money{color:#ffd989}\n.toast,.chatlog{position:fixed;left:16px;bottom:18px;z-index:170;width:340px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.show{opacity:1}\ntextarea{width:100%;height:110px;border:1px solid #d4af37;border-radius:14px;background:#070707;color:white;padding:12px;font-size:15px}\n.mode{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:165;width:410px;background:#fff;color:#444;border-radius:20px;display:none;overflow:hidden}.mh{text-align:center;padding:18px;font-size:28px;font-weight:900;border-bottom:1px solid #eee}.ms{padding:18px 24px;border-bottom:1px solid #eee}.mt{font-size:24px;margin-bottom:18px}.mc{display:flex;gap:32px;color:#aaa;font-size:20px;font-weight:900}.mc span{cursor:pointer}.mc .active{color:#1f8fe5}.next{width:calc(100% - 32px);margin:16px;border:0;border-radius:18px;background:#1f8fe5;color:white;font-weight:900;font-size:20px;padding:16px}\n</style>\n</head>\n<body>\n<div class="top">\n<a class="loca" href="/">▮ LOCA</a>\n<input id="username" placeholder="Kullanıcı adı">\n<select id="tokenSelect"><option value="✈️">✈️ Avion</option><option value="🎰">🎰 Machine</option><option value="🛥️">🛥️ Bateau</option><option value="🚗">🚗 Voiture</option><option value="🎩">🎩 Chapeau</option><option value="🐈">🐈 Chat noir</option></select>\n<button onclick="createRoom()">ODA OLUŞTUR</button>\n<input id="roomCodeInput" placeholder="MON-1234">\n<button onclick="joinRoom()">ODAYA GİR</button>\n<button onclick="startGame()">OYUNU BAŞLAT</button>\n<button onclick="modePanel.style.display=\'block\'">OYUN MODU</button>\n<button onclick="openChat()">CHAT</button>\n<span id="topRoomLabel" class="room">Oda: -</span>\n</div>\n<div id="stage">\n<div id="threeLayer"></div>\n<div id="overlay">\n<div id="dicePair" class="dicePair"><div id="diceA" class="diceBox"></div><div id="diceB" class="diceBox"></div></div>\n<button class="roll" onclick="rollDice()">🎲 ZAR AT</button>\n<div id="tokens"></div><div id="owners"></div><div id="builds"></div>\n</div>\n</div>\n<div class="buildControls"><button id="buildHouseBtn" class="buildBtn" onclick="buildHouse()">🏠 EV AL</button><button id="buildHotelBtn" class="buildBtn hotel" onclick="buildHotel()">🏨 OTEL AL</button><div id="buildHint" class="buildHint">Ev için aynı renk serisini tamamla.</div></div>\n<div class="players"><b>ONLINE OYUNCULAR</b><div id="playersList"></div></div>\n<div id="buyPanel" class="panel"><div id="buyCity" style="font-size:27px;font-weight:900;color:#ffb000">PARIS</div><div style="font-size:21px;font-weight:900;margin:12px 0">satın almak ister misin?</div><div id="buyPrice">Fiyat: 160 €</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">VAZGEÇ</button></div>\n<div id="chatPanel" class="panel"><h3>CHAT</h3><textarea id="chatInput" placeholder="Mesaj yaz..."></textarea><button onclick="sendChat()">GÖNDER</button><button onclick="chatPanel.style.display=\'none\'">KAPAT</button></div>\n<div id="modePanel" class="mode"><div class="mh">PARAMÈTRES</div><div class="ms"><div class="mt">Mode de jeu</div><div class="mc"><span class="active">EN DIRECT</span></div></div><div class="ms"><div class="mt">Joueurs</div><div class="mc" id="modePlayers"><span>2</span><span>3</span><span class="active">4</span></div></div><div class="ms"><div class="mt">Tours</div><div class="mc" id="modeTurns"><span>30</span><span>50</span><span class="active">ILLIMITÉ</span></div></div><div class="ms"><div class="mt">Objectif de patrimoine</div><div class="mc" id="modeTarget"><span>3000 $</span><span>5000 $</span><span class="active">7000 $</span></div></div><button class="next" onclick="saveMode()">SUIVANT</button></div>\n<div id="moneyToast" class="toast"></div><div id="chatLog" class="chatlog"></div>\n<script>\nconst socket=io();let roomCode="",myName="",currentState=null,lastCell=null,previousMoneyState={};\nconst cells=["ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Amsterdam","Eaux","Moscou","Caisse de Communauté","Rome","Madrid","Impôts","Sofia","Belgrad","Gare","Istanbul","Prague","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo"];\nconst prices=[0,0,420,400,380,360,260,260,0,300,280,0,260,0,260,250,0,180,190,0,200,220,240,0,420,420,0,450];\nconst coords=[[82.5,51.7],[79.3,68.9],[70.2,68.4],[60.9,68.1],[51.3,68.2],[42.1,68.3],[32.7,68.6],[23.5,68.7],[14.8,68.5],[18.3,57.4],[18.7,50.5],[19.1,43.4],[19.6,34.0],[19.9,29.2],[20.2,24.2],[20.4,17.7],[26.7,8.2],[34.8,8.2],[43.0,8.2],[51.1,8.1],[58.9,8.1],[66.8,8.1],[74.7,8.1],[82.7,8.1],[83.1,18.0],[83.3,28.0],[83.7,39.6],[84.2,50.2]];\nconst colorGroups={green:[2,3,5],brown:[6,7],orange:[9,10],purple:[12,14,15],yellow:[17,18,20],blue:[21,22],red:[24,25,27]};\nlet scene,camera,renderer,boardMesh,boardPlane,boardBox=null;\ninit3D();\nfunction init3D(){\n const c=threeLayer;\n scene=new THREE.Scene();\n camera=new THREE.PerspectiveCamera(24,c.clientWidth/c.clientHeight,.1,100);\n camera.position.set(0,13.6,2.2);\n camera.lookAt(0,0,0);\n renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});\n renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));\n renderer.setSize(c.clientWidth,c.clientHeight);\n renderer.shadowMap.enabled=true;\n c.appendChild(renderer.domElement);\n scene.add(new THREE.AmbientLight(0xffffff,.82));\n const spot=new THREE.SpotLight(0xffdc91,2.6,50,0.6,0.35,1);\n spot.position.set(0,10,5);spot.castShadow=true;scene.add(spot);\n const rim=new THREE.DirectionalLight(0x9fc7ff,.55);rim.position.set(-4,6,-6);scene.add(rim);\n const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png",()=>{renderer.render(scene,camera);updateBoardBox()});\n tex.colorSpace=THREE.SRGBColorSpace;\n boardMesh=new THREE.Mesh(new THREE.BoxGeometry(15.8,.22,9.85),new THREE.MeshStandardMaterial({map:tex,roughness:.42,metalness:.08}));\n boardMesh.receiveShadow=true;boardMesh.castShadow=true;scene.add(boardMesh);\n const base=new THREE.Mesh(new THREE.BoxGeometry(16.1,.28,10.15),new THREE.MeshStandardMaterial({color:0x100905,roughness:.35,metalness:.42}));\n base.position.y=-.26;scene.add(base);\n window.addEventListener("resize",resize3D);\n animate();\n}\nfunction animate(){requestAnimationFrame(animate);renderer.render(scene,camera)}\nfunction resize3D(){const c=threeLayer;camera.aspect=c.clientWidth/c.clientHeight;camera.updateProjectionMatrix();renderer.setSize(c.clientWidth,c.clientHeight);updateBoardBox();if(currentState)render(currentState)}\nfunction updateBoardBox(){if(!boardMesh)return;const v=[[-7.9,0,-4.925],[7.9,0,-4.925],[7.9,0,4.925],[-7.9,0,4.925]].map(p=>new THREE.Vector3(...p).project(camera));const r=threeLayer.getBoundingClientRect();const pts=v.map(p=>({x:(p.x*.5+.5)*r.width+r.left,y:(-p.y*.5+.5)*r.height+r.top}));const xs=pts.map(p=>p.x),ys=pts.map(p=>p.y);boardBox={left:Math.min(...xs),top:Math.min(...ys),width:Math.max(...xs)-Math.min(...xs),height:Math.max(...ys)-Math.min(...ys)}}\nfunction point(i,off=0){if(!boardBox)updateBoardBox();const r=boardBox||{left:0,top:72,width:innerWidth,height:innerHeight-72};const p=coords[i]||coords[0];return{x:r.left+p[0]/100*r.width+off,y:r.top+p[1]/100*r.height+off}}\nfunction safeId(n){return "tok_"+String(n).replace(/[^a-zA-Z0-9_-]/g,"_")}function name(){return (username.value||localStorage.getItem("montenoirUser")||"Misafir").trim()}\nfunction createRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}\nfunction joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value.trim().toUpperCase()})}\nfunction startGame(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_start_game",{code:roomCode})}\nfunction rollDice(){if(!roomCode||!myName){alert("Önce odaya gir veya oda oluştur.");return}hideBuy();socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}\nfunction buyProperty(){socket.emit("monopoly_buy_property",{code:roomCode,username:myName});hideBuy()}\nfunction hideBuy(){buyPanel.style.display="none"}function openChat(){chatPanel.style.display="block"}\nfunction getGroup(cell){cell=Number(cell);for(const [g,arr] of Object.entries(colorGroups)){if(arr.includes(cell))return g}return null}\nfunction ownsFullGroup(st,player,cell){const g=getGroup(cell);if(!g)return false;return colorGroups[g].every(c=>(st.owners||{})[String(c)]===player)}\nfunction canBuildHouseOn(cell){if(!currentState||!myName)return false;const c=String(cell);if((currentState.owners||{})[c]!==myName)return false;if(!ownsFullGroup(currentState,myName,Number(cell)))return false;if((currentState.hotels||{})[c])return false;return Number((currentState.houses||{})[c]||0)<4}\nfunction canBuildHotelOn(cell){if(!currentState||!myName)return false;const c=String(cell);if((currentState.owners||{})[c]!==myName)return false;if(!ownsFullGroup(currentState,myName,Number(cell)))return false;if((currentState.hotels||{})[c])return false;return Number((currentState.houses||{})[c]||0)>=4}\nfunction updateBuildButtons(){const p=currentState&&currentState.players?currentState.players[myName]:null;const cell=p?Number(p.position):null;const canH=cell!==null&&canBuildHouseOn(cell);const canO=cell!==null&&canBuildHotelOn(cell);buildHouseBtn.disabled=!canH;buildHotelBtn.disabled=!canO;if(!p)buildHint.textContent="Önce odaya gir.";else if((currentState.owners||{})[String(cell)]!==myName)buildHint.textContent="Kendi mülküne gel.";else if(!ownsFullGroup(currentState,myName,cell))buildHint.textContent="Ev için aynı renk serisini tamamla.";else if(canO)buildHint.textContent="4 ev tamam. Otel alabilirsin.";else if(canH)buildHint.textContent="Seri tamam. Ev alabilirsin.";else buildHint.textContent="Bu yerde inşa hakkı yok."}\nfunction buildHouse(){const c=getBuildCell();if(c==null||!canBuildHouseOn(c)){alert("Ev almak için aynı renk serisinin tamamı sende olmalı ve kendi mülkünde olmalısın.");return}socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:c})}\nfunction buildHotel(){const c=getBuildCell();if(c==null||!canBuildHotelOn(c)){alert("Otel almak için bu mülkte önce 4 ev olmalı.");return}socket.emit("monopoly_add_hotel",{code:roomCode,username:myName,cell:c})}\nfunction getBuildCell(){if(!currentState||!myName)return null;const p=currentState.players[myName];if(!p)return null;const c=String(p.position);if((currentState.owners||{})[c]===myName)return Number(p.position);return null}\nfunction diceDots(n){const p={1:["cc"],2:["tl","br"],3:["tl","cc","br"],4:["tl","tr","bl","br"],5:["tl","tr","cc","bl","br"],6:["tl","tr","ml","mr","bl","br"]};return (p[n]||p[1]).map(c=>\'<span class="diceDot \'+c+\'"></span>\').join("")}\nfunction setTwoDice(total){let a=Math.max(1,Math.min(6,Math.floor(total/2)));let b=total-a;if(b<1){a=1;b=1}if(b>6){b=6;a=total-b}diceA.innerHTML=diceDots(a);diceB.innerHTML=diceDots(b)}\nfunction showDice(n){setTwoDice(n)}\nfunction animateDice(n,done){dicePair.classList.remove("rolling");void dicePair.offsetWidth;dicePair.classList.add("rolling");setTimeout(()=>{dicePair.classList.remove("rolling");setTwoDice(n);if(done)done()},1100)}\nfunction render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;updateBoardBox();renderPlayers(st);renderOwners(st);renderBuilds(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];let el=document.getElementById(safeId(n));if(!el){el=document.createElement("div");el.id=safeId(n);el.className="tok";tokens.appendChild(el)}el.textContent=p.token||"🎩";el.style.color=p.color;const pt=point(p.position,i*7);el.style.left=pt.x+"px";el.style.top=pt.y+"px"});if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st);updateBuildButtons()}\nfunction renderPlayers(st){playersList.innerHTML="";Object.keys(st.players||{}).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];const d=document.createElement("div");d.className="player"+(i===st.turnIndex?" turn":"");d.innerHTML=`<div class="dot" style="background:${p.color}"></div><div>${p.token||"🎩"}</div><div><b>${n}</b><div class="money">${p.money}€</div></div>`;playersList.appendChild(d)})}\nfunction renderOwners(st){owners.innerHTML="";Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const pt=point(Number(cell));const dot=document.createElement("div");dot.className="ownerDot";dot.style.background=p.color;dot.style.color=p.color;dot.title="Propriétaire : "+owner;dot.style.left=pt.x+"px";dot.style.top=(pt.y+34)+"px";owners.appendChild(dot)})}\nfunction renderBuilds(st){builds.innerHTML="";Object.entries(st.houses||{}).forEach(([cell,n])=>{if(!n)return;const owner=(st.owners||{})[cell];const p=owner?st.players[owner]:null;const b=document.createElement("div");b.className="build";b.textContent="🏠".repeat(Number(n));if(p)b.style.color=p.color;const pt=point(Number(cell));b.style.left=pt.x+"px";b.style.top=(pt.y-24)+"px";builds.appendChild(b)});Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(!n)return;const owner=(st.owners||{})[cell];const p=owner?st.players[owner]:null;const b=document.createElement("div");b.className="build";b.textContent="🏨";if(p)b.style.color=p.color;const pt=point(Number(cell));b.style.left=pt.x+"px";b.style.top=(pt.y-24)+"px";builds.appendChild(b)})}\nfunction moveToken(n,from,to,state,done){let el=document.getElementById(safeId(n));if(!el){render(state);done();return}let steps=[],cur=Number(from),dest=Number(to),total=coords.length;while(cur!==dest){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Math.max(0,Object.keys(state.players).indexOf(n));let si=0;function one(){if(si>=steps.length){done();return}const step=steps[si];const a={x:parseFloat(el.style.left)||point(from).x,y:parseFloat(el.style.top)||point(from).y};const b=point(step,idx*7);let t0=null;const duration=285;el.classList.add("tap");function f(ts){if(!t0)t0=ts;const t=Math.min(1,(ts-t0)/duration);const e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;const arc=Math.sin(Math.PI*t)*36;el.style.left=(a.x+(b.x-a.x)*e)+"px";el.style.top=(a.y+(b.y-a.y)*e-arc)+"px";if(t<1)requestAnimationFrame(f);else{el.style.left=b.x+"px";el.style.top=b.y+"px";el.classList.remove("tap");si++;setTimeout(one,55)}}requestAnimationFrame(f)}one()}\nfunction sendChat(){const msg=(chatInput.value||"").trim();if(!msg)return;socket.emit("monopoly_chat",{code:roomCode,username:myName||name(),message:msg});chatInput.value="";chatPanel.style.display="none"}\nfunction showChatLog(u,msg){chatLog.textContent=u+": "+msg;chatLog.classList.add("show");clearTimeout(window.chatTimer);window.chatTimer=setTimeout(()=>chatLog.classList.remove("show"),5000)}\nfunction showMoneyToast(msg,pos=true){moneyToast.textContent=msg;moneyToast.style.color=pos?"#00ff66":"#ff4d4d";moneyToast.classList.add("show");clearTimeout(window.moneyToastTimer);window.moneyToastTimer=setTimeout(()=>moneyToast.classList.remove("show"),5000)}\nfunction detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const d=p.money-previousMoneyState[n];if(d>0)showMoneyToast(n+" "+d+"€ aldı",true);if(d<0)showMoneyToast(n+" "+Math.abs(d)+"€ ödedi",false)}previousMoneyState[n]=p.money})}\nfunction activeText(id){const e=document.querySelector("#"+id+" .active");return e?e.textContent.trim():""}function saveMode(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}const p=Number(activeText("modePlayers")||4);const tr=activeText("modeTurns")||"ILLIMITÉ";const turns=tr.includes("30")?30:(tr.includes("50")?50:"illimite");const target=Number((activeText("modeTarget")||"7000").replace(/[^0-9]/g,""))||7000;socket.emit("monopoly_update_settings",{code:roomCode,players:p,turns:turns,target:target});modePanel.style.display="none"}document.querySelectorAll(".mc span").forEach(s=>s.onclick=()=>{[...s.parentElement.children].forEach(x=>x.classList.remove("active"));s.classList.add("active")});\nsocket.on("monopoly_room_state",render);socket.on("monopoly_error",d=>alert(d.msg));socket.on("monopoly_chat_message",d=>showChatLog(d.username,d.message));socket.on("monopoly_animate_move",d=>{animateDice(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=Number(d.state.players[myName].position);lastCell=landed;buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";setTimeout(()=>buyPanel.style.display="block",160)}})}else render(d.state)})});username.value=localStorage.getItem("montenoirUser")||"";setTimeout(()=>setTwoDice(2),50);\n</script>\n</body>\n</html>'
