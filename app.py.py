...
.imageMask,.diceMask,.buildMarketMask{
...
.rollButtonOnBoard{
  ...
  border:2px solid #ffd989 !important;
  background:linear-gradient(180deg,#ff5c4d,#b51512 58%,#5d0504) !important;
  color:#fff !important;
  font-size:32px !important;
  font-weight:900 !important;
  letter-spacing:1px !important;
  text-shadow:0 3px 5px #000 !important;
  box-shadow:0 0 25px rgba(255,40,30,.55),0 14px 22px rgba(0,0,0,.85),inset 0 0 12px rgba(255,255,255,.18) !important;
...
.propertiesDock{
  ...
  box-shadow:0 0 26px rgba(0,0,0,.75),inset 0 0 12px rgba(212,175,55,.08);
  z-index:110;
  padding:12px 16px;
  display:flex;
  gap:14px;
...
.propCard{
  ...
  border:1px solid rgba(212,175,55,.65);
  border-radius:12px;
  background:linear-gradient(180deg,rgba(255,149,0,.88),rgba(60,30,0,.55));
  overflow:hidden;
...
.buildSlot.canBuild{
  cursor:pointer;
  box-shadow:0 0 12px rgba(0,255,102,.45);
}
.propPrice{
  color:#ffd989;
  text-align:center;
...
.token{
  transition:left .23s ease,top .23s ease,transform .23s ease !important;
}
.token.hop{
  animation:tokenHop .28s ease;
}
@keyframes tokenHop{
  0%{transform:translateY(0) scale(1)}
  45%{transform:translateY(-18px) scale(1.16)}
...
@media(max-width:900px){
  .propertiesDock{position:relative;bottom:auto;left:auto;transform:none;width:100%;margin-top:20px}
  .rollButtonOnBoard{bottom:-8% !important;width:220px !important;font-size:24px !important}
  .rollInfo{display:none}
}
...
def mr(data):
    ...
    if typ in ["property","transport","utility"]:
        ...
        p["money"]-=base
        log+=f" Vergi ödendi: -{base}€."
    elif typ=="bank":
        b=random.choice([50,75,100,150])
        p["money"]+=b
        log+=f" Bankadan ödeme: +{b}€."
    elif typ=="auction":
...
def me(data):
    ...
    if len(names)<=1 and r.get("started"):
        ...
        if names[r["turnIndex"]%len(names)] in r["players"]:
            r["players"][names[r["turnIndex"]%len(names)]]["hasRolled"]=False
        r["turnIndex"]=(r["turnIndex"]+1)%len(names)
        if names[r["turnIndex"]] in r["players"]:
            r["players"][names[r["turnIndex"]]]["hasRolled"]=False
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Montenoir Monopoly Online</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
<style>
*{box-sizing:border-box}
body{
  ...
  min-height:100vh;
  background:
    radial-gradient(circle at 45% 0%,rgba(212,175,55,.26),transparent 35%),
    linear-gradient(135deg,#120b03,#020202 58%,#000);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  overflow:hidden;
}
.loca{
  ...
  border-radius:14px;background:rgba(0,0,0,.78);
  padding:10px 15px;font-weight:900;box-shadow:0 0 18px rgba(212,175,55,.32)
}
.screen{
  height:100vh;
  display:grid;
...
.boardStage{
  position:relative;
  width:min(1320px,calc(100vw - 335px));
...
.boardGlow{
  ...
  inset:0 3% 8% 3%;
  border-radius:38px;
  background:radial-gradient(circle,rgba(212,175,55,.18),transparent 58%);
  filter:blur(18px);
}
.boardWrap{
  position:relative;
  ...
  transform-style:preserve-3d;
  filter:drop-shadow(0 42px 48px rgba(0,0,0,.95));
}
.boardImg{
  width:100%;
  height:100%;
  ...
  box-shadow:
    ...
    inset 0 0 35px rgba(212,175,55,.18);
  user-select:none;
  pointer-events:none;
}
.token{
  ...
  font-size:27px;
  background:radial-gradient(circle at 32% 25%,#fff9 0%,#d4af37 18%,#4b2d05 52%,#020202 100%);
...
.token.moving{transform:scale(1.28) translateY(-10px)}
.house{
  position:absolute;
  z-index:9;
  font-size:25px;
  filter:drop-shadow(0 0 7px rgba(212,175,55,.85)) drop-shadow(0 8px 8px #000);
}
...
.buyPanel{
  ...
  background:rgba(0,0,0,.88);border:1px solid #d4af37;border-radius:18px;
  padding:13px 16px;display:none;z-index:40;text-align:center;
  box-shadow:0 0 28px rgba(212,175,55,.38)
}
.side{
  background:linear-gradient(180deg,rgba(18,18,18,.95),rgba(0,0,0,.88));
  border:1px solid rgba(212,175,55,.72);
  border-radius:22px;
  padding:15px;
  overflow:auto;
  box-shadow:0 0 38px rgba(0,0,0,.9),inset 0 0 18px rgba(212,175,55,.08);
}
h2{color:#d4af37;margin:8px 0 12px;text-shadow:0 0 14px rgba(212,175,55,.34)}
input,select,button{
  width:100%;padding:12px;border-radius:14px;border:1px solid #d4af37;margin:6px 0
}
input,select{background:#050505;color:#fff}
button{
  background:linear-gradient(180deg,#f9df8c,#94600e);
  color:#150b00;font-weight:900;cursor:pointer;
  box-shadow:0 0 16px rgba(212,175,55,.18);
}
button.dark{background:linear-gradient(180deg,#171717,#050505);color:#fff}
.player{
  display:flex;gap:10px;align-items:center;
  border:1px solid rgba(212,175,55,.38);
  border-radius:15px;padding:10px;margin:7px 0;
  background:linear-gradient(180deg,rgba(255,255,255,.055),rgba(0,0,0,.45));
}
.player.turn{box-shadow:0 0 18px rgba(0,255,102,.72);border-color:#00ff66}
.picon{
  width:40px;height:40px;border-radius:50%;
  display:grid;place-items:center;border:1px solid #ffe39b;
  background:radial-gradient(circle,#d4af37,#100800);
  font-size:23px;box-shadow:0 0 12px rgba(212,175,55,.35)
}
.log{
  font-family:Consolas,monospace;color:#00ff66;background:#020202;
  border:1px solid #0f63;border-radius:12px;padding:9px;height:128px;overflow:auto;font-size:12px
}
.houseBtn{width:auto;padding:8px 10px;margin:4px}
#housesPanel{display:flex;flex-wrap:wrap;gap:4px}
...
@media(max-width:900px){
  body{overflow:auto}
  .screen{height:auto;grid-template-columns:1fr}
  .boardStage{width:98vw}
  ...
  <div class="main">
    <div class="boardStage">
      ...
      <div class="boardWrap" id="boardWrap">
        <img class="boardImg" src="/static/monopoly_board_lux_3d.png" alt="Monopoly Board">
        <div class="diceBox">
          <div id="diceCube" class="realDice dice-1">
        ...
        <div id="buyPanel" class="buyPanel">
          <div id="buyText">Bu yeri satın almak ister misin?</div>
          <button onclick="buyProperty()">SATIN AL</button>
          <button class="dark" onclick="hideBuy()">PAS GEÇ</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
const socket=io();
let roomCode="",myName="",currentState=null;
const faces=["1","2","3","4","5","6"];


// Koordinatlar resmin üzerinde kartların merkezlerine göre ayarlandı.
const coords=[
[89,63],  // ALLEZ
[88,78],  // Banque
[78,79],  // New York
...
function getGroup(i){
  for(const [g,arr] of Object.entries(groupMap)){ if(arr.includes(i)) return g; }
  return "";
}
...
function updateRollInfo(st){
  if(!rollInfo || !st || !st.players || !st.players[myName])return;
  const p=st.players[myName];
  const canRoll = !p.hasRolled || st.lastDice===6;
  rollInfo.innerHTML = "<b>Atma hakkınız: "+(canRoll?1:0)+"</b><br><span>6 gelirse tekrar oynarsınız</span>";
}
function renderPropertiesDock(st){
  if(!propertiesDock || !st || !st.players || !st.players[myName])return;
  propertiesDock.innerHTML='<div class="propertiesDockTitle">MÜLKLERİN</div>';
  const owned=Object.entries(st.owners||{}).filter(([cell,owner])=>owner===myName);
  if(!owned.length){
    propertiesDock.innerHTML+='<div style="color:#aaa;padding:45px 10px;font-size:13px">Henüz mülkün yok.</div>';
    return;
  }
  owned.forEach(([cell,owner])=>{
    const i=+cell, title=cells[i]||("Mülk "+i);
    const houses=+(st.houses?.[cell]||0), hotel=+(st.hotels?.[cell]||0);
    const card=document.createElement("div");
    card.className="propCard";
    card.innerHTML='<div class="propCardHead">'+title+' <span>'+((st.players[owner]||{}).token||"")+'</span></div><div class="buildSlots"></div><div class="propPrice">Ev maliyeti: 50€</div>';
    const slots=card.querySelector(".buildSlots");
    if(hotel){
      slots.innerHTML='<div class="buildSlot">🏨</div>';
    }else{
      for(let s=0;s<4;s++){
        const slot=document.createElement("div");
        slot.className="buildSlot "+(s===houses?"canBuild":"");
        slot.textContent=s<houses?"🏠":"";
        if(s===houses) slot.onclick=()=>socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:i});
        slots.appendChild(slot);
      }
      const h=document.createElement("div");
      h.className="buildSlot "+(houses>=4?"canBuild":"");
      h.textContent=houses>=4?"🏨":"";
      if(houses>=4) h.onclick=()=>socket.emit("monopoly_add_hotel",{code:roomCode,username:myName,cell:i});
      slots.appendChild(h);
    }
    propertiesDock.appendChild(card);
  });
}

function logMsg(t){log.innerHTML=t+"<br>"+log.innerHTML}


function addOwnerMarks(st){
 Object.entries(st.owners||{}).forEach(([cell,owner])=>{
   const p=st.players[owner];
   if(!p)return;
   const c=coords[+cell]||[50,50];
   const m=document.createElement("div");
   m.className="ownerMark";
   m.style.background=p.color||"#d4af37";
   m.style.color=p.color||"#d4af37";
   m.style.left=`calc(${c[0]}% - 21px)`;
   m.style.top=`calc(${c[1]}% + 19px)`;
   boardWrap.appendChild(m);
   const ic=document.createElement("div");
   ic.className="ownerIcon";
   ic.textContent=p.token;
   ic.style.left=`calc(${c[0]}% + 15px)`;
   ic.style.top=`calc(${c[1]}% + 13px)`;
   boardWrap.appendChild(ic);
 });
}
...
function render(st){
 currentState=st;
 roomCode=st.code;
 roomCodeInput.value=st.code;
 roomCode.textContent=st.code;
 document.querySelectorAll(".token,.house,.ownerMark,.ownerIcon").forEach(e=>e.remove());
 addOwnerMarks(st);
 const names=Object.keys(st.players);
 players.innerHTML="";
 names.forEach((n,i)=>{
   const p=st.players[n],turn=i===st.turnIndex;
   const d=document.createElement("div");
   d.className="player"+(turn?" turn":"");
   d.innerHTML=`<div class="picon" style="box-shadow:0 0 14px ${p.color||"#d4af37"};border-color:${p.color||"#d4af37"}">${p.token}</div><div><b><span style="color:${p.color||"#d4af37"}">●</span> ${n}</b><br>${p.money}€ ${p.bankrupt?"— İFLAS":""}</div>`;
 ...
 Object.entries(st.houses||{}).forEach(([cell,n])=>{
   ...
   for(let i=0;i<n;i++){
     const h=document.createElement("div");
     h.className="house ownerHouse";h.textContent="🏠";let owner=(st.owners||{})[cell];let op=st.players[owner];if(op){h.style.background=op.color;h.style.boxShadow="0 0 12px "+op.color;}
     h.style.left=`calc(${c[0]}% - 18px + ${i*13}px)`;
     h.style.top=`calc(${c[1]}% - 46px)`;
     boardWrap.appendChild(h);
   }
 });
 Object.entries(st.hotels||{}).forEach(([cell,n])=>{
   if(n){
     ...
     const h=document.createElement("div");
     h.className="house ownerHouse";h.textContent="🏨";let owner=(st.owners||{})[cell];let op=st.players[owner];if(op){h.style.background=op.color;h.style.boxShadow="0 0 12px "+op.color;}
     h.style.left=`calc(${c[0]}% - 20px)`;
     h.style.top=`calc(${c[1]}% - 50px)`;
     boardWrap.appendChild(h);
   }
 });
 logMsg(st.lastLog||"");
 ...
 updateRollInfo(st);
 renderPropertiesDock(st);
 renderHouses(st);
}
function addToken(p,i){
 const c=coords[p.position]||[87,80];
 const el=document.createElement("div");
 el.className="token";el.textContent=p.token;
 el.style.left=`calc(${c[0]}% - 23px + ${i*7}px)`;
 el.style.top=`calc(${c[1]}% - 23px + ${i*7}px)`;
 boardWrap.appendChild(el);
}
function renderHouses(st){
 housesPanel.innerHTML="";
 const groups={};
 Object.entries(st.owners||{}).forEach(([cell,owner])=>{
   if(owner===myName){
     const groupName=getGroup(+cell);
     if(groupName && groupName!=="transport" && groupName!=="utility" && groupName!=="corner" && groupName!=="special" && groupName!=="tax"){
       groups[groupName]=groups[groupName]||[];
       groups[groupName].push(+cell);
     }
   }
 });
 Object.entries(groups).forEach(([g,ownedCells])=>{
   const needed=groupMap[g]||[];
   const full=needed.length>0 && needed.every(x=>ownedCells.includes(x));
   if(full){
     const title=document.createElement("div");
     title.style.width="100%";
     title.style.color="#d4af37";
     title.style.fontWeight="900";
     title.textContent="Set tamamlandı: "+g;
     housesPanel.appendChild(title);
     needed.forEach(cell=>{
       const b=document.createElement("button");
       b.className="houseBtn";
       b.textContent="🏠 "+cells[cell];
       b.onclick=()=>socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:cell});
       housesPanel.appendChild(b);
       const h=document.createElement("button");
       h.className="houseBtn";
       h.textContent="🏨 "+cells[cell];
       h.onclick=()=>socket.emit("monopoly_add_hotel",{code:roomCode,username:myName,cell:cell});
       housesPanel.appendChild(h);
     });
   }
 });
 if(!housesPanel.innerHTML){
   housesPanel.innerHTML='<span style="font-size:12px;color:#bbb">Ev koymak için aynı renkteki tüm şehirleri satın almalısın.</span>';
 }
}
function animateOneToken(username, from, to, state, done){
 ...
 const tokenEl=[...document.querySelectorAll(".token")][idx];
 if(!tokenEl){done();return;}
 ...
 const timer=setInterval(()=>{
   ...
   const c=coords[steps[k]]||[50,50];
   tokenEl.style.left=`calc(${c[0]}% - 23px + ${idx*7}px)`;
   tokenEl.style.top=`calc(${c[1]}% - 23px + ${idx*7}px)`;
   tokenEl.classList.remove("hop"); void tokenEl.offsetWidth; tokenEl.classList.add("hop");
   k++;
 },260);
}
...
socket.on("monopoly_animate_move",d=>{
 ...
 setTimeout(()=>{
   ...
   showDice(d.dice);
   if(currentState){
     animateOneToken(d.username,d.from,d.to,d.state,()=>{
       render(d.state);
       if(d.pendingBuy&&d.state.players[myName]?.position===d.to){buyText.textContent=cells[d.to]+" satın alınabilir. Satın almak ister misin?";buyPanel.style.display="block";}
     });
   }else{
     render(d.state);
     if(d.pendingBuy&&d.state.players[myName]?.position===d.to){buyText.textContent=cells[d.to]+" satın alınabilir. Satın almak ister misin?";buyPanel.style.display="block";}
...
def mc(data):
    ...
    r["players"][u]={"name":u,"token":t,"money":1000,"position":0,"jailed":0,"bankrupt":False}
