...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Montenoir Monopoly</title>
<style>
*{box-sizing:border-box}
body{
  margin:0;
  ...
  background:
    ...
    linear-gradient(135deg,#090909,#020202 55%,#0b0702);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  overflow-x:hidden;
}
...
.userCard,.toolBtn,.playerCard,.bottomBox{
  border:1px solid rgba(212,175,55,.74);
  border-radius:12px;
  background:linear-gradient(180deg,rgba(22,22,22,.92),rgba(5,5,5,.9));
  box-shadow:0 0 16px rgba(212,175,55,.15), inset 0 0 14px rgba(212,175,55,.05);
}
.userCard{padding:12px;display:flex;gap:10px;align-items:center}
.avatar{width:56px;height:56px;border-radius:50%;border:2px solid #d4af37;background:url('/static/montenoir_home.png') center/cover,#111}
.username{font-size:18px;font-weight:900}
.moneyTag{color:#d4af37;font-size:16px;font-weight:900;margin-top:4px}
.toolBtn{
  min-height:86px;
  display:flex;align-items:center;justify-content:center;flex-direction:column;
  color:#f2d37a;font-weight:900;text-decoration:none;font-size:15px;
  cursor:pointer;
}
.toolBtn .ico{font-size:31px;margin-bottom:6px}
.toolBtn:hover{background:linear-gradient(180deg,rgba(212,175,55,.24),rgba(5,5,5,.9))}
.main{display:flex;flex-direction:column;align-items:center}
.loca{
  align-self:flex-start;
  color:#d4af37;text-decoration:none;border:1px solid #d4af37;
  background:rgba(0,0,0,.72);border-radius:12px;padding:9px 14px;
...
.board{
  ...
  border:5px solid #c7962d;
  border-radius:24px;
  background:linear-gradient(145deg,#111,#030303);
  box-shadow:0 35px 65px rgba(0,0,0,.78),0 0 35px rgba(212,175,55,.32), inset 0 0 30px rgba(212,175,55,.09);
...
.center{
  position:absolute;
  left:18.5%;right:18.5%;top:19%;bottom:19%;
  border:2px solid rgba(212,175,55,.35);
  border-radius:8px;
  background:
    radial-gradient(circle at center,rgba(212,175,55,.10),rgba(0,0,0,.62) 62%),
...
.monopolyTitle{
  ...
  font-weight:900;
  color:#d4af37;
  text-shadow:0 8px 0 #241300,0 0 24px rgba(212,175,55,.5);
  letter-spacing:2px;
  z-index:2;
}
.diceArea{
...
.dice3d{
  ...
  background:linear-gradient(145deg,#fff,#d8d8d8 48%,#8d8d8d);
  color:#2b1700;
  display:grid;place-items:center;
  font-size:38px;
  font-weight:900;
  border:2px solid #fff;
  box-shadow:0 18px 25px rgba(0,0,0,.65),0 0 16px rgba(255,255,255,.28);
}
.diceResult{
...
.rolling{animation:diceSpin .65s linear infinite}
@keyframes diceSpin{0%{transform:rotateX(0) rotateY(0)}100%{transform:rotateX(360deg) rotateY(540deg)}}
.cell{
  position:absolute;
...
.cell .bar{height:13px;width:100%;border-radius:2px;margin-bottom:4px}
.cell .price{font-size:10px;color:#f1d275;margin-top:3px}
.houseRow{position:absolute;bottom:2px;display:flex;gap:1px;font-size:13px}
.token{position:absolute;right:3px;bottom:0;font-size:24px;filter:drop-shadow(0 0 7px #00ff66)}
.brown{background:#8b4513}.sky{background:#118bd2}.pink{background:#a020f0}.orange{background:#ff9c2e}.red{background:#e33131}.yellow{background:#f7d348}.green{background:#33c46b}.blue{background:#0648d8}.gold{background:#d4af37}.utility{background:#1cb8e8}.transport{background:#dadada}.bank{background:#d4af37}
.corner{font-size:16px;justify-content:center}
.bottomControls{
  margin-top:20px;
  display:grid;
...
.gameBtn{
  ...
  border-radius:14px;
  padding:14px 18px;
  background:linear-gradient(180deg,#f3d27b,#87560a);
  color:#111;
  font-size:28px;
  font-weight:900;
...
.gameBtn.dark{background:linear-gradient(180deg,#1c1c1c,#050505);color:#ddd;font-size:18px}
.playerCard{display:flex;align-items:center;gap:10px;padding:10px}
.playerIcon{width:48px;height:48px;border-radius:50%;border:1px solid #d4af37;display:grid;place-items:center;font-size:25px;background:#111}
.playerName{font-weight:900}.playerMoney{color:#d4af37;font-weight:900}
.bottomLegend{
  grid-column:1/4;
  margin-top:16px;
  display:grid;
...
.legendBox{padding:12px;border:1px solid rgba(212,175,55,.55);border-radius:10px;background:rgba(0,0,0,.65)}
.legendItems{display:flex;gap:18px;flex-wrap:wrap}
.legendItem{text-align:center;color:#d4af37;font-weight:900;font-size:12px}
select{
  width:100%;padding:10px;border-radius:10px;border:1px solid #d4af37;
  background:#070707;color:#fff;margin-top:8px;
}
.log{font-family:Consolas,monospace;color:#00ff66;font-size:12px;line-height:1.4;max-height:100px;overflow:auto;margin-top:8px}
@media(max-width:1000px){
 .page{grid-template-columns:1fr}
 .boardWrap{width:96vw}
 .sideRight{display:none}
 .bottomControls{grid-template-columns:1fr}
 .bottomLegend{grid-column:auto;grid-template-columns:1fr}
 .monopolyTitle{font-size:40px}
}
</style>
</head>
<body>
<div class="page">
  <div class="sideLeft">
    <div class="userCard">
      ...
      <div>
        <div class="username" id="playerName">Yohanna</div>
        <div class="moneyTag">💵 <span id="money">1500</span></div>
      </div>
    </div>
    <div class="toolBtn"><span class="ico" id="selectedToken">🎩</span><span>PİYONUN</span>
      <select id="tokenSelect" onchange="changeToken()">
        <option value="✈️">✈️ Jet Privé</option>
        <option value="🎰">🎰 Machine à Sous</option>
        <option value="🛥️">🛥️ Yacht</option>
        <option value="🚗">🚗 Supercar</option>
        <option value="🎩" selected>🎩 Haut-de-forme</option>
        <option value="🐈">🐈 Chat Noir</option>
      </select>
    </div>
    <button class="toolBtn" onclick="payBank()"><span class="ico">🏦</span><span>BANKAYA<br>ÖDEME YAP</span></button>
    <button class="toolBtn"><span class="ico">💬</span><span>SOHBET</span></button>
    <a class="toolBtn" href="/"><span class="ico">🚪</span><span>LOCA</span></a>
  </div>
  ...
  <div class="main">
    ...
    <div class="boardWrap">
      <div class="board" id="board">
        <div class="center">
          <div class="skyline">🏙️ 🏢 🏙️ 🏢 🏙️</div>
          <div class="monopolyTitle">MONOPOLY</div>
          <div class="diceArea">
            <div class="dice3d" id="diceA">⚀</div>
            <div class="diceResult" id="diceResult">0</div>
            <div class="dice3d" id="diceB">⚀</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bottomControls">
      <button class="gameBtn dark" onclick="undoMove()">◀ GERİ AL</button>
      <button class="gameBtn" onclick="rollDice()">ZAR AT</button>
      <button class="gameBtn dark" onclick="autoPlay()">OTOMATİK OYNA ▶</button>
      <div class="bottomLegend">
        <div class="legendBox">
          <b style="color:#d4af37">ÖZEL YAPILAR</b>
          <div class="legendItems">
            <div class="legendItem">✈️<br>AÉROPORT<br>200.000</div>
            <div class="legendItem">🚢<br>PORT<br>200.000</div>
            <div class="legendItem">🚄<br>GARE<br>200.000</div>
            <div class="legendItem">⚡<br>ÉLECTRIQUE<br>150.000</div>
            <div class="legendItem">💧<br>EAUX<br>150.000</div>
          </div>
        </div>
        <div class="legendBox">
          <b style="color:#d4af37">YAPILAR</b>
          <div class="legendItems">
            <div class="legendItem">🏠<br>1 EV</div>
            <div class="legendItem">🏠🏠<br>2 EV</div>
            <div class="legendItem">🏠🏠🏠<br>3 EV</div>
            <div class="legendItem">🏨<br>OTEL</div>
          </div>
        </div>
      </div>
  ...
  <div class="sideRight">
    <div class="playerCard"><div class="playerIcon">✈️</div><div><div class="playerName">Yohanna</div><div class="playerMoney">12.450.000</div></div></div>
    <div class="playerCard"><div class="playerIcon">🎰</div><div><div class="playerName">Svetlana</div><div class="playerMoney">8.750.000</div></div></div>
    <div class="playerCard"><div class="playerIcon">🛥️</div><div><div class="playerName">Pedro</div><div class="playerMoney">6.300.000</div></div></div>
    <div class="playerCard"><div class="playerIcon">🚗</div><div><div class="playerName">Florian</div><div class="playerMoney">5.500.000</div></div></div>
    <div class="playerCard"><div class="playerIcon">🎩</div><div><div class="playerName">Martina</div><div class="playerMoney">4.200.000</div></div></div>
    <div class="playerCard"><div class="playerIcon">🐈</div><div><div class="playerName">Nicolas</div><div class="playerMoney">3.900.000</div></div></div>
  </div>
</div>

<script>
const cells=[
 ["ALLEZ",0,"gold","start"],["Sofia",120,"brown","city"],["Bucarest",120,"brown","city"],["Belgrade",140,"brown","city"],["İstanbul",140,"brown","city"],["🚄 Gare Grande Vitesse",200,"transport","transport"],["Prague",160,"sky","city"],["Budapest",160,"sky","city"],["Lisbonne",180,"sky","city"],["Athènes",180,"sky","city"],["🚢 Port International",200,"transport","transport"],
 ["Amsterdam",240,"orange","city"],["Milan",240,"orange","city"],["Paris",260,"red","city"],["Londres",260,"red","city"],["💧 Compagnie des Eaux",150,"utility","utility"],["Dubaï",340,"blue","city"],["Pékin",340,"blue","city"],
 ["Tokyo",320,"green","city"],["Singapour",320,"green","city"],["🚄 Gare Grande Vitesse",200,"transport","transport"],["New York",300,"blue","city"],["Monaco",300,"blue","city"],["Hong Kong",280,"blue","city"],["Toronto",280,"blue","city"],
 ["JUSTE VISITE",0,"gold","visit"],["⚡ Compagnie Électrique",150,"utility","utility"],["Moscou",220,"pink","city"],["Berlin",220,"pink","city"],["Rome",200,"pink","city"],["Madrid",200,"pink","city"],["✈️ Aéroport International",200,"transport","transport"]
];
let pos=0,money=1500,owned=[],lastPos=0,playerToken="🎩";
const username=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Yohanna";
playerName.textContent=username;

function changeToken(){
  playerToken=tokenSelect.value;
  selectedToken.textContent=playerToken;
  renderBoard();
}
...
function placeCell(i,el){
 ...
 const cw=Math.max(65,w/11.5), ch=Math.max(58,h/8.6);
 const leftW=cw, topH=ch;
 let x=0,y=0;
 if(i<=10){ // top row, right to left
   x=w-cw-(i*cw); y=0; el.style.width=cw+"px"; el.style.height=ch+"px";
 }else if(i<=17){ // right side
   x=w-cw; y=ch+((i-11)*((h-2*ch)/7)); el.style.width=cw+"px"; el.style.height=((h-2*ch)/7)+"px";
 }else if(i<=25){ // bottom row
   x=(i-18)*((w-2*cw)/8)+cw; y=h-ch; el.style.width=((w-2*cw)/8)+"px"; el.style.height=ch+"px";
 }else{ // left side
   x=0; y=h-ch-((i-25)*((h-2*ch)/6)); el.style.width=cw+"px"; el.style.height=((h-2*ch)/6)+"px";
 }
...
function renderBoard(){
 ...
 cells.forEach((c,i)=>{
  ...
  div.className="cell"+(i==0||i==25?" corner":"");
  const houses = owned.includes(i)&&c[3]==="city" ? '<div class="houseRow">🏠🏠</div>' : "";
  div.innerHTML='<div class="bar '+c[2]+'"></div><div class="name">'+c[0]+'</div>'+(c[1]>0?'<div class="price">'+c[1]+'.000</div>':'')+houses+(i===pos?'<div class="token">'+playerToken+'</div>':'');
...
function rollDice(){
 const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];
 diceA.classList.add("rolling");diceB.classList.add("rolling");
 let n=0;
 const timer=setInterval(()=>{
  diceA.textContent=faces[Math.floor(Math.random()*6)];
  diceB.textContent=faces[Math.floor(Math.random()*6)];
  n++;
  if(n>12){
    clearInterval(timer);
    const a=Math.floor(Math.random()*6)+1,b=Math.floor(Math.random()*6)+1,total=a+b;
    diceA.classList.remove("rolling");diceB.classList.remove("rolling");
    diceA.textContent=faces[a-1];diceB.textContent=faces[b-1];
...
function move(n){
 ...
 if(pos<old){money+=200;addLog("ALLEZ geçildi: +200.000")}
 addLog(username+" "+n+" ilerledi: "+cells[pos][0]);
 renderBoard();
}
function undoMove(){pos=lastPos;addLog("Geri alındı.");renderBoard();}
function autoPlay(){rollDice();}
function payBank(){if(money>=50){money-=50;addLog("🏦 Bankaya ödeme: -50.000");renderBoard();}else addLog("Para yetersiz.")}
renderBoard();
