...
<style>
*{box-sizing:border-box}
body{margin:0;overflow:hidden;background:radial-gradient(circle at top,#281706,#050505 55%,#000);color:#fff;font-family:Arial,Helvetica,sans-serif}
.loca{position:fixed;top:12px;left:12px;z-index:50;padding:10px 16px;border:1px solid #d4af37;border-radius:12px;background:#000d;color:#ffd989;text-decoration:none;font-weight:900}
.topLobby{position:fixed;top:12px;left:130px;right:330px;z-index:50;height:56px;display:flex;gap:8px;align-items:center;padding:8px 10px;border:1px solid #d4af37aa;border-radius:16px;background:#000d}
...
#game3d{position:fixed;top:74px;left:0;right:320px;bottom:70px;z-index:1}
.playersPanel{position:fixed;top:12px;right:12px;width:300px;bottom:86px;z-index:45;border:1px solid #d4af37aa;border-radius:18px;background:linear-gradient(180deg,#121212ee,#000e);box-shadow:0 0 25px #000;padding:14px;overflow:auto}
.playersPanel h2{margin:0 0 12px;color:#ffd989}
.playerCard{display:flex;align-items:center;gap:10px;border:1px solid #d4af3755;border-radius:13px;padding:10px;margin:8px 0;background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(0,0,0,.52))}
.playerCard.turn{border-color:#00ff66;box-shadow:0 0 16px #00ff6670}
.colorDot{width:18px;height:18px;border-radius:50%;border:2px solid rgba(255,255,255,.75);box-shadow:0 0 10px currentColor}
.pawnIcon{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;background:#0009;border:1px solid #d4af37;font-size:23px}
.playerName{font-weight:900}.playerMoney{color:#ffd989}
.rollBtn{position:fixed;left:calc((100vw - 320px)/2);bottom:10px;transform:translateX(-50%);z-index:60;width:300px;height:58px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(#ff5c4d,#b51512 60%,#5d0504);color:#fff;font-size:30px;font-weight:900;text-shadow:0 3px 5px #000;box-shadow:0 0 24px #ff2a2a80,0 12px 18px #000;cursor:pointer}
.buyPanel{position:fixed;left:calc((100vw - 320px)/2);top:45%;transform:translate(-50%,-50%);z-index:70;width:410px;max-width:90%;padding:22px;border-radius:22px;background:#000c;border:1px solid #d4af37;text-align:center;display:none}
...
.toast{position:fixed;left:16px;bottom:16px;z-index:80;width:250px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.toast.show{opacity:1}
.buildPanel{position:fixed;left:calc((100vw - 320px)/2);bottom:78px;transform:translateX(-50%);z-index:65;width:min(820px,calc(100vw - 360px));min-height:92px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(180deg,#111d,#000d);box-shadow:0 0 22px #000;display:none;padding:12px 14px;gap:10px;overflow-x:auto}
.buildTitle{position:absolute;top:-24px;left:50%;transform:translateX(-50%);color:#ffd989;font-weight:900;white-space:nowrap}
.buildCard{min-width:160px;border:1px solid #d4af3766;border-radius:12px;background:linear-gradient(180deg,#2a1705,#070707);padding:8px;text-align:center}
.buildName{font-weight:900;color:#ffd989;margin-bottom:8px}
.buildSlots{display:flex;justify-content:center;gap:5px}
.buildSlot{width:24px;height:23px;border:1px solid #ffffff55;border-radius:5px;background:#0008;display:grid;place-items:center;font-size:15px}
.buildCard button{margin-top:8px;width:100%;padding:7px;border-radius:9px;border:1px solid #d4af37;background:linear-gradient(#ffe08a,#a96d0d);font-weight:900;cursor:pointer}
@media(max-width:950px){body{overflow:auto}.topLobby{left:8px;right:8px;top:56px;height:auto;flex-wrap:wrap}.playersPanel{position:relative;top:auto;right:auto;width:auto;margin:8px}.loca{top:8px}#game3d{position:relative;top:auto;right:auto;height:65vw;min-height:420px;margin-top:150px}.rollBtn{left:50%;width:230px;font-size:24px}.buildPanel{left:50%;width:92vw;bottom:84px}}
</style>
</head>
<body>
...
<h2>ONLINE OYUNCULAR / RENKLER</h2>
<div id="playersList"></div>
</div>

<button class="rollBtn" onclick="rollDice()">🎲 ZAR AT</button>

<div id="buildPanel" class="buildPanel"><div class="buildTitle">EV / OTEL İNŞA ET</div></div>
...
[86.8,19.8],  // Dubaï
[86.8,31.2],  // Pékin
[86.8,43.0],  // Électricité
[86.8,54.7]   // Tokyo
];
const groupMap={green:[2,3,5],brown:[6,7],orange:[9,10,11],purple:[13,14,15],yellow:[17,18,20],blue:[21,22],red:[24,25,27]};
let scene,camera,renderer,board,dice,players3d={},owners3d={},houses3d={};
let gltfLoader,diceModelPrefab,houseModelPrefab,hotelModelPrefab;
let world,diceBody,groundBody,physicsRunning=false,physicsTarget=1;
const boardW=14,boardH=8.75;
init3D();


function loadModelPrefabs(){
 gltfLoader=new THREE.GLTFLoader();
 gltfLoader.load("/static/models/dice.glb",g=>{diceModelPrefab=g.scene; replaceDiceWithModel();});
 gltfLoader.load("/static/models/house.glb",g=>{houseModelPrefab=g.scene;});
 gltfLoader.load("/static/models/hotel.glb",g=>{hotelModelPrefab=g.scene;});
}
function cloneModel(prefab,color){
...
function replaceDiceWithModel(){
 if(!diceModelPrefab || !dice)return;
 const pos=dice.position.clone();
 const quat=dice.quaternion.clone();
 scene.remove(dice);
 dice=cloneModel(diceModelPrefab,null);
 dice.scale.set(.9,.9,.9);
...
function init3D(){
 ...
 loadModelPrefabs();
 camera=new THREE.PerspectiveCamera(29,c.clientWidth/c.clientHeight,.1,100);
 camera.position.set(0,14.2,5.2);
 camera.lookAt(0,0,0);
 ...
 const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png");
 ...
 dice=createDice(); dice.position.set(0,0.92,0.15); scene.add(dice); initPhysics();
...
function animate(){
 ...
 if(world && physicsRunning){
   world.step(1/60);
   syncDiceFromPhysics();
 }
 renderer.render(scene,camera);
}
function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.43,(p[1]/100-.5)*boardH+off)}
function tokenMesh(icon,color){
 const g=new THREE.Group();
 const s=new THREE.Mesh(new THREE.SphereGeometry(.18,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.72,roughness:.25}));
 s.castShadow=true;g.add(s);
 const canvas=document.createElement("canvas");canvas.width=128;canvas.height=128;
 const ctx=canvas.getContext("2d");ctx.font="72px Arial";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(icon||"🎩",64,68);
 const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(canvas),transparent:true}));
 sp.position.y=.36;sp.scale.set(.44,.44,.44);g.add(sp);
 return g;
}
...
function initPhysics(){
 ...
 groundBody.addShape(new CANNON.Box(new CANNON.Vec3(boardW/2,0.08,boardH/2)));
 groundBody.position.set(0,0.40,0);
 ...
 diceBody.addShape(new CANNON.Box(new CANNON.Vec3(0.46,0.46,0.46)));
 diceBody.position.set(0,0.72,0.28);
 world.addBody(diceBody);
}
function syncDiceFromPhysics(){
 dice.position.set(diceBody.position.x,diceBody.position.y,diceBody.position.z);
 dice.quaternion.set(diceBody.quaternion.x,diceBody.quaternion.y,diceBody.quaternion.z,diceBody.quaternion.w);
}
function forceDiceFace(n){
 showDice(n);
...
function createDice(){
 if(diceModelPrefab){
   const model=cloneModel(diceModelPrefab,null);
   model.scale.set(.9,.9,.9);
   return model;
 }
 const g=new THREE.Group();
 const mat=new THREE.MeshStandardMaterial({color:0xffffff,roughness:.16,metalness:.015});
 const box=new THREE.Mesh(new THREE.BoxGeometry(1.05,1.05,1.05,6,6,6),mat);
 box.castShadow=true; box.receiveShadow=true; g.add(box);
 const dotMat=new THREE.MeshStandardMaterial({color:0x020202,roughness:.38});
 const faces=[
  [[0,0,.535],[0,0,0],[[0,0]]],
  [[0,0,-.535],[0,Math.PI,0],[[-.22,.22],[.22,-.22]]],
  [[.535,0,0],[0,Math.PI/2,0],[[-.25,.25],[0,0],[.25,-.25]]],
  [[-.535,0,0],[0,-Math.PI/2,0],[[-.25,.25],[.25,.25],[-.25,-.25],[.25,-.25]]],
  [[0,.535,0],[-Math.PI/2,0,0],[[-.27,.27],[.27,.27],[0,0],[-.27,-.27],[.27,-.27]]],
  [[0,-.535,0],[Math.PI/2,0,0],[[-.27,.29],[.27,.29],[-.27,0],[.27,0],[-.27,-.29],[.27,-.29]]]
 ];
 faces.forEach(f=>f[2].forEach(([a,b])=>{
   const dot=new THREE.Mesh(new THREE.SphereGeometry(.075,24,12),dotMat);
   dot.position.set(a,b,.555);
   const h=new THREE.Group(); h.add(dot); h.position.set(...f[0]); h.rotation.set(...f[1]); g.add(h);
 }));
 return g;
}
...
function rollDiceAnim(n,done){
 // Real physics if Cannon is loaded; stable visual fallback if not.
 if(window.CANNON && world && diceBody){
   physicsRunning=true;
   diceBody.position.set(-1.25,2.75,-0.45);
   diceBody.velocity.set(2.2,-2.2,1.05);
   diceBody.angularVelocity.set(10+Math.random()*5,14+Math.random()*5,8+Math.random()*4);
   diceBody.quaternion.setFromEuler(Math.random()*Math.PI,Math.random()*Math.PI,Math.random()*Math.PI);

   let ticks=0;
   const watcher=setInterval(()=>{
     ticks++;
     diceBody.position.x=Math.max(-1.9,Math.min(1.9,diceBody.position.x));
     diceBody.position.z=Math.max(-1.1,Math.min(1.1,diceBody.position.z));
     const v=diceBody.velocity.length();
     const av=diceBody.angularVelocity.length();

     if((ticks>68 && v<0.25 && av<0.38) || ticks>145){
       clearInterval(watcher);
       physicsRunning=false;
       dice.position.set(0,0.92,0.15);
       showDice(n);
       diceBody.position.set(0,0.92,0.15);
       diceBody.velocity.set(0,0,0);
       diceBody.angularVelocity.set(0,0,0);
       diceBody.quaternion.set(dice.quaternion.x,dice.quaternion.y,dice.quaternion.z,dice.quaternion.w);
       if(done)done();
 ...
 // Fallback: fake gravity + bounce, always visible.
 let frame=0;
 const duration=48;
 const startPos=new THREE.Vector3(-1.3,2.7,-0.45);
 const endPos=new THREE.Vector3(0,0.92,0.15);
 dice.position.copy(startPos);
 const interval=setInterval(()=>{
   frame++;
   const t=frame/duration;
   dice.position.x=startPos.x+(endPos.x-startPos.x)*(1-Math.pow(1-t,2.15));
   dice.position.z=startPos.z+(endPos.z-startPos.z)*(1-Math.pow(1-t,1.85));
   const bounce=Math.abs(Math.sin(t*Math.PI*4.5))*(1-t)*.85;
   dice.position.y=endPos.y+bounce;
   const speed=1-t;
   dice.rotation.x+=.72*speed+.04;
   dice.rotation.y+=.88*speed+.03;
   dice.rotation.z+=.52*speed+.02;
   if(frame>=duration){
...
function makeHouse(color){
 const c = color && color.isColor ? "#"+color.getHexString() : color;
 const model=cloneModel(houseModelPrefab,c);
 if(model){
   model.scale.set(.16,.16,.16);
   return model;
 }
 const g=new THREE.Group();
 const base=new THREE.Mesh(new THREE.BoxGeometry(.16,.13,.16),new THREE.MeshStandardMaterial({color:color||0x34c759,metalness:.25,roughness:.35}));
 base.position.y=.065;
 const roof=new THREE.Mesh(new THREE.ConeGeometry(.14,.12,4),new THREE.MeshStandardMaterial({color:0x111111,metalness:.25,roughness:.32}));
 roof.rotation.y=Math.PI/4; roof.position.y=.19; g.add(base);g.add(roof);return g;
}
function makeHotel(color){
 const c = color && color.isColor ? "#"+color.getHexString() : color;
 const model=cloneModel(hotelModelPrefab,c);
 if(model){
   model.scale.set(.17,.17,.17);
   return model;
 }
 const g=new THREE.Group();
 const base=new THREE.Mesh(new THREE.BoxGeometry(.24,.22,.20),new THREE.MeshStandardMaterial({color:color||0xff3b30,metalness:.28,roughness:.35}));
 base.position.y=.11;
 const roof=new THREE.Mesh(new THREE.BoxGeometry(.28,.06,.24),new THREE.MeshStandardMaterial({color:0xffd989,metalness:.45,roughness:.25}));
 roof.position.y=.25; g.add(base);g.add(roof);return g;
}
function getGroup(i){for(const [g,arr] of Object.entries(groupMap)){if(arr.includes(i))return g}return ""}
function ownsFullGroup(st,owner,group){const arr=groupMap[group]||[];return arr.length>0&&arr.every(i=>(st.owners||{})[i]===owner||(st.owners||{})[String(i)]===owner)}
function clearHouses3D(){Object.values(houses3d).forEach(arr=>arr.forEach(o=>scene.remove(o)));houses3d={}}
function renderHouses3D(st){clearHouses3D();Object.entries(st.houses||{}).forEach(([cell,n])=>{const owner=(st.owners||{})[cell];const p=(st.players||{})[owner];const color=new THREE.Color(p?.color||"#34c759");houses3d[cell]=[];const base=pos3D(+cell);for(let i=0;i<Number(n);i++){const h=makeHouse(color);h.position.set(base.x-.24+i*.16,.52,base.z-.34);scene.add(h);houses3d[cell].push(h)}});Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(!n)return;const owner=(st.owners||{})[cell];const p=(st.players||{})[owner];const color=new THREE.Color(p?.color||"#ff3b30");houses3d[cell]=houses3d[cell]||[];const base=pos3D(+cell);const h=makeHotel(color);h.position.set(base.x,.52,base.z-.34);scene.add(h);houses3d[cell].push(h)})}
function renderBuildPanel(st){buildPanel.innerHTML='<div class="buildTitle">EV / OTEL İNŞA ET</div>';if(!st.players||!st.players[myName]){buildPanel.style.display="none";return}const owned=Object.entries(st.owners||{}).filter(([cell,owner])=>owner===myName);let show=false;owned.forEach(([cell,owner])=>{const i=Number(cell);const g=getGroup(i);if(!ownsFullGroup(st,myName,g))return;show=true;const houses=Number((st.houses||{})[cell]||0);const hotel=Number((st.hotels||{})[cell]||0);const card=document.createElement("div");card.className="buildCard";card.innerHTML='<div class="buildName">'+(cells[i]||"Mülk")+'</div><div class="buildSlots"></div>';const slots=card.querySelector(".buildSlots");if(hotel){slots.innerHTML='<div class="buildSlot">🏨</div>'}else{for(let s=0;s<4;s++){const sl=document.createElement("div");sl.className="buildSlot";sl.textContent=s<houses?"🏠":"";slots.appendChild(sl)}const btn=document.createElement("button");btn.textContent=houses>=4?"🏨 OTEL KUR":"🏠 EV KUR";btn.onclick=()=>socket.emit(houses>=4?"monopoly_add_hotel":"monopoly_add_house",{code:roomCode,username:myName,cell:i});card.appendChild(btn)}buildPanel.appendChild(card)});buildPanel.style.display=show?"flex":"none"}
...
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayersPanel(st);renderOwners(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const fallbackColors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color){p.color=fallbackColors[i%fallbackColors.length]}if(!players3d[n]){players3d[n]=tokenMesh(p.token,p.color);scene.add(players3d[n])}players3d[n].position.copy(pos3D(p.position,i*.035))});renderHouses3D(st);renderBuildPanel(st);if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st)}
function renderPlayersPanel(st){const names=Object.keys(st.players||{});playersList.innerHTML="";names.forEach((n,i)=>{const p=st.players[n];const fallbackColors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color){p.color=fallbackColors[i%fallbackColors.length]}const d=document.createElement("div");d.className="playerCard"+(i===st.turnIndex?" turn":"");d.innerHTML=`<div class="colorDot" style="background:${p.color};color:${p.color}"></div><div class="pawnIcon">${p.token||"🎩"}</div><div><div class="playerName">${n}</div><div class="playerMoney">${p.money}€</div></div>`;playersList.appendChild(d)})}
function renderOwners(st){Object.values(owners3d).forEach(o=>scene.remove(o));owners3d={};Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const m=new THREE.Mesh(new THREE.BoxGeometry(.72,.06,.16),new THREE.MeshStandardMaterial({color:p.color||0xd4af37,metalness:.3,roughness:.35}));m.position.copy(pos3D(+cell));m.position.y=.48;m.position.z+=.33;scene.add(m);owners3d[cell]=m})}
function moveToken(n,from,to,state,done){const mesh=players3d[n];if(!mesh){done();return}let steps=[],cur=from,total=coords2d.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let k=0;const id=setInterval(()=>{if(k>=steps.length){clearInterval(id);done();return}mesh.position.copy(pos3D(steps[k],idx*.035));mesh.position.y=.70;setTimeout(()=>mesh.position.y=.43,105);k++},220)}
...
socket.on("monopoly_animate_move",d=>{rollDiceAnim(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){
  const landed=Number(d.state.players[myName].position);
  buyCity.textContent=cells[landed]||"Mülk";
  buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";
  setTimeout(()=>{buyPanel.style.display="block";},120);
}})}else render(d.state)})});
