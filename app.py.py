...
.leftBtn{min-height:82px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;font-size:14px}.leftBtn .bigIcon{font-size:31px}
...
<div class="leftHud">
  ...
  <div class="leftBtn"><span class="bigIcon">🏦</span><span>BANKAYA<br>ÖDEME YAP</span></div>
  <div class="leftBtn"><span class="bigIcon">💬</span><span>SOHBET</span></div>
  <div class="leftBtn"><span class="bigIcon">☰</span><span>MENÜ</span></div>
</div>
...
<div id="buyPanel" class="buyPanel">
<div class="city" id="buyCity">PARIS</div>
<div class="question">satın almak ister misin?</div>
<div class="price" id="buyPrice">Fiyat: 160 €</div>
<button onclick="buyProperty()">SATIN AL</button>
<button class="dark" onclick="hideBuy()">VAZGEÇ</button>
</div>
...
<div class="bottomHud">
  <div class="hudPanel">
    <div class="hudTitle">ÖZEL YAPILAR</div>
    <div class="hudItem"><span class="icon">✈️</span>AÉROPORT<br>INTERNATIONAL<div class="price">200.000</div></div>
    <div class="hudItem"><span class="icon">🛥️</span>PORT<br>INTERNATIONAL<div class="price">200.000</div></div>
    <div class="hudItem"><span class="icon">🚐</span>GARE GRANDE<br>VITESSE<div class="price">200.000</div></div>
    <div class="hudItem"><span class="icon">⚡</span>COMPAGNIE<br>ÉLECTRIQUE<div class="price">150.000</div></div>
    <div class="hudItem"><span class="icon">💧</span>COMPAGNIE<br>DES EAUX<div class="price">150.000</div></div>
  </div>
  <div class="hudPanel">
    <div class="hudTitle">YAPILAR</div>
    <div class="hudItem"><span class="icon">🏠</span>1 EV<div class="price">200.000</div></div>
    <div class="hudItem"><span class="icon">🏘️</span>2 EV<div class="price">400.000</div></div>
    <div class="hudItem"><span class="icon">🏘️</span>3 EV<div class="price">600.000</div></div>
    <div class="hudItem"><span class="icon">🏘️</span>4 EV<div class="price">800.000</div></div>
    <div class="hudItem"><span class="icon">🏨</span>OTEL<div class="price">1.000.000</div></div>
  </div>
</div>
...
let scene,camera,renderer,board,dice,players3d={},owners3d={};
let world,diceBody,groundBody,physicsRunning=false;
...
function init3D(){
 ...
 camera=new THREE.PerspectiveCamera(31,c.clientWidth/c.clientHeight,.1,100);
 camera.position.set(0,12.6,6.1);
 ...
 const dlight=new THREE.DirectionalLight(0xffe1a0,1.8);
 dlight.position.set(0,9,5);dlight.castShadow=true;scene.add(dlight);
 const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png",()=>renderer.render(scene,camera));
 tex.colorSpace=THREE.SRGBColorSpace;
 board=new THREE.Mesh(new THREE.BoxGeometry(boardW,.18,boardH),new THREE.MeshStandardMaterial({map:tex,roughness:.52,metalness:.04}));
 board.rotation.x=0;board.receiveShadow=true;scene.add(board);
 const base=new THREE.Mesh(new THREE.BoxGeometry(boardW+.28,.16,boardH+.28),new THREE.MeshStandardMaterial({color:0x120c06,metalness:.45,roughness:.35}));
 base.position.y=-.19;scene.add(base);
 dice=createDice();dice.position.set(0,.98,.05);scene.add(dice);
 initPhysics();
 window.addEventListener("resize",resize3D);
 animate();
}
function initPhysics(){if(!window.CANNON)return;world=new CANNON.World();world.gravity.set(0,-9.82,0);world.broadphase=new CANNON.NaiveBroadphase();world.solver.iterations=12;const groundMat=new CANNON.Material("ground");const diceMat=new CANNON.Material("dice");world.addContactMaterial(new CANNON.ContactMaterial(groundMat,diceMat,{friction:.42,restitution:.38}));groundBody=new CANNON.Body({mass:0,material:groundMat});groundBody.addShape(new CANNON.Box(new CANNON.Vec3(boardW/2,.08,boardH/2)));groundBody.position.set(0,.48,0);world.addBody(groundBody);diceBody=new CANNON.Body({mass:1.15,material:diceMat,linearDamping:.28,angularDamping:.32});diceBody.addShape(new CANNON.Box(new CANNON.Vec3(.56,.56,.56)));diceBody.position.set(0,.98,.05);world.addBody(diceBody)}
...
function animate(){requestAnimationFrame(animate);if(world&&physicsRunning){world.step(1/60);dice.position.set(diceBody.position.x,diceBody.position.y,diceBody.position.z);dice.quaternion.set(diceBody.quaternion.x,diceBody.quaternion.y,diceBody.quaternion.z,diceBody.quaternion.w)}renderer.render(scene,camera)}
...
function createDice(){const g=new THREE.Group();const mat=new THREE.MeshStandardMaterial({color:0xffffff,roughness:.13,metalness:.015});const body=new THREE.Mesh(new THREE.BoxGeometry(1.12,1.12,1.12,8,8,8),mat);body.castShadow=true;body.receiveShadow=true;g.add(body);const edge=new THREE.Mesh(new THREE.BoxGeometry(1.15,1.15,1.15),new THREE.MeshStandardMaterial({color:0xd9d9d9,roughness:.28,transparent:true,opacity:.18}));g.add(edge);const dotMat=new THREE.MeshStandardMaterial({color:0x010101,roughness:.35});const faces=[[[0,0,.575],[0,0,0],[[0,0]]],[[0,0,-.575],[0,Math.PI,0],[[-.26,.26],[.26,-.26]]],[[.575,0,0],[0,Math.PI/2,0],[[-.29,.29],[0,0],[.29,-.29]]],[[-.575,0,0],[0,-Math.PI/2,0],[[-.29,.29],[.29,.29],[-.29,-.29],[.29,-.29]]],[[0,.575,0],[-Math.PI/2,0,0],[[-.31,.31],[.31,.31],[0,0],[-.31,-.31],[.31,-.31]]],[[0,-.575,0],[Math.PI/2,0,0],[[-.31,.33],[.31,.33],[-.31,0],[.31,0],[-.31,-.33],[.31,-.33]]]];faces.forEach(f=>f[2].forEach(([a,b])=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.08,24,12),dotMat);dot.position.set(a,b,.598);const h=new THREE.Group();h.add(dot);h.position.set(...f[0]);h.rotation.set(...f[1]);g.add(h)}));return g}
function showDice(n){const r={1:[-.25,.45,0],2:[1.25,.45,0],3:[0,3.14,0],4:[0,-1.57,0],5:[-1.57,0,0],6:[1.57,0,0]}[n]||[-.25,.45,0];dice.rotation.set(r[0],r[1],r[2])}
function rollDiceAnim(n,done){if(window.CANNON&&world&&diceBody){physicsRunning=true;diceBody.position.set(-1.55,3.0,-.55);diceBody.velocity.set(2.9,-2.6,1.15);diceBody.angularVelocity.set(13+Math.random()*5,17+Math.random()*5,10+Math.random()*5);diceBody.quaternion.setFromEuler(Math.random()*Math.PI,Math.random()*Math.PI,Math.random()*Math.PI);let ticks=0;const watcher=setInterval(()=>{ticks++;diceBody.position.x=Math.max(-2.15,Math.min(2.15,diceBody.position.x));diceBody.position.z=Math.max(-1.25,Math.min(1.25,diceBody.position.z));const v=diceBody.velocity.length();const av=diceBody.angularVelocity.length();if((ticks>75&&v<.22&&av<.34)||ticks>170){clearInterval(watcher);physicsRunning=false;dice.position.set(0,.98,.05);showDice(n);diceBody.position.set(0,.98,.05);diceBody.velocity.set(0,0,0);diceBody.angularVelocity.set(0,0,0);diceBody.quaternion.set(dice.quaternion.x,dice.quaternion.y,dice.quaternion.z,dice.quaternion.w);if(done)done()}},32);return}let frame=0;const duration=56;const startPos=new THREE.Vector3(-1.55,3.0,-.55);const endPos=new THREE.Vector3(0,.98,.05);dice.position.copy(startPos);const interval=setInterval(()=>{frame++;const t=frame/duration;const ease=1-Math.pow(1-t,2.4);dice.position.x=startPos.x+(endPos.x-startPos.x)*ease;dice.position.z=startPos.z+(endPos.z-startPos.z)*ease;const bounce=Math.abs(Math.sin(t*Math.PI*5.4))*(1-t)*1.05;dice.position.y=endPos.y+bounce;const speed=Math.max(.05,1-t);dice.rotation.x+=.95*speed;dice.rotation.y+=1.10*speed;dice.rotation.z+=.62*speed;if(frame>=duration){clearInterval(interval);dice.position.copy(endPos);showDice(n);if(done)done()}},22)}
...
function joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value})}
function startGame(){socket.emit("monopoly_start_game",{code:roomCode})}
function rollDice(){socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}
...
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayersPanel(st);renderOwners(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];if(!players3d[n]){players3d[n]=tokenMesh(p.token,p.color);scene.add(players3d[n])}players3d[n].position.copy(pos3D(p.position,i*.025))});if(st.players&&st.players[myName]){const lp=st.players[myName];leftPlayerName.textContent=myName;leftPlayerMoney.textContent=lp.money+"€";leftPawn.textContent=lp.token||tokenSelect.value}if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st)}
...
socket.on("monopoly_animate_move",d=>{rollDiceAnim(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=Number(d.state.players[myName].position);buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";setTimeout(()=>{buyPanel.style.display="block"},180)}})}else render(d.state)})});
