...
def m_public(r):
    return {"code":r["code"],"started":r["started"],"turnIndex":r["turnIndex"],"players":r["players"],"owners":r["owners"],"houses":r.get("houses",{}),"hotels":r.get("hotels",{}),"mortgages":r.get("mortgages",{}),"lastLog":r["lastLog"],"lastDice":r["lastDice"]}
...
def mr(data):
    ...
    r["lastLog"]=log
    emit("monopoly_animate_move",{"username":u,"from":old,"to":new,"dice":d,"state":m_public(r),"pendingBuy":pending},room=c)
...
def mh(data):
    ...
    try:
        ...
        emit("monopoly_error",{"msg":"Hatalı arsa."}); return
    ok,msg=can_build_house(r,u,i)
    if not ok:
        emit("monopoly_error",{"msg":msg}); return
    if r["players"][u]["money"]<HOUSE_COST:
        emit("monopoly_error",{"msg":"Ev için para yetmiyor."}); return
    r["players"][u]["money"]-=HOUSE_COST
    r["houses"][pos]=int(r["houses"].get(pos,0))+1
    r["lastLog"]=f"{u}, {MONOPOLY_CELLS[i][0]} üzerine ev kurdu: -50€."
    emit("monopoly_room_state",m_public(r),room=c)
...
def mhotel(data):
    ...
    try:
        ...
        emit("monopoly_error",{"msg":"Hatalı arsa."}); return
    ok,msg=can_build_hotel(r,u,i)
    if not ok:
        emit("monopoly_error",{"msg":msg}); return
    if r["players"][u]["money"]<HOTEL_COST:
        emit("monopoly_error",{"msg":"Otel için para yetmiyor."}); return
    r["players"][u]["money"]-=HOTEL_COST
    r["houses"][pos]=0
    r["hotels"][pos]=1
    r["lastLog"]=f"{u}, {MONOPOLY_CELLS[i][0]} üzerine otel kurdu: -50€."
    emit("monopoly_room_state",m_public(r),room=c)
...
@socketio.on("monopoly_add_house")
def mh(data):
    c=((data or {}).get("code","") or "").strip().upper(); u=(data or {}).get("username","").strip(); pos=str((data or {}).get("cell","")); r=MONOPOLY_ROOMS.get(c)
    if not r or r["owners"].get(pos)!=u: emit("monopoly_error",{"msg":"Ev koyamazsın."}); return
    i=int(pos); grp=MONOPOLY_CELLS[i][4]
    if not all(r["owners"].get(str(x))==u for x in MONOPOLY_GROUPS.get(grp,[])): emit("monopoly_error",{"msg":"Aynı rengin tamamını almalısın."}); return
    if r["players"][u]["money"]<50: emit("monopoly_error",{"msg":"Para yetmiyor."}); return
    r["players"][u]["money"]-=50; r["houses"][pos]=int(r["houses"].get(pos,0))+1; r["lastLog"]=u+" ev koydu."; emit("monopoly_room_state",m_public(r),room=c)
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>METROPOLY DELUXE FIXED</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/cannon@0.6.2/build/cannon.min.js"></script>
<style>
*{box-sizing:border-box}
body{margin:0;overflow:hidden;background:radial-gradient(circle at 50% 0%,#201204,#050505 48%,#000);color:#fff;font-family:Arial,Helvetica,sans-serif}
.loca{position:fixed;top:12px;left:12px;z-index:90;width:110px;height:58px;display:flex;align-items:center;justify-content:center;border:1px solid #d4af37;border-radius:16px;background:#000d;color:#ffd989;text-decoration:none;font-size:18px;font-weight:900;box-shadow:0 0 18px rgba(212,175,55,.35),inset 0 0 12px rgba(212,175,55,.12)}
.topLobby{position:fixed;top:12px;left:130px;right:330px;z-index:90;height:58px;display:flex;gap:8px;align-items:center;padding:8px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(180deg,rgba(25,25,25,.96),rgba(0,0,0,.94));box-shadow:0 0 22px #000,inset 0 0 12px rgba(212,175,55,.08)}
.topLobby input,.topLobby select,.topLobby button{height:40px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:#fff;padding:8px 10px;font-weight:800}
.topLobby button{background:linear-gradient(#ffe08a,#a96d0d);color:#160800;cursor:pointer}
#username{width:155px}#tokenSelect{width:140px}#roomCodeInput{width:125px}.roomLabel{color:#d4af37;font-weight:900;white-space:nowrap}
#game3d{position:fixed;top:84px;left:150px;right:330px;bottom:160px;z-index:1}
.leftHud{position:fixed;left:14px;top:84px;bottom:160px;width:122px;z-index:44;display:flex;flex-direction:column;gap:12px}
.leftCard,.leftBtn{border:1px solid #d4af37aa;border-radius:14px;background:linear-gradient(180deg,rgba(20,20,20,.95),rgba(0,0,0,.92));box-shadow:0 0 18px rgba(0,0,0,.8),inset 0 0 10px rgba(212,175,55,.06);color:#ffd989;text-align:center;padding:10px 6px;font-weight:900}
.leftCard .avatar{width:58px;height:58px;border-radius:50%;margin:0 auto 6px;background:radial-gradient(circle,#4a2a12,#050505);border:2px solid #d4af37;display:grid;place-items:center;font-size:34px}
.leftBtn{min-height:82px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;font-size:14px;cursor:pointer;user-select:none}
.leftBtn:hover{filter:brightness(1.16);box-shadow:0 0 18px rgba(212,175,55,.55),inset 0 0 12px rgba(212,175,55,.15)}.leftBtn .bigIcon{font-size:31px}
.playersPanel{position:fixed;top:84px;right:14px;width:300px;bottom:160px;z-index:45;border:1px solid #d4af37aa;border-radius:18px;background:linear-gradient(180deg,rgba(16,16,16,.96),rgba(0,0,0,.94));box-shadow:0 0 25px #000;padding:14px;overflow:auto}
.playersPanel h2{margin:0 0 12px;color:#ffd989;font-size:21px;line-height:1.05}
.playerCard{display:flex;align-items:center;gap:10px;border:1px solid #d4af3755;border-radius:14px;min-height:66px;padding:10px;margin:8px 0;background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(0,0,0,.52))}
.playerCard.turn{border-color:#00ff66;box-shadow:0 0 16px #00ff6670}.colorDot{width:18px;height:18px;border-radius:50%;border:2px solid rgba(255,255,255,.75);box-shadow:0 0 10px currentColor}.pawnIcon{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;background:#0009;border:1px solid #d4af37;font-size:23px}.playerName{font-weight:900}.playerMoney{color:#ffd989}
.rollBtn{position:fixed;left:50%;bottom:98px;transform:translateX(-50%);z-index:70;width:330px;height:62px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(180deg,#f6c361 0%,#b87312 50%,#4d2700 100%);color:#180b00;font-size:32px;font-weight:900;text-shadow:0 1px 0 rgba(255,255,255,.45);box-shadow:0 0 25px rgba(212,175,55,.55),0 12px 24px #000,inset 0 0 12px rgba(255,255,255,.2);cursor:pointer}
.bottomHud{position:fixed;left:14px;right:14px;bottom:12px;height:78px;z-index:46;display:grid;grid-template-columns:1fr 1fr;gap:14px;pointer-events:none}
.hudPanel{border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(180deg,rgba(15,15,15,.96),rgba(0,0,0,.94));box-shadow:0 0 22px #000,inset 0 0 12px rgba(212,175,55,.07);display:flex;align-items:center;justify-content:space-around;padding:8px 12px;position:relative}.hudTitle{position:absolute;top:-19px;left:50%;transform:translateX(-50%);color:#d4af37;font-size:14px;font-weight:900;background:#050505;padding:0 12px}.hudItem{color:#ffd989;text-align:center;font-weight:900;font-size:11px;min-width:82px}.hudItem .icon{font-size:26px;display:block;margin-bottom:2px}.hudItem .price{color:#e7be64;font-size:12px}
.buyPanel{position:fixed;left:50%;top:43%;transform:translate(-50%,-50%);z-index:80;width:410px;max-width:90%;padding:22px;border-radius:22px;background:rgba(0,0,0,.78);border:1px solid #d4af37;text-align:center;display:none;box-shadow:0 0 26px rgba(212,175,55,.45),inset 0 0 14px rgba(212,175,55,.08)}
.buyPanel .city{font-size:26px;font-weight:900;color:#ff9500}.buyPanel .question{font-size:21px;font-weight:800;margin:12px 0}.buyPanel .price{margin-bottom:18px}.buyPanel button{width:135px;margin:0 8px;padding:12px;border-radius:12px;border:1px solid #d4af37;font-weight:900;cursor:pointer}.buyPanel button:first-of-type{background:#11651b;color:#fff}.buyPanel .dark{background:#55120f;color:#fff}
.toast{position:fixed;left:16px;bottom:96px;z-index:80;width:250px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.toast.show{opacity:1}
.diceOverlay{position:fixed;left:50%;top:45%;width:96px;height:96px;transform-style:preserve-3d;transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg);z-index:72;pointer-events:none;filter:drop-shadow(0 18px 16px rgba(0,0,0,.9)) drop-shadow(0 0 14px rgba(255,255,255,.28));transition:transform .35s ease,left .35s ease,top .35s ease}
.diceOverlay.rolling{animation:deluxeDiceThrow 1.25s cubic-bezier(.16,.83,.28,1) both}
@keyframes deluxeDiceThrow{0%{left:42%;top:33%;transform:translate(-50%,-50%) rotateX(0deg) rotateY(0deg) rotateZ(0deg) scale(.88)}22%{left:47%;top:38%;transform:translate(-50%,-50%) rotateX(260deg) rotateY(340deg) rotateZ(130deg) scale(1.08)}45%{left:51%;top:46%;transform:translate(-50%,-50%) rotateX(540deg) rotateY(620deg) rotateZ(290deg) scale(1)}68%{left:49.5%;top:42%;transform:translate(-50%,-50%) rotateX(760deg) rotateY(820deg) rotateZ(410deg) scale(.96)}100%{left:50%;top:45%;transform:translate(-50%,-50%) rotateX(940deg) rotateY(1040deg) rotateZ(520deg) scale(1)}}
.diceFaceCss{position:absolute;width:96px;height:96px;border-radius:18px;background:radial-gradient(circle at 28% 22%,rgba(255,255,255,.95),rgba(255,255,255,0) 28%),linear-gradient(145deg,#ffffff,#e4e4e4 56%,#9b9b9b);border:3px solid #fff;box-shadow:inset -12px -12px 22px rgba(0,0,0,.28), inset 9px 9px 18px rgba(255,255,255,.88)}
.diceFront{transform:translateZ(48px)}.diceBack{transform:rotateY(180deg) translateZ(48px)}.diceRight{transform:rotateY(90deg) translateZ(48px)}.diceLeft{transform:rotateY(-90deg) translateZ(48px)}.diceTop{transform:rotateX(90deg) translateZ(48px)}.diceBottom{transform:rotateX(-90deg) translateZ(48px)}
.dotCss{position:absolute;width:15px;height:15px;border-radius:50%;background:radial-gradient(circle at 30% 30%,#555,#050505);box-shadow:0 2px 3px rgba(0,0,0,.55)}.dtl{left:20px;top:20px}.dtr{right:20px;top:20px}.dbl{left:20px;bottom:20px}.dbr{right:20px;bottom:20px}.dc{left:50%;top:50%;transform:translate(-50%,-50%)}.dml{left:20px;top:50%;transform:translateY(-50%)}.dmr{right:20px;top:50%;transform:translateY(-50%)}
.diceOverlay.show1{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg)}.diceOverlay.show2{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}.diceOverlay.show3{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(208deg)}.diceOverlay.show4{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(-62deg)}.diceOverlay.show5{transform:translate(-50%,-50%) rotateX(-108deg) rotateY(28deg)}.diceOverlay.show6{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}
.popupPanel{position:fixed;left:50%;top:42%;transform:translate(-50%,-50%);width:390px;max-width:90vw;border:1px solid #d4af37;border-radius:18px;background:rgba(0,0,0,.9);z-index:120;padding:18px;box-shadow:0 0 26px rgba(212,175,55,.35);display:none}.popupPanel h3{margin:0 0 12px;color:#ffd989}.popupPanel textarea{width:100%;height:90px;border-radius:12px;background:#080808;color:white;border:1px solid #d4af37;padding:10px}.popupPanel button{margin-top:10px;padding:10px 14px;border-radius:10px;border:1px solid #d4af37;background:linear-gradient(#ffe08a,#a96d0d);font-weight:900;cursor:pointer}
@media(max-width:950px){body{overflow:auto}.leftHud,.bottomHud{display:none}.topLobby{left:8px;right:8px;top:56px;height:auto;flex-wrap:wrap}.playersPanel{position:relative;top:auto;right:auto;width:auto;margin:8px}.loca{top:8px}#game3d{position:relative;top:auto;left:0;right:0;height:65vw;min-height:420px;margin-top:150px}.rollBtn{left:50%;bottom:12px;width:230px;font-size:24px}.toast{bottom:80px}.diceOverlay{width:78px;height:78px}.diceFaceCss{width:78px;height:78px}.diceFront{transform:translateZ(39px)}.diceBack{transform:rotateY(180deg) translateZ(39px)}.diceRight{transform:rotateY(90deg) translateZ(39px)}.diceLeft{transform:rotateY(-90deg) translateZ(39px)}.diceTop{transform:rotateX(90deg) translateZ(39px)}.diceBottom{transform:rotateX(-90deg) translateZ(39px)}}
</style>
</head>
<body>
<a href="/" class="loca">🚪 LOCA</a>
<div class="topLobby">
<input id="username" placeholder="Kullanıcı adı">
<select id="tokenSelect">
<option value="✈️">✈️ Jet</option><option value="🎰">🎰 Casino</option><option value="🛥️">🛥️ Yacht</option><option value="🚗">🚗 Voiture</option><option value="🎩">🎩 Chapeau</option><option value="🐈">🐈 Chat</option>
</select>
<button onclick="createRoom()">ODA OLUŞTUR</button>
<input id="roomCodeInput" placeholder="MON-1234">
<button onclick="joinRoom()">ODAYA GİR</button>
<button onclick="startGame()">OYUNU BAŞLAT</button>
<span id="topRoomLabel" class="roomLabel">Oda: -</span>
</div>
<div id="game3d"></div>
<div class="leftHud">
  <div class="leftCard"><div class="avatar">👤</div><div id="leftPlayerName">Oyuncu</div><div id="leftPlayerMoney" style="color:#e7be64;margin-top:4px">1000€</div></div>
  <div class="leftBtn"><span class="bigIcon">🎲</span><span>PİYONUN</span><span id="leftPawn">🎩</span></div>
  <div class="leftBtn" onclick="bankPay()"><span class="bigIcon">🏦</span><span>BANKAYA<br>ÖDEME YAP</span></div>
  <div class="leftBtn" onclick="openPopup('chatPanel')"><span class="bigIcon">💬</span><span>SOHBET</span></div>
  <div class="leftBtn" onclick="openPopup('menuPanel')"><span class="bigIcon">☰</span><span>MENÜ</span></div>
</div>
<div class="playersPanel"><h2>ONLINE OYUNCULAR / RENKLER</h2><div id="playersList"></div></div>
<div id="diceOverlay" class="diceOverlay show1">
  <div class="diceFaceCss diceFront"><span class="dotCss dc"></span></div>
  <div class="diceFaceCss diceBack"><span class="dotCss dtl"></span><span class="dotCss dc"></span><span class="dotCss dbr"></span></div>
  <div class="diceFaceCss diceRight"><span class="dotCss dtl"></span><span class="dotCss dtr"></span><span class="dotCss dbl"></span><span class="dotCss dbr"></span></div>
  <div class="diceFaceCss diceLeft"><span class="dotCss dtl"></span><span class="dotCss dtr"></span><span class="dotCss dc"></span><span class="dotCss dbl"></span><span class="dotCss dbr"></span></div>
  <div class="diceFaceCss diceTop"><span class="dotCss dtl"></span><span class="dotCss dtr"></span><span class="dotCss dml"></span><span class="dotCss dmr"></span><span class="dotCss dbl"></span><span class="dotCss dbr"></span></div>
  <div class="diceFaceCss diceBottom"><span class="dotCss dtl"></span><span class="dotCss dbr"></span></div>
</div>
<button class="rollBtn" onclick="rollDice()">🎲 ZAR AT</button>
<div id="buyPanel" class="buyPanel"><div class="city" id="buyCity">PARIS</div><div class="question">satın almak ister misin?</div><div class="price" id="buyPrice">Fiyat: 160 €</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">VAZGEÇ</button></div>
<div id="moneyToast" class="toast"></div>
<div id="chatPanel" class="popupPanel"><h3>💬 Sohbet</h3><textarea id="chatText" placeholder="Mesaj yaz..."></textarea><button onclick="closePopup('chatPanel')">KAPAT</button></div>
<div id="menuPanel" class="popupPanel"><h3>☰ Menü</h3><button onclick="location.href='/'">LOCA'YA DÖN</button><button onclick="closePopup('menuPanel')">KAPAT</button></div>
<div class="bottomHud">
  <div class="hudPanel"><div class="hudTitle">ÖZEL YAPILAR</div><div class="hudItem"><span class="icon">✈️</span>AÉROPORT<br>INTERNATIONAL<div class="price">200.000</div></div><div class="hudItem"><span class="icon">🛥️</span>PORT<br>INTERNATIONAL<div class="price">200.000</div></div><div class="hudItem"><span class="icon">🚐</span>GARE GRANDE<br>VITESSE<div class="price">200.000</div></div><div class="hudItem"><span class="icon">⚡</span>COMPAGNIE<br>ÉLECTRIQUE<div class="price">150.000</div></div><div class="hudItem"><span class="icon">💧</span>COMPAGNIE<br>DES EAUX<div class="price">150.000</div></div></div>
  <div class="hudPanel"><div class="hudTitle">YAPILAR</div><div class="hudItem"><span class="icon">🏠</span>1 EV<div class="price">200.000</div></div><div class="hudItem"><span class="icon">🏘️</span>2 EV<div class="price">400.000</div></div><div class="hudItem"><span class="icon">🏘️</span>3 EV<div class="price">600.000</div></div><div class="hudItem"><span class="icon">🏘️</span>4 EV<div class="price">800.000</div></div><div class="hudItem"><span class="icon">🏨</span>OTEL<div class="price">1.000.000</div></div></div>
</div>
<script>
const socket=io();
let roomCode="",myName="",currentState=null,previousMoneyState={};
const cells=["ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Milan","Amsterdam","Eaux","Moscou","Rome","Madrid","Impôts","Sofia","Belgrad","Gare","Istanbul","Prague","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo"];
const prices=[0,0,220,200,200,200,100,100,0,160,140,140,100,120,100,100,0,60,70,100,70,80,90,0,160,160,150,180];
const coords2d=[[87.0,63.5],[85.5,78.0],[75.0,78.0],[64.0,78.0],[53.6,78.0],[42.9,78.0],[33.0,78.0],[23.8,78.0],[16.8,77.4],[17.3,65.7],[17.3,55.4],[17.3,45.2],[17.3,35.0],[17.3,25.8],[17.3,18.5],[17.3,11.6],[21.8,7.9],[31.8,7.9],[41.4,7.9],[50.7,7.9],[59.5,7.9],[68.7,7.9],[77.8,7.9],[86.8,7.9],[86.8,19.8],[86.8,31.2],[86.8,43.0],[86.8,54.7]];
let scene,camera,renderer,board,players3d={},owners3d={};
const boardW=14.4,boardH=9.0;
init3D();
function init3D(){const c=document.getElementById("game3d");scene=new THREE.Scene();camera=new THREE.PerspectiveCamera(31,c.clientWidth/c.clientHeight,.1,100);camera.position.set(0,12.6,6.1);camera.lookAt(0,0.05,0);renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.45));renderer.setSize(c.clientWidth,c.clientHeight);renderer.shadowMap.enabled=true;c.appendChild(renderer.domElement);scene.add(new THREE.AmbientLight(0xffffff,.95));const dlight=new THREE.DirectionalLight(0xffe1a0,1.8);dlight.position.set(0,9,5);dlight.castShadow=true;scene.add(dlight);const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png",()=>renderer.render(scene,camera));tex.colorSpace=THREE.SRGBColorSpace;board=new THREE.Mesh(new THREE.BoxGeometry(boardW,.18,boardH),new THREE.MeshStandardMaterial({map:tex,roughness:.52,metalness:.04}));board.rotation.x=0;board.receiveShadow=true;scene.add(board);const base=new THREE.Mesh(new THREE.BoxGeometry(boardW+.28,.16,boardH+.28),new THREE.MeshStandardMaterial({color:0x120c06,metalness:.45,roughness:.35}));base.position.y=-.19;scene.add(base);window.addEventListener("resize",resize3D);animate()}
function resize3D(){const c=document.getElementById("game3d");camera.aspect=c.clientWidth/c.clientHeight;camera.updateProjectionMatrix();renderer.setSize(c.clientWidth,c.clientHeight)}
function animate(){requestAnimationFrame(animate);renderer.render(scene,camera)}
function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.50,(p[1]/100-.5)*boardH+off)}
function tokenMesh(icon,color){const g=new THREE.Group();const s=new THREE.Mesh(new THREE.SphereGeometry(.16,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.72,roughness:.25}));s.castShadow=true;g.add(s);const canvas=document.createElement("canvas");canvas.width=128;canvas.height=128;const ctx=canvas.getContext("2d");ctx.font="72px Arial";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(icon||"🎩",64,68);const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(canvas),transparent:true}));sp.position.y=.34;sp.scale.set(.40,.40,.40);g.add(sp);return g}
function openPopup(id){const el=document.getElementById(id);if(el)el.style.display="block"}
function closePopup(id){const el=document.getElementById(id);if(el)el.style.display="none"}
function bankPay(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}showMoneyToast("Banka işlemi otomatik yapılır.",true)}
function showDiceOverlay(n){if(!window.diceOverlay)return;diceOverlay.classList.remove("show1","show2","show3","show4","show5","show6","rolling");diceOverlay.classList.add("show"+n)}
function animateDiceOverlay(n,done){if(!window.diceOverlay){if(done)done();return}diceOverlay.classList.remove("show1","show2","show3","show4","show5","show6","rolling");void diceOverlay.offsetWidth;diceOverlay.classList.add("rolling");setTimeout(()=>{diceOverlay.classList.remove("rolling");diceOverlay.classList.add("show"+n);if(done)done()},1280)}
function name(){return (username.value||localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Misafir").trim()}
function createRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}
function joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value.trim().toUpperCase()})}
function startGame(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_start_game",{code:roomCode})}
function rollDice(){if(!roomCode||!myName){alert("Önce odaya gir veya oda oluştur.");return}hideBuy();socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}
function buyProperty(){if(!roomCode||!myName){alert("Oda bilgisi eksik.");return}socket.emit("monopoly_buy_property",{code:roomCode,username:myName});hideBuy()}
function hideBuy(){buyPanel.style.display="none"}
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayersPanel(st);renderOwners(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];if(!players3d[n]){players3d[n]=tokenMesh(p.token,p.color);scene.add(players3d[n])}players3d[n].position.copy(pos3D(p.position,i*.025))});if(st.players&&st.players[myName]){const lp=st.players[myName];leftPlayerName.textContent=myName;leftPlayerMoney.textContent=lp.money+"€";leftPawn.textContent=lp.token||tokenSelect.value}if(st.lastDice)showDiceOverlay(st.lastDice);detectMoneyChanges(st)}
function renderPlayersPanel(st){const names=Object.keys(st.players||{});playersList.innerHTML="";names.forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];const d=document.createElement("div");d.className="playerCard"+(i===st.turnIndex?" turn":"");d.innerHTML=`<div class="colorDot" style="background:${p.color};color:${p.color}"></div><div class="pawnIcon">${p.token||"🎩"}</div><div><div class="playerName">${n}</div><div class="playerMoney">${p.money}€</div></div>`;playersList.appendChild(d)})}
function renderOwners(st){Object.values(owners3d).forEach(o=>scene.remove(o));owners3d={};Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const m=new THREE.Mesh(new THREE.BoxGeometry(.72,.06,.16),new THREE.MeshStandardMaterial({color:p.color||0xd4af37,metalness:.3,roughness:.35}));m.position.copy(pos3D(+cell));m.position.y=.52;m.position.z+=.33;scene.add(m);owners3d[cell]=m})}
function moveToken(n,from,to,state,done){const mesh=players3d[n];if(!mesh){done();return}let steps=[],cur=from,total=coords2d.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let stepIndex=0;function animateStep(){if(stepIndex>=steps.length){done();return}const start=mesh.position.clone();const end=pos3D(steps[stepIndex],idx*.025);let startTime=null;const duration=260;function frame(ts){if(!startTime)startTime=ts;const t=Math.min(1,(ts-startTime)/duration);const ease=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;mesh.position.x=start.x+(end.x-start.x)*ease;mesh.position.z=start.z+(end.z-start.z)*ease;mesh.position.y=.50+Math.sin(Math.PI*t)*.28;if(t<1){requestAnimationFrame(frame)}else{mesh.position.copy(end);mesh.position.y=.50;stepIndex++;setTimeout(animateStep,35)}}requestAnimationFrame(frame)}animateStep()}
function showMoneyToast(msg,positive=true){moneyToast.textContent=msg;moneyToast.style.color=positive?"#00ff66":"#ff4d4d";moneyToast.classList.add("show");clearTimeout(window.moneyToastTimer);window.moneyToastTimer=setTimeout(()=>moneyToast.classList.remove("show"),5000)}
function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const d=p.money-previousMoneyState[n];if(d>0)showMoneyToast(n+" "+d+"€ aldı",true);if(d<0)showMoneyToast(n+" "+Math.abs(d)+"€ ödedi",false)}previousMoneyState[n]=p.money})}
socket.on("monopoly_room_state",render);
socket.on("monopoly_error",d=>alert(d.msg));
socket.on("monopoly_animate_move",d=>{animateDiceOverlay(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=Number(d.state.players[myName].position);buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";setTimeout(()=>{buyPanel.style.display="block"},180)}})}else render(d.state)})});
username.value=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"";
</script>
</body>
</html>"""
