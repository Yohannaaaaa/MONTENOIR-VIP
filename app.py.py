...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>🏛️ MONTENOIR VIP Özellikler</title>
<style>
...
.wrap{max-width:1150px;margin:auto;padding:35px 18px}
h1{font-family:Georgia,serif;font-size:46px;text-shadow:0 0 18px #d4af37}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:18px;margin-top:28px}
.card{border:1px solid #d4af37;border-radius:24px;background:rgba(0,0,0,.72);padding:22px;box-shadow:0 0 22px rgba(212,175,55,.25)}
.card h2{font-size:23px;margin:5px 0 12px}
.card p{color:#f4dfb0;line-height:1.5;font-size:14px}
.btn{display:inline-block;margin-top:12px;padding:12px 18px;border:1px solid #d4af37;border-radius:14px;color:#000;background:linear-gradient(135deg,#d4af37,#fff0a8,#9b7419);text-decoration:none;font-weight:900}
...
<h1>🏛️ MONTENOIR VIP Özellik Merkezi</h1>
<div class="grid">
<div class="card"><h2>🪙 MONTENOIR Kasası</h2><p>Ortak jeton sistemi. Tüm oyunlar ve tarot aynı bakiyeyi kullanır.</p><a class="btn" href="/kasa">Kasaya Git</a></div>
<div class="card"><h2>🎟️ Turnuvalar</h2><p>Haftalık Codenames turnuvası, giriş 100 jeton, ödül 5.000 jeton.</p><a class="btn" href="/turnuvalar">Aç</a></div>
<div class="card"><h2>🎨 Profil Özelleştirme</h2><p>Altın, Elmas, Barok çerçeveler, isim renkleri ve animasyonlu profil.</p><a class="btn" href="/profil-magazasi">Mağaza</a></div>
<div class="card"><h2>🎁 Sandık Sistemi</h2><p>Bronz, Gümüş, Altın, Elmas sandıklar. İçinden jeton, rozet ve çerçeve çıkabilir.</p><a class="btn" href="/sandiklar">Sandıkları Aç</a></div>
<div class="card"><h2>🏆 Başarımlar</h2><p>İlk Zafer, 10 Oyun Kazandı, 100 Oyun Oynadı, Tarot Ustası, Poker Kralı.</p><a class="btn" href="/basarimlar">Gör</a></div>
<div class="card"><h2>👥 Arkadaş Sistemi</h2><p>Arkadaş ekleme, özel mesaj ve çevrimiçi durumu.</p><a class="btn" href="/arkadaslar">Aç</a></div>
<div class="card"><h2>🤖 AI Tarot Premium</h2><p>Doğum tarihi, soru ve fotoğraf notu ile uzun kişisel yorum.</p><a class="btn" href="/ai-tarot-premium">Başlat</a></div>
</div>
</div>
</body></html>
"""
...
SIMPLE_PAGE_TEMPLATE = r"""
<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{title}</title>
<style>body{{margin:0;min-height:100vh;background:radial-gradient(circle at top,#30220a,#000);color:#f6d777;font-family:Arial;text-align:center}}.wrap{{max-width:1000px;margin:auto;padding:35px 18px}}.box{{border:1px solid #d4af37;border-radius:24px;background:rgba(0,0,0,.75);padding:24px;margin:18px auto;box-shadow:0 0 22px rgba(212,175,55,.3)}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}}.item{{border:1px solid rgba(212,175,55,.6);border-radius:18px;padding:18px;background:rgba(255,255,255,.04)}}a{{color:#d4af37}}</style></head><body><div class="wrap"><a href="/">← Ana sayfa</a><h1>{title}</h1><div class="box">{content}</div></div></body></html>
"""

@app.route("/features")
def features_center():
    return render_template_string(FEATURES_HTML)

@app.route("/kasa")
def montenoir_kasa():
    return render_template_string(KASA_HTML)
...
def tournaments_page():
    ...
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="🎟️ Turnuvalar", content=content)
...
def profile_shop_page():
    ...
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="🎨 Profil Özelleştirme", content=content)
...
def chests_page():
    ...
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="🎁 Sandık Sistemi", content=content)
...
def achievements_page():
    ...
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="🏆 Başarımlar", content=content)
...
def friends_page():
    ...
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="👥 Arkadaş Sistemi", content=content)

@app.route("/ai-tarot-premium")
def ai_tarot_premium_page():
    content = """<p>Premium AI tarot için kullanıcı doğum tarihi, soru ve fotoğraf notu gönderecek.</p><p>Sonraki sürümde uzun kişisel yorum motoru bağlanacak.</p><a href='/tarot'>Tarot sayfasına git</a>"""
    return render_template_string(SIMPLE_PAGE_TEMPLATE, title="🤖 AI Tarot Premium", content=content)
