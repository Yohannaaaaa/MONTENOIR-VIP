...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>METROPOLY 3D stable</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
...
const coords2d=[[87.0,63.5],[85.5,78.0],[77.2,78.0],[66.8,78.0],[56.5,78.0],[46.5,78.0],[36.8,78.0],[27.5,78.0],[16.8,77.4],[17.3,65.7],[17.3,55.4],[17.3,45.2],[17.3,35.0],[17.3,25.8],[17.3,18.5],[17.3,11.6],[21.8,7.9],[31.8,7.9],[41.4,7.9],[50.7,7.9],[59.5,7.9],[68.7,7.9],[77.8,7.9],[86.8,7.9],[86.8,19.8],[86.8,31.2],[86.8,43.0],[86.8,54.7]];
...
function init3D(){
 ...
 dice=createDice(); dice.position.set(0,0.92,0.15); scene.add(dice);
 initPhysics();
...
function initPhysics(){
 ...
 world=new CANNON.World(); world.gravity.set(0,-9.82,0); world.broadphase=new CANNON.NaiveBroadphase(); world.solver.iterations=12;
 const groundMat=new CANNON.Material("ground"); const diceMat=new CANNON.Material("dice");
 world.addContactMaterial(new CANNON.ContactMaterial(groundMat,diceMat,{friction:.42,restitution:.38}));
 groundBody=new CANNON.Body({mass:0,material:groundMat}); groundBody.addShape(new CANNON.Box(new CANNON.Vec3(boardW/2,.08,boardH/2))); groundBody.position.set(0,.40,0); world.addBody(groundBody);
 diceBody=new CANNON.Body({mass:1.15,material:diceMat,linearDamping:.28,angularDamping:.32}); diceBody.addShape(new CANNON.Box(new CANNON.Vec3(.525,.525,.525))); diceBody.position.set(0,.92,.15); world.addBody(diceBody);
}
...
function tokenMesh(icon,color){const g=new THREE.Group();const s=new THREE.Mesh(new THREE.SphereGeometry(.18,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.72,roughness:.25}));s.castShadow=true;g.add(s);const canvas=document.createElement("canvas");canvas.width=128;canvas.height=128;const ctx=canvas.getContext("2d");ctx.font="72px Arial";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(icon||"🎩",64,68);const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(canvas),transparent:true}));sp.position.y=.34;sp.scale.set(.44,.44,.44);g.add(sp);return g}
function createDice(){
 const g=new THREE.Group();
 const mat=new THREE.MeshStandardMaterial({color:0xffffff,roughness:.16,metalness:.015});
 const box=new THREE.Mesh(new THREE.BoxGeometry(1.05,1.05,1.05,6,6,6),mat);box.castShadow=true;box.receiveShadow=true;g.add(box);
 const dotMat=new THREE.MeshStandardMaterial({color:0x020202,roughness:.38});
 const faces=[[[0,0,.535],[0,0,0],[[0,0]]],[[0,0,-.535],[0,Math.PI,0],[[-.22,.22],[.22,-.22]]],[[.535,0,0],[0,Math.PI/2,0],[[-.25,.25],[0,0],[.25,-.25]]],[[-.535,0,0],[0,-Math.PI/2,0],[[-.25,.25],[.25,.25],[-.25,-.25],[.25,-.25]]],[[0,.535,0],[-Math.PI/2,0,0],[[-.27,.27],[.27,.27],[0,0],[-.27,-.27],[.27,-.27]]],[[0,-.535,0],[Math.PI/2,0,0],[[-.27,.29],[.27,.29],[-.27,0],[.27,0],[-.27,-.29],[.27,-.29]]]];
 faces.forEach(f=>f[2].forEach(([a,b])=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.075,24,12),dotMat);dot.position.set(a,b,.555);const h=new THREE.Group();h.add(dot);h.position.set(...f[0]);h.rotation.set(...f[1]);g.add(h)}));
 return g;
}
...
function rollDiceAnim(n,done){
 if(window.CANNON&&world&&diceBody){physicsRunning=true;diceBody.position.set(-1.25,2.75,-.45);diceBody.velocity.set(2.2,-2.2,1.05);diceBody.angularVelocity.set(10+Math.random()*5,14+Math.random()*5,8+Math.random()*4);diceBody.quaternion.setFromEuler(Math.random()*Math.PI,Math.random()*Math.PI,Math.random()*Math.PI);let ticks=0;const watcher=setInterval(()=>{ticks++;diceBody.position.x=Math.max(-1.9,Math.min(1.9,diceBody.position.x));diceBody.position.z=Math.max(-1.1,Math.min(1.1,diceBody.position.z));const v=diceBody.velocity.length();const av=diceBody.angularVelocity.length();if((ticks>68&&v<.25&&av<.38)||ticks>145){clearInterval(watcher);physicsRunning=false;dice.position.set(0,.92,.15);showDice(n);diceBody.position.set(0,.92,.15);diceBody.velocity.set(0,0,0);diceBody.angularVelocity.set(0,0,0);diceBody.quaternion.set(dice.quaternion.x,dice.quaternion.y,dice.quaternion.z,dice.quaternion.w);if(done)done()}},35);return}
 let frame=0;const duration=48;const startPos=new THREE.Vector3(-1.3,2.7,-.45);const endPos=new THREE.Vector3(0,.92,.15);dice.position.copy(startPos);const interval=setInterval(()=>{frame++;const t=frame/duration;dice.position.x=startPos.x+(endPos.x-startPos.x)*(1-Math.pow(1-t,2.15));dice.position.z=startPos.z+(endPos.z-startPos.z)*(1-Math.pow(1-t,1.85));const bounce=Math.abs(Math.sin(t*Math.PI*4.5))*(1-t)*.85;dice.position.y=endPos.y+bounce;const speed=1-t;dice.rotation.x+=.72*speed+.04;dice.rotation.y+=.88*speed+.03;dice.rotation.z+=.52*speed+.02;if(frame>=duration){clearInterval(interval);dice.position.copy(endPos);showDice(n);if(done)done()}},24);
}
