...
.propertiesDock{
  ...
  gap:12px;align-items:center;padding:16px;z-index:100;box-shadow:0 0 25px #000;
  overflow-x:auto;
}
...
/* FINAL PATCH METROPOLY */
.boardWrap{overflow:visible!important}
.diceBox{
  left:50%!important;
  top:56%!important;
  transform:translate(-50%,-50%)!important;
...
.realDice{
  ...
  transform:rotateX(-22deg) rotateY(26deg)!important;
}
.diceFace{width:86px!important;height:86px!important;border-radius:17px!important}
.diceFace.front{transform:translateZ(43px)!important}
.diceFace.back{transform:rotateY(180deg) translateZ(43px)!important}
.diceFace.right{transform:rotateY(90deg) translateZ(43px)!important}
.diceFace.left{transform:rotateY(-90deg) translateZ(43px)!important}
.diceFace.top{transform:rotateX(90deg) translateZ(43px)!important}
.diceFace.bottom{transform:rotateX(-90deg) translateZ(43px)!important}
.pip{width:14px!important;height:14px!important}
...
.pip.ml{left:18px!important}.pip.mr{right:18px!important}
.realDice.rolling{animation:realDiceRollFixed .78s linear infinite!important}
@keyframes realDiceRollFixed{
  0%{transform:rotateX(0deg) rotateY(0deg) rotateZ(0deg)}
  100%{transform:rotateX(720deg) rotateY(540deg) rotateZ(360deg)}
}
.realDice.show-1{transform:rotateX(-22deg) rotateY(26deg)!important}
.realDice.show-2{transform:rotateX(68deg) rotateY(26deg)!important}
.realDice.show-3{transform:rotateX(-22deg) rotateY(206deg)!important}
.realDice.show-4{transform:rotateX(-22deg) rotateY(-64deg)!important}
.realDice.show-5{transform:rotateX(-112deg) rotateY(26deg)!important}
.realDice.show-6{transform:rotateX(68deg) rotateY(26deg)!important}
.token{width:42px!important;height:42px!important;font-size:25px!important;transition:left .24s ease,top .24s ease,transform .24s ease!important}
.token.hop{animation:pionHopOnCard .26s ease!important}
@keyframes pionHopOnCard{
  0%{transform:translateY(0) scale(1)}
  45%{transform:translateY(-15px) scale(1.14)}
  100%{transform:translateY(0) scale(1)}
}
.rollArea{bottom:-74px!important;z-index:95!important}
.propertiesDock{bottom:8px!important;height:118px!important;z-index:90!important}
.buyPanel{top:43%!important;z-index:85!important}
</style>
</head>
<body>
...
function addToken(p,i){
 const c=coords[p.position]||[89,63];
 const el=document.createElement("div");
 el.className="token";
 el.textContent=p.token;
 el.style.left=`calc(${c[0]}% - 21px + ${i*4}px)`;
 el.style.top=`calc(${c[1]}% - 21px + ${i*4}px)`;
...
socket.on("monopoly_animate_move",d=>{diceCube.classList.add("rolling");setTimeout(()=>{diceCube.classList.remove("rolling");showDice(d.dice);if(currentState){animateOneToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){
  const landed=d.state.players[myName].position;
  buyCity.textContent=cells[landed]||"Mülk";
  buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";
  buyPanel.style.display="block";
}})}else{render(d.state)}},850)});
