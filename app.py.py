...
function updateProfileChip(){
    if(currentAccount){
        profileChip.innerHTML='👤 '+currentAccount+' · 🪙 '+currentChips;
    }else{
...
function requireLogin(){
    if(!currentAccount || !currentProfile){
        alert('Oynamak için önce Üyelik bölümünden kayıt ol veya giriş yap.');
        openAuth();
        return false;
    }
    return true;
...
socket.on('connect',()=>{
    ...
    if(a){
        playerName.value=a;
        currentAccount=a;
        authStatus.innerHTML='Compte mémorisé : '+a+' (reconnecte-toi si besoin)';updateProfileChip();
    }
...
def login_account(data):
    ...
    emit('login_result', {'ok': True, 'profile': private_profile(username, users[username])})
