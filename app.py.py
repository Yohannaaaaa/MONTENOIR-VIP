...
<div class="vipCasinoMarks"><span class="m1">A♠</span><span class="m2">K♥</span><span class="m3">Q♣</span><span class="m4">J♦</span><span class="m5">♠♥♣♦</span><span class="m6">A K Q J</span></div><div id="profileChip" class="vipProfileChip" onclick="openProfile()">👤 Profil</div><div class="topLeftFixed"><button onclick="startTimer()">⏱ Süre</button><button onclick="newGame()">🎲 Yeni Oyun</button><button onclick="goLobby()">🚪 Lobi</button><button class="closeTableBtn" onclick="closeCurrentTable()">❌ Masayı Kapat</button><button id="ownerPanelBtn" style="display:none;" onclick="openOwnerPanel()">👑 Owner Panel</button><button onclick="openAuth()">👤 Üyelik</button><button onclick="openProfile()">🏆 Profil</button><button onclick="openRanking()">📊 Classement</button><button onclick="openSettings()">⚙️ Ayarlar</button><button onclick="openWords()">📚 Kelimeler</button><button onclick="openBet()">🎰 Bahis</button><button onclick="openShop()">🪙 Jeton Al</button><button id="tarotTopBtn" class="tarotTopBtn" onclick="window.location.href='/tarot'">🚪 ✦ 🔮 TAROT & RITUEL ✦</button></div>
...
function updateProfileChip(){
    if(currentAccount){
        ...
        profileChip.innerHTML='<img src="'+avatarSrc+'" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:'+imgBorder+';box-shadow:'+imgShadow+';"> <span class="'+genderCls+'">'+currentAccount+'</span>'+vip+' <span>🪙 '+currentChips+'</span>';
    }else{
