...
def mr(data):
    ...
    # COMMUNITY_CARD_INDEX_13
    if p.get("position")==13:
        monopoly_apply_community_effect(r,u)
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>METROPOLY FINAL</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
<style>
*{box-sizing:border-box}
body{margin:0;overflow:hidden;background:#050505;color:white;font-family:Arial,Helvetica,sans-serif}
.top{position:fixed;left:12px;right:12px;top:10px;height:54px;z-index:100;display:flex;gap:8px;align-items:center;padding:7px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#171717,#030303);box-shadow:0 0 22px #000}
...
#wrap{position:fixed;left:0;right:0;top:72px;bottom:0;overflow:hidden;background:radial-gradient(circle,#1b1005,#000 72%)}
#board{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(100vw,calc((100vh - 72px)*1.6));height:auto;pointer-events:none;user-select:none}
.tok{position:absolute;width:54px;height:54px;border-radius:50%;display:grid;place-items:center;transform:translate(-50%,-50%);font-size:32px;background:radial-gradient(circle at 32% 24%,#fff7,#fff0 28%),linear-gradient(145deg,#f9d36a,#b77410 55%,#3b2100);border:2px solid #ffe39a;box-shadow:0 0 18px #d4af37aa,0 12px 16px #000,inset -8px -8px 14px #0007,inset 6px 6px 12px #fff6;z-index:40}
.owner{position:absolute;width:48px;height:8px;border-radius:8px;transform:translate(-50%,-50%);box-shadow:0 0 9px currentColor;z-index:28}
.build{position:absolute;transform:translate(-50%,-50%);font-size:20px;z-index:32;white-space:nowrap;filter:drop-shadow(0 0 4px #000)}
.dice{position:absolute;left:50%;top:39%;width:88px;height:88px;transform-style:preserve-3d;transform:translate(-50%,-50%) rotateX(-22deg) rotateY(32deg);z-index:70;filter:drop-shadow(0 18px 14px #000) drop-shadow(0 0 12px #d4af3788);pointer-events:none}
.dice.rolling{animation:diceRoll 1.15s cubic-bezier(.14,.82,.31,1) both}
@keyframes diceRoll{0%{left:45%;top:32%;transform:translate(-50%,-50%) rotateX(0) rotateY(0) rotateZ(0) scale(.82)}25%{left:50%;top:40%;transform:translate(-50%,-50%) rotateX(300deg) rotateY(380deg) rotateZ(160deg) scale(1.08)}52%{left:53%;top:44%;transform:translate(-50%,-50%) rotateX(650deg) rotateY(760deg) rotateZ(380deg)}76%{left:49%;top:36%;transform:translate(-50%,-50%) rotateX(890deg) rotateY(1020deg) rotateZ(540deg) scale(.96)}100%{left:50%;top:39%;transform:translate(-50%,-50%) rotateX(1080deg) rotateY(1260deg) rotateZ(720deg) scale(1)}}
.face{position:absolute;width:88px;height:88px;border-radius:12px;background:radial-gradient(circle at 30% 20%,#fff6,#fff0 26%),linear-gradient(145deg,#ffdf77,#b97813 58%,#3a1f00);border:2px solid #ffe7a6;box-shadow:inset -10px -10px 18px #0008,inset 7px 7px 14px #fff5}
.front{transform:translateZ(44px)}.back{transform:rotateY(180deg) translateZ(44px)}.right{transform:rotateY(90deg) translateZ(44px)}.leftf{transform:rotateY(-90deg) translateZ(44px)}.topf{transform:rotateX(90deg) translateZ(44px)}.bottomf{transform:rotateX(-90deg) translateZ(44px)}
.pip{position:absolute;width:13px;height:13px;border-radius:50%;background:#050505}.tl{left:18px;top:18px}.tr{right:18px;top:18px}.bl{left:18px;bottom:18px}.br{right:18px;bottom:18px}.cc{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:18px;top:50%;transform:translateY(-50%)}.mr{right:18px;top:50%;transform:translateY(-50%)}
.show1{transform:translate(-50%,-50%) rotateX(-22deg) rotateY(32deg)}.show2{transform:translate(-50%,-50%) rotateX(68deg) rotateY(32deg)}.show3{transform:translate(-50%,-50%) rotateX(-22deg) rotateY(212deg)}.show4{transform:translate(-50%,-50%) rotateX(-22deg) rotateY(-58deg)}.show5{transform:translate(-50%,-50%) rotateX(-112deg) rotateY(32deg)}.show6{transform:translate(-50%,-50%) rotateX(68deg) rotateY(32deg)}
.roll{position:absolute;left:50%;top:47%;transform:translate(-50%,-50%);z-index:80;width:210px;height:58px;border-radius:20px;border:2px solid #ffd989;background:linear-gradient(#f7c863,#b87913 55%,#4d2700);color:#180900;font-size:26px;font-weight:900;box-shadow:0 0 26px #d4af3788,0 12px 24px #000;cursor:pointer}
.hot{position:absolute;z-index:90;border-radius:12px;cursor:pointer}.hot:hover{outline:2px solid #fff8}
.panel{position:fixed;left:50%;top:44%;transform:translate(-50%,-50%);z-index:150;width:410px;max-width:92vw;padding:22px;border-radius:22px;background:#000e;border:1px solid #d4af37;box-shadow:0 0 30px #d4af3777;text-align:center;display:none}.panel button{border:1px solid #d4af37;border-radius:12px;padding:12px 18px;margin:8px;font-weight:900;cursor:pointer;background:linear-gradient(#ffe08a,#b87913);color:#150800}.dark{background:linear-gradient(#cf4d42,#6d120b)!important;color:white!important}
.players{position:fixed;right:12px;top:74px;width:260px;max-height:40vh;z-index:95;border:1px solid #d4af37aa;border-radius:16px;background:#000c;padding:10px;overflow:auto}.player{display:flex;align-items:center;gap:8px;border:1px solid #d4af3755;border-radius:12px;margin:6px 0;padding:8px;background:#111c}.turn{border-color:#00ff66!important;box-shadow:0 0 12px #00ff6670}.dot{width:16px;height:16px;border-radius:50%;border:2px solid #fff9}.money{color:#ffd989}
.toast,.chatlog{position:fixed;left:16px;bottom:18px;z-index:160;width:320px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.show{opacity:1}textarea{width:100%;height:110px;border:1px solid #d4af37;border-radius:14px;background:#070707;color:white;padding:12px;font-size:15px}
.mode{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:155;width:410px;background:#fff;color:#444;border-radius:20px;display:none;overflow:hidden}.mh{text-align:center;padding:18px;font-size:28px;font-weight:900;border-bottom:1px solid #eee}.ms{padding:18px 24px;border-bottom:1px solid #eee}.mt{font-size:24px;margin-bottom:18px}.mc{display:flex;gap:32px;color:#aaa;font-size:20px;font-weight:900}.mc span{cursor:pointer}.mc .active{color:#1f8fe5}.next{width:calc(100% - 32px);margin:16px;border:0;border-radius:18px;background:#1f8fe5;color:white;font-weight:900;font-size:20px;padding:16px}
</style></head>
<body>
<div class="top"><a class="loca" href="/">▮ LOCA</a><input id="username" placeholder="Kullanıcı adı"><select id="tokenSelect"><option value="✈️">✈️ Avion</option><option value="🎰">🎰 Machine</option><option value="🛥️">🛥️ Bateau</option><option value="🚗">🚗 Voiture</option><option value="🎩">🎩 Chapeau</option><option value="🐈">🐈 Chat noir</option></select><button onclick="createRoom()">ODA OLUŞTUR</button><input id="roomCodeInput" placeholder="MON-1234"><button onclick="joinRoom()">ODAYA GİR</button><button onclick="startGame()">OYUNU BAŞLAT</button><button onclick="modePanel.style.display='block'">OYUN MODU</button><button onclick="openChat()">CHAT</button><span id="topRoomLabel" class="room">Oda: -</span></div>
<div id="wrap"><img id="board" src="/static/monopoly_board_lux_3d.png">
<div id="dice" class="dice show1"><div class="face front"><span class="pip cc"></span></div><div class="face back"><span class="pip tl"></span><span class="pip cc"></span><span class="pip br"></span></div><div class="face right"><span class="pip tl"></span><span class="pip tr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face leftf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip cc"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face topf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip ml"></span><span class="pip mr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face bottomf"><span class="pip tl"></span><span class="pip br"></span></div></div>
<button class="roll" onclick="rollDice()">🎲 ZAR AT</button><div id="tokens"></div><div id="owners"></div><div id="builds"></div><div id="houseBtn" class="hot" onclick="buildHouse()"></div><div id="hotelBtn" class="hot" onclick="buildHotel()"></div></div>
...
const prices=[0,0,220,200,200,200,100,100,0,160,140,100,120,0,100,100,0,60,70,100,70,80,90,0,160,160,150,180];
...
function rect(){return board.getBoundingClientRect()}function point(i,off=0){const r=rect(),p=coords[i]||coords[0];return{x:r.left+p[0]/100*r.width+off,y:r.top+p[1]/100*r.height+off}}function placeHot(){const r=rect();houseBtn.style.left=(r.left+.02*r.width)+"px";houseBtn.style.top=(r.top+.85*r.height)+"px";houseBtn.style.width=(.80*r.width)+"px";houseBtn.style.height=(.10*r.height)+"px";hotelBtn.style.left=(r.left+.82*r.width)+"px";hotelBtn.style.top=(r.top+.85*r.height)+"px";hotelBtn.style.width=(.16*r.width)+"px";hotelBtn.style.height=(.10*r.height)+"px"}window.addEventListener("resize",()=>{placeHot();if(currentState)render(currentState)});board.onload=placeHot;setTimeout(placeHot,400);
function safeId(n){return "tok_"+String(n).replace(/[^a-zA-Z0-9_-]/g,"_")}function name(){return (username.value||localStorage.getItem("montenoirUser")||"Misafir").trim()}function createRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}function joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value.trim().toUpperCase()})}function startGame(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_start_game",{code:roomCode})}function rollDice(){if(!roomCode||!myName){alert("Önce odaya gir veya oda oluştur.");return}hideBuy();socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}function buyProperty(){socket.emit("monopoly_buy_property",{code:roomCode,username:myName});hideBuy()}function hideBuy(){buyPanel.style.display="none"}function openChat(){chatPanel.style.display="block"}function buildHouse(){const c=getBuildCell();if(c==null){alert("Önce kendi mülküne düşmelisin.");return}socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:c})}function buildHotel(){const c=getBuildCell();if(c==null){alert("Önce kendi mülküne düşmelisin.");return}socket.emit("monopoly_add_hotel",{code:roomCode,username:myName,cell:c})}function getBuildCell(){if(!currentState||!myName)return null;const p=currentState.players[myName];if(!p)return null;const c=String(p.position);if((currentState.owners||{})[c]===myName)return p.position;return lastCell}
function showDice(n){dice.classList.remove("show1","show2","show3","show4","show5","show6","rolling");dice.classList.add("show"+n)}function animateDice(n,done){dice.classList.remove("show1","show2","show3","show4","show5","show6","rolling");void dice.offsetWidth;dice.classList.add("rolling");setTimeout(()=>{dice.classList.remove("rolling");dice.classList.add("show"+n);if(done)done()},1150)}
...
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayers(st);renderOwners(st);renderBuilds(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];let el=document.getElementById(safeId(n));if(!el){el=document.createElement("div");el.id=safeId(n);el.className="tok";tokens.appendChild(el)}el.textContent=p.token||"🎩";el.style.color=p.color;const pt=point(p.position,i*8);el.style.left=pt.x+"px";el.style.top=pt.y+"px"});if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st)}
...
function renderOwners(st){owners.innerHTML="";Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const m=document.createElement("div");m.className="owner";m.style.background=p.color;m.style.color=p.color;const pt=point(Number(cell));m.style.left=pt.x+"px";m.style.top=(pt.y+22)+"px";owners.appendChild(m)})}
function renderBuilds(st){builds.innerHTML="";Object.entries(st.houses||{}).forEach(([cell,n])=>{if(!n)return;const b=document.createElement("div");b.className="build";b.textContent="🏠".repeat(Number(n));const pt=point(Number(cell));b.style.left=pt.x+"px";b.style.top=(pt.y-24)+"px";builds.appendChild(b)});Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(!n)return;const b=document.createElement("div");b.className="build";b.textContent="🏨";const pt=point(Number(cell));b.style.left=pt.x+"px";b.style.top=(pt.y-24)+"px";builds.appendChild(b)})}
function moveToken(n,from,to,state,done){let el=document.getElementById(safeId(n));if(!el){render(state);done();return}let steps=[],cur=from,total=coords.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let si=0;function one(){if(si>=steps.length){done();return}const a={x:parseFloat(el.style.left),y:parseFloat(el.style.top)},b=point(steps[si],idx*8);let t0=null;function f(ts){if(!t0)t0=ts;const t=Math.min(1,(ts-t0)/240),e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;el.style.left=(a.x+(b.x-a.x)*e)+"px";el.style.top=(a.y+(b.y-a.y)*e-Math.sin(Math.PI*t)*28)+"px";if(t<1)requestAnimationFrame(f);else{el.style.left=b.x+"px";el.style.top=b.y+"px";si++;setTimeout(one,40)}}requestAnimationFrame(f)}one()}
function sendChat(){const msg=(chatInput.value||"").trim();if(!msg)return;socket.emit("monopoly_chat",{code:roomCode,username:myName||name(),message:msg});chatInput.value="";chatPanel.style.display="none"}function showChatLog(u,msg){chatLog.textContent=u+": "+msg;chatLog.classList.add("show");clearTimeout(window.chatTimer);window.chatTimer=setTimeout(()=>chatLog.classList.remove("show"),5000)}function showMoneyToast(msg,pos=true){moneyToast.textContent=msg;moneyToast.style.color=pos?"#00ff66":"#ff4d4d";moneyToast.classList.add("show");clearTimeout(window.moneyToastTimer);window.moneyToastTimer=setTimeout(()=>moneyToast.classList.remove("show"),5000)}function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const d=p.money-previousMoneyState[n];if(d>0)showMoneyToast(n+" "+d+"€ aldı",true);if(d<0)showMoneyToast(n+" "+Math.abs(d)+"€ ödedi",false)}previousMoneyState[n]=p.money})}
