...
@socketio.on("monopoly_update_settings")
def monopoly_update_settings(data):
    c=((data or {}).get("code","") or "").strip().upper()
    r=MONOPOLY_ROOMS.get(c)
    if not r:
        return
    r["settings"]={"players":int((data or {}).get("players",4) or 4),"turns":(data or {}).get("turns","illimite"),"target":int((data or {}).get("target",7000) or 7000)}
    emit("monopoly_room_state",m_public(r),room=c)
...
def monopoly_chat(data):
    ...
    if not c or not msg:
        return
    emit("monopoly_chat_message",{"username":u or "Oyuncu","message":msg},room=c)
...
@app.route("/monopoly")
def monopoly_page():
    return r"""<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>METROPOLY V2</title><script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script><script src="https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js"></script>
<style>
*{box-sizing:border-box}body{margin:0;overflow:hidden;background:radial-gradient(circle at 50% 0%,#211306,#050505 55%,#000);color:white;font-family:Arial,Helvetica,sans-serif}.loca{position:fixed;left:14px;top:12px;width:108px;height:54px;z-index:90;border:1px solid #d4af37;border-radius:16px;background:#060606;color:#ffd989;display:grid;place-items:center;text-decoration:none;font-weight:900;box-shadow:0 0 20px #000}.top{position:fixed;left:132px;right:330px;top:12px;height:54px;z-index:90;display:flex;gap:8px;align-items:center;padding:7px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#171717,#030303)}.top input,.top select,.top button{height:38px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:white;padding:8px 10px;font-weight:900}.top button{background:linear-gradient(#ffe08a,#b87913);color:#160800;cursor:pointer}#username{width:150px}#tokenSelect{width:135px}#roomCodeInput{width:122px}.roomLabel{color:#ffd989;font-weight:900;white-space:nowrap}#game3d{position:fixed;left:138px;right:318px;top:70px;bottom:110px;z-index:1}.left{position:fixed;left:14px;top:76px;bottom:120px;width:118px;z-index:65;display:flex;flex-direction:column;gap:10px}.card,.lbtn{border:1px solid #d4af37aa;border-radius:14px;background:linear-gradient(#171717,#030303);box-shadow:0 0 18px #000;color:#ffd989;text-align:center;font-weight:900;padding:9px 6px}.avatar{width:56px;height:56px;border-radius:50%;margin:auto auto 6px;display:grid;place-items:center;border:2px solid #d4af37;background:#111;font-size:32px}.lbtn{min-height:86px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;cursor:pointer}.lbtn:hover{filter:brightness(1.18)}.big{font-size:30px}.players{position:fixed;right:14px;top:76px;bottom:120px;width:300px;z-index:70;border:1px solid #d4af37aa;border-radius:18px;background:linear-gradient(#151515,#020202);box-shadow:0 0 25px #000;padding:14px;overflow:auto}.players h2{margin:0 0 12px;color:#ffd989;font-size:20px}.player{display:flex;align-items:center;gap:10px;border:1px solid #d4af3755;border-radius:14px;min-height:64px;padding:10px;margin:8px 0;background:linear-gradient(#ffffff14,#0008)}.player.turn{border-color:#00ff66;box-shadow:0 0 16px #00ff6670}.dotc{width:18px;height:18px;border-radius:50%;border:2px solid #fff9}.pawn{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;background:#0009;border:1px solid #d4af37;font-size:23px}.money{color:#ffd989}.roll{position:fixed;left:50%;bottom:76px;transform:translateX(-50%);z-index:85;width:320px;height:60px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(#f7c863,#b87913 55%,#4d2700);color:#180900;font-size:31px;font-weight:900;box-shadow:0 0 26px #d4af3788,0 12px 24px #000;cursor:pointer}.bottom{position:fixed;left:14px;right:14px;bottom:10px;height:58px;z-index:60;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#141414,#020202);display:flex;align-items:center;justify-content:center;gap:34px}.bi{color:#ffd989;text-align:center;font-size:12px;font-weight:900}.bi span{display:block;font-size:23px}.modal{position:fixed;left:50%;top:43%;transform:translate(-50%,-50%);z-index:140;width:420px;max-width:92vw;padding:22px;border-radius:22px;background:#000d;border:1px solid #d4af37;box-shadow:0 0 30px #d4af3755;text-align:center;display:none}.modal .city{font-size:27px;font-weight:900;color:#ffb000}.modal button{border:1px solid #d4af37;border-radius:12px;padding:12px 18px;margin:8px;font-weight:900;cursor:pointer;background:linear-gradient(#ffe08a,#b87913);color:#150800}.modal .dark{background:linear-gradient(#cf4d42,#6d120b);color:white}.chat textarea{width:100%;height:115px;border:1px solid #d4af37;border-radius:14px;background:#070707;color:white;padding:12px;font-size:15px}.mode{text-align:left;background:#fff;color:#444;border:none;padding:0;overflow:hidden}.mh{text-align:center;padding:18px;font-size:28px;font-weight:900;border-bottom:1px solid #eee;position:relative}.x{position:absolute;right:18px;top:10px;font-size:34px;color:#777;cursor:pointer}.ms{padding:18px 24px;border-bottom:1px solid #eee}.mt{font-size:24px;margin-bottom:18px}.mc{display:flex;gap:42px;color:#aaa;font-size:20px;font-weight:900}.mc span{cursor:pointer}.mc .active{color:#1f8fe5}.next{display:block;width:calc(100% - 32px);margin:16px!important;border:none!important;border-radius:18px!important;background:#1f8fe5!important;color:white!important;font-size:20px!important}.dice{position:fixed;left:50%;top:44%;width:98px;height:98px;transform-style:preserve-3d;transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg);z-index:90;pointer-events:none;filter:drop-shadow(0 18px 16px #000);transition:.35s}.dice.rolling{animation:diceThrow 1.18s cubic-bezier(.14,.82,.31,1) both}@keyframes diceThrow{0%{left:42%;top:30%;transform:translate(-50%,-50%) rotateX(0) rotateY(0) rotateZ(0) scale(.82)}18%{left:46%;top:36%;transform:translate(-50%,-50%) rotateX(260deg) rotateY(330deg) rotateZ(140deg) scale(1.12)}42%{left:51%;top:47%;transform:translate(-50%,-50%) rotateX(620deg) rotateY(760deg) rotateZ(350deg) scale(1)}68%{left:49%;top:41%;transform:translate(-50%,-50%) rotateX(850deg) rotateY(980deg) rotateZ(520deg) scale(.95)}100%{left:50%;top:44%;transform:translate(-50%,-50%) rotateX(1080deg) rotateY(1260deg) rotateZ(720deg) scale(1)}}.face{position:absolute;width:98px;height:98px;border-radius:18px;background:radial-gradient(circle at 28% 22%,#fff,#fff0 28%),linear-gradient(145deg,#fff,#e4e4e4 56%,#999);border:3px solid white;box-shadow:inset -12px -12px 22px #0004,inset 9px 9px 18px #ffffffe0}.front{transform:translateZ(49px)}.back{transform:rotateY(180deg) translateZ(49px)}.right{transform:rotateY(90deg) translateZ(49px)}.leftf{transform:rotateY(-90deg) translateZ(49px)}.topf{transform:rotateX(90deg) translateZ(49px)}.bottomf{transform:rotateX(-90deg) translateZ(49px)}.pip{position:absolute;width:15px;height:15px;border-radius:50%;background:#111}.tl{left:20px;top:20px}.tr{right:20px;top:20px}.bl{left:20px;bottom:20px}.br{right:20px;bottom:20px}.cc{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:20px;top:50%;transform:translateY(-50%)}.mr{right:20px;top:50%;transform:translateY(-50%)}.show1{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(28deg)}.show2{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}.show3{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(208deg)}.show4{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(-62deg)}.show5{transform:translate(-50%,-50%) rotateX(-108deg) rotateY(28deg)}.show6{transform:translate(-50%,-50%) rotateX(72deg) rotateY(28deg)}.toast{position:fixed;left:16px;bottom:78px;z-index:150;width:280px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.toast.show{opacity:1}@media(max-width:950px){body{overflow:auto}.left,.bottom{display:none}.top{left:8px;right:8px;top:56px;height:auto;flex-wrap:wrap}.players{position:relative;top:auto;right:auto;width:auto;margin:8px}.loca{top:8px}#game3d{position:relative;top:auto;left:0;right:0;height:70vw;min-height:430px;margin-top:150px}.roll{left:50%;bottom:12px;width:230px;font-size:24px}}</style></head><body>
<a class="loca" href="/">▮ LOCA</a><div class="top"><input id="username" placeholder="Kullanıcı adı"><select id="tokenSelect"><option value="✈️">✈️ Jet</option><option value="🎰">🎰 Casino</option><option value="🛥️">🛥️ Yacht</option><option value="🚗">🚗 Voiture</option><option value="🎩">🎩 Chapeau</option><option value="🐈">🐈 Chat</option></select><button onclick="createRoom()">ODA OLUŞTUR</button><input id="roomCodeInput" placeholder="MON-1234"><button onclick="joinRoom()">ODAYA GİR</button><button onclick="startGame()">OYUNU BAŞLAT</button><span id="topRoomLabel" class="roomLabel">Oda: -</span></div><div id="game3d"></div>
<div class="left"><div class="card"><div class="avatar">👤</div><div id="leftPlayerName">Oyuncu</div><div id="leftPlayerMoney" style="color:#e7be64">1000€</div></div><div class="lbtn"><span class="big">🎲</span>PİYONUN <span id="leftPawn">🎩</span></div><div class="lbtn" onclick="openChat()"><span class="big">💬</span>CHAT</div><div class="lbtn" onclick="openMode()"><span class="big">⚙️</span>OYUN MODU</div></div>
<div class="players"><h2>ONLINE OYUNCULAR / RENKLER</h2><div id="playersList"></div></div><div id="diceOverlay" class="dice show1"><div class="face front"><span class="pip cc"></span></div><div class="face back"><span class="pip tl"></span><span class="pip cc"></span><span class="pip br"></span></div><div class="face right"><span class="pip tl"></span><span class="pip tr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face leftf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip cc"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face topf"><span class="pip tl"></span><span class="pip tr"></span><span class="pip ml"></span><span class="pip mr"></span><span class="pip bl"></span><span class="pip br"></span></div><div class="face bottomf"><span class="pip tl"></span><span class="pip br"></span></div></div><button class="roll" onclick="rollDice()">🎲 ZAR AT</button>
<div class="bottom"><div class="bi"><span>✈️</span>AÉROPORT</div><div class="bi"><span>🛥️</span>PORT</div><div class="bi"><span>🚐</span>GARE</div><div class="bi"><span>⚡</span>ÉLECTRICITÉ</div><div class="bi"><span>💧</span>EAUX</div><div class="bi"><span>🏠</span>1 EV</div><div class="bi"><span>🏘️</span>4 EV</div><div class="bi"><span>🏨</span>OTEL</div></div>
<div id="buyPanel" class="modal"><div class="city" id="buyCity">PARIS</div><div style="font-size:21px;font-weight:900;margin:12px 0">satın almak ister misin?</div><div id="buyPrice">Fiyat: 160 €</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">VAZGEÇ</button></div><div id="chatPanel" class="modal chat"><h3>CHAT</h3><textarea id="chatInput" placeholder="Mesaj yaz..."></textarea><button onclick="sendChat()">GÖNDER</button><button onclick="chatPanel.style.display='none'">KAPAT</button></div><div id="modePanel" class="modal mode"><div class="mh">PARAMÈTRES <span class="x" onclick="modePanel.style.display='none'">×</span></div><div class="ms"><div class="mt">Mode de jeu</div><div class="mc"><span class="active">EN DIRECT</span></div></div><div class="ms"><div class="mt">Joueurs</div><div class="mc" id="modePlayers"><span>2</span><span>3</span><span class="active">4</span></div></div><div class="ms"><div class="mt">Tours</div><div class="mc" id="modeTurns"><span>30</span><span>50</span><span class="active">ILLIMITÉ</span></div></div><div class="ms"><div class="mt">Objectif de patrimoine</div><div class="mc" id="modeTarget"><span>3000 $</span><span>5000 $</span><span class="active">7000 $</span></div></div><button class="next" onclick="saveMode()">SUIVANT</button></div><div id="moneyToast" class="toast"></div><div id="chatLog" class="chatLog"></div><div id="modeSaved" class="modeSaved"></div>
<script>const socket=io();let roomCode="",myName="",currentState=null,previousMoneyState={};const cells=["ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Milan","Amsterdam","Rome","Caisse de Communauté","Moscou","Madrid","Impôts","Sofia","Belgrad","Gare","Istanbul","Prague","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo"];const prices=[0,0,220,200,200,200,100,100,0,160,140,140,100,0,120,100,0,60,70,100,70,80,90,0,160,160,150,180];const coords2d=[[87,63.5],[85.5,78],[75,78],[64,78],[53.6,78],[42.9,78],[33,78],[23.8,78],[16.8,77.4],[17.3,65.7],[17.3,55.4],[17.3,45.2],[17.3,36.4],[17.3,30.8],[17.3,24.7],[17.3,14.8],[21.8,7.9],[31.8,7.9],[41.4,7.9],[50.7,7.9],[59.5,7.9],[68.7,7.9],[77.8,7.9],[86.8,7.9],[86.8,19.8],[86.8,31.2],[86.8,43],[86.8,54.7]];let scene,camera,renderer,board,players3d={},owners3d={};const boardW=15.4,boardH=9.6;init3D();function init3D(){const c=game3d;scene=new THREE.Scene();camera=new THREE.PerspectiveCamera(18,c.clientWidth/c.clientHeight,.1,100);camera.position.set(0,22,1.15);camera.lookAt(0,0,0);renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.4));renderer.setSize(c.clientWidth,c.clientHeight);c.appendChild(renderer.domElement);scene.add(new THREE.AmbientLight(0xffffff,.98));const l=new THREE.DirectionalLight(0xffe1a0,1.7);l.position.set(0,9,5);scene.add(l);const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png",()=>renderer.render(scene,camera));tex.colorSpace=THREE.SRGBColorSpace;board=new THREE.Mesh(new THREE.BoxGeometry(boardW,.12,boardH),new THREE.MeshStandardMaterial({map:tex,roughness:.48,metalness:.04}));scene.add(board);window.addEventListener("resize",resize3D);animate()}function resize3D(){const c=game3d;camera.aspect=c.clientWidth/c.clientHeight;camera.updateProjectionMatrix();renderer.setSize(c.clientWidth,c.clientHeight)}function animate(){requestAnimationFrame(animate);renderer.render(scene,camera)}function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.38,(p[1]/100-.5)*boardH+off)}function tokenMesh(icon,color){const g=new THREE.Group();const s=new THREE.Mesh(new THREE.SphereGeometry(.15,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.7,roughness:.23}));g.add(s);const canvas=document.createElement("canvas");canvas.width=128;canvas.height=128;const ctx=canvas.getContext("2d");ctx.font="72px Arial";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(icon||"🎩",64,68);const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(canvas),transparent:true}));sp.position.y=.32;sp.scale.set(.38,.38,.38);g.add(sp);return g}function showDice(n){diceOverlay.classList.remove("show1","show2","show3","show4","show5","show6","rolling");diceOverlay.classList.add("show"+n)}function animateDice(n,done){diceOverlay.classList.remove("show1","show2","show3","show4","show5","show6","rolling");void diceOverlay.offsetWidth;diceOverlay.classList.add("rolling");setTimeout(()=>{diceOverlay.classList.remove("rolling");diceOverlay.classList.add("show"+n);if(done)done()},1180)}function name(){return (username.value||localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Misafir").trim()}function createRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}function joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value.trim().toUpperCase()})}function startGame(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_start_game",{code:roomCode})}function rollDice(){if(!roomCode||!myName){alert("Önce odaya gir veya oda oluştur.");return}hideBuy();socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}function buyProperty(){socket.emit("monopoly_buy_property",{code:roomCode,username:myName});hideBuy()}function hideBuy(){buyPanel.style.display="none"}function openChat(){chatPanel.style.display="block"}function openMode(){modePanel.style.display="block"}function activeText(id){const el=document.querySelector("#"+id+" .active");return el?el.textContent.trim():""}function saveMode(){const p=Number(activeText("modePlayers")||4);const tr=activeText("modeTurns")||"ILLIMITÉ";const tar=activeText("modeTarget")||"7000 $";const turns=tr.includes("30")?30:(tr.includes("50")?50:"illimite");const target=Number(tar.replace(/[^0-9]/g,"")||7000);if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_update_settings",{code:roomCode,players:p,turns:turns,target:target});modePanel.style.display="none";if(window.modeSaved){modeSaved.textContent="Mode: "+p+" joueurs / "+(turns==="illimite"?"illimité":turns+" tours")+" / "+target+"$";modeSaved.style.display="block";setTimeout(()=>modeSaved.style.display="none",3500)}}document.querySelectorAll(".mc span").forEach(s=>s.onclick=()=>{[...s.parentElement.children].forEach(x=>x.classList.remove("active"));s.classList.add("active")});function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayersPanel(st);renderOwners(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];if(!players3d[n]){players3d[n]=tokenMesh(p.token,p.color);scene.add(players3d[n])}players3d[n].position.copy(pos3D(p.position,i*.022))});if(st.players&&st.players[myName]){const lp=st.players[myName];leftPlayerName.textContent=myName;leftPlayerMoney.textContent=lp.money+"€";leftPawn.textContent=lp.token||tokenSelect.value}if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st)}function renderPlayersPanel(st){playersList.innerHTML="";Object.keys(st.players||{}).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];const d=document.createElement("div");d.className="player"+(i===st.turnIndex?" turn":"");d.innerHTML=`<div class="dotc" style="background:${p.color};color:${p.color}"></div><div class="pawn">${p.token||"🎩"}</div><div><div style="font-weight:900">${n}</div><div class="money">${p.money}€</div></div>`;playersList.appendChild(d)})}function renderOwners(st){Object.values(owners3d).forEach(o=>scene.remove(o));owners3d={};Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const m=new THREE.Mesh(new THREE.BoxGeometry(.68,.05,.14),new THREE.MeshStandardMaterial({color:p.color||0xd4af37}));m.position.copy(pos3D(+cell));m.position.y=.43;m.position.z+=.30;scene.add(m);owners3d[cell]=m})}function moveToken(n,from,to,state,done){const mesh=players3d[n];if(!mesh){done();return}let steps=[],cur=from,total=coords2d.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let si=0;function one(){if(si>=steps.length){done();return}const a=mesh.position.clone(),b=pos3D(steps[si],idx*.022);let t0=null;function f(ts){if(!t0)t0=ts;const t=Math.min(1,(ts-t0)/250);const e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;mesh.position.x=a.x+(b.x-a.x)*e;mesh.position.z=a.z+(b.z-a.z)*e;mesh.position.y=.38+Math.sin(Math.PI*t)*.30;if(t<1)requestAnimationFrame(f);else{mesh.position.copy(b);mesh.position.y=.38;si++;setTimeout(one,35)}}requestAnimationFrame(f)}one()}function showMoneyToast(msg,pos=true){moneyToast.textContent=msg;moneyToast.style.color=pos?"#00ff66":"#ff4d4d";moneyToast.classList.add("show");clearTimeout(window.moneyToastTimer);window.moneyToastTimer=setTimeout(()=>moneyToast.classList.remove("show"),5000)}function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const d=p.money-previousMoneyState[n];if(d>0)showMoneyToast(n+" "+d+"€ aldı",true);if(d<0)showMoneyToast(n+" "+Math.abs(d)+"€ ödedi",false)}previousMoneyState[n]=p.money})}function sendChat(){const msg=(chatInput.value||"").trim();if(!msg)return;if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("monopoly_chat",{code:roomCode,username:myName||name(),message:msg});chatInput.value="";chatPanel.style.display="none"}function showChatLog(u,msg){if(!window.chatLog)return;chatLog.textContent=u+": "+msg;chatLog.classList.add("show");clearTimeout(window.chatLogTimer);window.chatLogTimer=setTimeout(()=>chatLog.classList.remove("show"),5000)}socket.on("monopoly_chat_message",d=>showChatLog(d.username,d.message));socket.on("monopoly_room_state",render);socket.on("monopoly_error",d=>alert(d.msg));socket.on("monopoly_animate_move",d=>{animateDice(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=Number(d.state.players[myName].position);buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";setTimeout(()=>buyPanel.style.display="block",160)}})}else render(d.state)})});username.value=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"";</script></body></html>"""
