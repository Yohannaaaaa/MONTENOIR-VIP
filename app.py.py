...
<div class="home">
  ...
  <div class="bottomCards">
    <div class="card"><div class="cardIcon">🎁</div><div class="cardText"><span data-i18n="daily">GÜNLÜK ÖDÜL</span><small><span data-i18n="dailyText">Hediyeni almayı unutma!</span></small></div><button>AL</button></div>
...
function spinDailyWheel(){
  ...
  .then(r=>r.json()).then(d=>{
    ...
    const map={30:30,50:90,100:150,300:210,1000:270,2000:330};
    const deg=1440+(map[prize]||30);
...
function uploadAvatar(){
  const u=currentMontenoirUser();
  if(!u){avatarStatus.innerHTML="Önce giriş yap.";return;}
  if(!avatarFile.files[0]){avatarStatus.innerHTML="Dosya seç.";return;}
  const fd=new FormData();fd.append("username",u);fd.append("avatar",avatarFile.files[0]);
  fetch("/api/profile/avatar",{method:"POST",body:fd}).then(r=>r.json()).then(d=>{
    avatarStatus.innerHTML=d.msg||JSON.stringify(d);
    if(d.ok&&d.avatar){
      let p=localStorage.getItem("montenoirProfile");
      if(p){try{p=JSON.parse(p);p.avatar=d.avatar;p.avatarData=d.avatar;localStorage.setItem("montenoirProfile",JSON.stringify(p));localStorage.setItem("codenamesProfile",JSON.stringify(p));}catch(e){}}
    }
  }).catch(()=>avatarStatus.innerHTML="Yükleme hatası.");
}
</script>
...
def api_daily_reward():
    ...
    if not key:
        return {"ok": False, "msg": "Kullanıcı bulunamadı."}
    # weighted prizes: high prizes rare
    prizes = [30, 50, 100, 300, 1000, 2000]
    weights = [42, 30, 17, 8, 2, 1]
...
@app.route("/premium")
def premium_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>💎 Premium Üyelik</h1><p>Premium üyelik sistemi yakında aktif olacak.</p></body></html>"""

@app.route("/kasa")
def kasa_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>🪙 MONTENOIR Kasası</h1><p>Jeton paketleri ve ödeme sistemi burada olacak.</p></body></html>"""

@app.route("/turnuvalar")
def turnuvalar_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>🏆 Turnuvalar</h1><p>Haftalık Codenames Turnuvası — Giriş: 100 jeton — Ödül: 5000 jeton.</p></body></html>"""

@app.route("/arkadaslar")
def arkadaslar_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>👥 Arkadaşlar</h1><p>Arkadaş ekleme, özel mesaj ve çevrimiçi durumu yakında.</p></body></html>"""

@app.route("/oyun-kurallari")
def oyun_kurallari_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>📜 Oyun Kuralları</h1><p>Saygılı oyun, hile yasak, uygunsuz davranış yasak.</p></body></html>"""

@app.route("/sandiklar")
def sandiklar_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>🎁 Sandık Sistemi</h1><p>Bronz, Gümüş, Altın ve Elmas sandıklar yakında.</p></body></html>"""

@app.route("/profil-magazasi")
def profil_magazasi_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>🎨 Profil Özelleştirme</h1><p>Altın çerçeve, elmas çerçeve, barok çerçeve, isim renkleri ve animasyonlu profil yakında.</p></body></html>"""

@app.route("/ai-tarot-premium")
def ai_tarot_premium_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none}</style></head><body><a class='locaBtn' href='/'>← LOCA</a><h1>🤖 AI Tarot Premium</h1><p>Doğum tarihi, soru ve fotoğraf ile kişiselleştirilmiş yorum yakında.</p></body></html>"""

if __name__ == "__main__":
    import os, uuid, uuid, sys, traceback
    try:
