...
<style>
body{margin:0;min-height:100vh;font-family:Arial,sans-serif;color:#d4af37;background:radial-gradient(circle at top,#2a1a05 0%,#050505 48%,#000 100%);overflow-x:hidden;}
...
h1{font-size:56px;text-shadow:0 0 18px #d4af37,0 0 50px #8a6a00;margin:25px 0 5px}.sub{color:#f4df94;letter-spacing:3px}.topbar{display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin:20px 0}
...
.panel h2{text-align:center;color:#ffd978}.service{width:100%;margin:6px 0;text-align:left}input,textarea,select{width:95%;padding:12px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:#f8e7a0;margin:6px 0;}textarea{height:90px}.resultBox{white-space:pre-wrap;color:#f8e7a0;line-height:1.5}.adminItem{border:1px solid #d4af37;border-radius:14px;padding:12px;margin:8px 0;background:rgba(255,255,255,.04)}.candle{font-size:40px;animation:candle 1.4s ease-in-out infinite alternate}@keyframes candle{from{filter:drop-shadow(0 0 4px #d4af37)}to{filter:drop-shadow(0 0 20px #ffcc55)}}
</style></head>
<body><div class="tarotCardsBg"></div><div class="stars"></div><div class="fog"></div>
<div class="wrap">
<h1>🔮 TAROT & RITUEL</h1><div class="sub">Siyah kadife · Altın sır · Mum ışığı</div><div class="candle">🕯️ 🕯️ 🕯️</div>
<div class="topbar"><button class="btn" onclick="location.href='/'">♠ Codenames'e Dön</button><button class="btn" onclick="loadProfile()">👤 Profili Yenile</button><button class="btn" onclick="spinWheel()">🎡 Şans Çarkı</button><button class="btn" onclick="loadAdminRequests()">👑 Talep Paneli</button></div>
<div class="panel" style="text-align:center"><b>Kullanıcı:</b> <span id="userName">-</span> · <b>Jeton:</b> <span id="chips">-</span> 🪙</div>
...
<div class="panel"><h2>🪙 Paketler</h2><div>200 jeton · 4,99 £</div><div>500 jeton · 9,99 £</div><div>1200 jeton · 19,99 £</div><div>3000 jeton · 39,99 £</div><div>8000 jeton · 89,99 £</div><p style="font-size:12px;color:#f4df94">Stripe / PayPal / Google Pay entegrasyonu sonraki aşamada bağlanacak.</p></div></div>
...
<div class="panel"><h2>👑 Admin Talep Paneli</h2><div id="adminRequests">Admin paneli için Yohanna / admin hesabıyla giriş yapılmalı.</div></div>
</div>
<script>
const socket=io();let account=localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||'';let selectedService='';
function loadProfile(){account=localStorage.getItem('codenamesAccount')||localStorage.getItem('loggedUser')||account||'';if(!account){alert('Önce Codenames tarafında giriş yap.');return}socket.emit('tarot_get_profile',{account});}
...
socket.on('tarot_profile',d=>{if(!d.ok){alert(d.msg);return}userName.innerHTML=d.profile.username;chips.innerHTML=d.profile.chips;});
socket.on('tarot_result',d=>{if(!d.ok){alert(d.msg);return}chips.innerHTML=d.profile.chips;resultBox.innerHTML=d.msg+"\\n\\nTalep #"+d.request.id+"\\nDurum: "+d.request.status+"\\n\\n"+(d.request.result||"Bakımcı yorumu bekleniyor.");pdfBox.innerHTML='<br><button class="btn" onclick="window.open(\\'/tarot_pdf/'+d.request.id+'\\',\\'_blank\\')">📄 PDF Ritüelini Al</button>';});
socket.on('tarot_wheel_result',d=>{if(!d.ok){alert(d.msg);return}chips.innerHTML=d.profile.chips;alert('Şans çarkı: +'+d.reward+' jeton');});
...
<div class="vipCasinoMarks"><span class="m1">A♠</span><span class="m2">K♥</span><span class="m3">Q♣</span><span class="m4">J♦</span><span class="m5">♠♥♣♦</span><span class="m6">A K Q J</span></div><div id="profileChip" class="vipProfileChip" onclick="openProfile()">👤 Profil</div><div class="topLeftFixed"><button onclick="startTimer()">⏱ Süre</button><button onclick="newGame()">🎲 Yeni Oyun</button><button onclick="goLobby()">🚪 Lobi</button><button class="closeTableBtn" onclick="closeCurrentTable()">❌ Masayı Kapat</button><button onclick="openAuth()">👤 Üyelik</button><button onclick="openProfile()">🏆 Profil</button><button onclick="openRanking()">📊 Classement</button><button onclick="openSettings()">⚙️ Ayarlar</button><button onclick="openWords()">📚 Kelimeler</button><button onclick="openBet()">🎰 Bahis</button><button onclick="openShop()">🪙 Jeton Al</button><button class="tarotBtnMain" onclick="window.location.href='/tarot'">🔮 TAROT & RITUEL</button></div>
...
<div class="tarotPortalPanel" onclick="window.location.href='/tarot'">
  <div class="tarotPortalInner">
    <div class="tarotMiniCard">☾</div>
    <div>
      <div class="tarotPortalTitle">🔮 TAROT & RITUEL</div>
      <div class="tarotPortalSub">Le voile s’ouvre · Tarot · Rituels · Niyet · Jeton</div>
      <div class="tarotCandleLine">🕯️ ✦ 🕯️ ✦ 🕯️</div>
      <div class="tarotPortalBtn">Portalı Aç</div>
    </div>
    <div class="tarotMiniCard">✦</div>
  </div>
</div>
