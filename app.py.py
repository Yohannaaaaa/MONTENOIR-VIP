...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Tarot & Ritüel - MONTENOIR VIP</title>
<style>
*{box-sizing:border-box}
html,body{
  margin:0;
...
fetch('/api/owner/requests?key='+encodeURIComponent(key)).then(r=>r.json()).then(d=>{
 ...
 box.innerHTML=d.requests.map(x=>{
  ...
  <div class=row><div class=l>Soru</div><div>${(x.question||'').replace(/</g,'&lt;')}</div></div>
...
@app.route("/tarot")
def tarot_page():
    return """<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Tarot & Ritüel</title><style>
*{box-sizing:border-box}body{margin:0;background:#050505;color:#fff;font-family:Arial}.page{min-height:100vh;padding:24px;background:linear-gradient(180deg,rgba(0,0,0,.08),rgba(0,0,0,.72)),url('/static/tarot_home.png') center top/cover no-repeat fixed}.back{position:fixed;top:16px;left:16px;color:#d4af37;text-decoration:none;border:1px solid #d4af37;border-radius:12px;background:rgba(0,0,0,.62);padding:10px 14px;font-weight:bold}.wrap{max-width:1120px;margin:80px auto 40px;display:grid;gap:18px}.panel{border:1px solid rgba(212,175,55,.55);border-radius:16px;background:rgba(0,0,0,.68);padding:18px;box-shadow:0 0 22px rgba(0,0,0,.55)}h2{color:#d4af37;margin-top:0}table{width:100%;border-collapse:collapse}td,th{padding:10px;border-bottom:1px solid rgba(212,175,55,.22);text-align:left}button{border:1px solid #d4af37;border-radius:10px;background:linear-gradient(180deg,#d4af37,#7a4d09);color:white;padding:9px 12px;font-weight:900;cursor:pointer}.formGrid{display:grid;grid-template-columns:1fr 1fr;gap:10px}input,textarea{width:100%;padding:11px;border-radius:10px;border:1px solid #d4af37;background:#080808;color:white}textarea{min-height:100px;grid-column:1/-1}.full{grid-column:1/-1}.status{color:#00ff66;margin-top:10px}@media(max-width:760px){.formGrid{grid-template-columns:1fr}.wrap{margin-top:70px}}</style></head><body><div class='page'><a class='back' href='/'>← Ana Sayfa</a><div class='wrap'>
<div class='panel'><h2>🔮 Tarot Bakımları Hizmet</h2><table><tr><th>Hizmet</th><th>Süre</th><th>Fiyat</th><th></th></tr>
<tr><td>Tek Soru Bakımı</td><td>5 dk</td><td>🪙 300 jeton</td><td><button onclick="fillService('Tarot Bakımı','Tek Soru Bakımı','5 dk','300 jeton')">Talep / Jeton Yükle</button></td></tr>
<tr><td>3 Soru Bakımı</td><td>10 dk</td><td>🪙 700 jeton</td><td><button onclick="fillService('Tarot Bakımı','3 Soru Bakımı','10 dk','700 jeton')">Talep / Jeton Yükle</button></td></tr>
<tr><td>Aşk Açılımı</td><td>20 dk</td><td>🪙 1000 jeton</td><td><button onclick="fillService('Tarot Bakımı','Aşk Açılımı','20 dk','1000 jeton')">Talep / Jeton Yükle</button></td></tr>
<tr><td>Genel Bakım</td><td>30 dk</td><td>🪙 1500 jeton</td><td><button onclick="fillService('Tarot Bakımı','Genel Bakım','30 dk','1500 jeton')">Talep / Jeton Yükle</button></td></tr></table></div>
<div class='panel'><h2>✨ Ritüeller Ritüel Fiyat</h2><table><tr><th>Ritüel</th><th>Fiyat</th><th></th></tr>
...
<tr><td>🌙 Kişisel Niyet Ritüeli</td><td>🪙 1500 jeton</td><td><button onclick="fillService('Ritüel','Kişisel Niyet Ritüeli','','1500 jeton')">Talep / Jeton Yükle</button></td></tr></table></div>
<div class='panel'><h2>🪙 Jeton Paketleri Paket Fiyat</h2><table><tr><td>200 jeton</td><td>£4.99</td></tr><tr><td>500 jeton</td><td>£9.99</td></tr><tr><td>1200 jeton</td><td>£19.99</td></tr><tr><td>3000 jeton</td><td>£39.99</td></tr><tr><td>8000 jeton</td><td>£89.99</td></tr></table></div>
<div class='panel'><h2>🤖 Anında Tarot Yorumu</h2><p>Rastgele Kart Açılımı — 🪙 100 jeton</p><button onclick='instantTarot()'>🤖 AI yorum al</button><div id='aiResult' class='status'></div></div>
<div class='panel'><h2>✍️ Sorunu Yaz</h2><form id='tarotForm' enctype='multipart/form-data'><input type='hidden' name='category' id='category' value='Serbest Soru'><input type='hidden' name='service' id='service' value='Sorunu Yaz'><input type='hidden' name='price' id='price'><input type='hidden' name='duration' id='duration'><div class='formGrid'><input name='name' placeholder='İsim' required><input name='motherName' placeholder='Anne adı'><input name='birthDate' type='date'><input name='email' type='email' placeholder='E-posta' required><textarea name='question' placeholder='Sorunu yaz' required></textarea><input class='full' name='file' type='file' accept='.pdf,.jpg,.jpeg,.png'><button class='full'>🪙 Jeton Yükle / Talep Gönder</button></div><div id='formStatus' class='status'></div></form></div>
</div></div><script>
function fillService(c,s,d,p){category.value=c;service.value=s;duration.value=d;price.value=p;tarotForm.scrollIntoView({behavior:'smooth'});formStatus.textContent='Seçilen hizmet: '+c+' / '+s+' / '+p}
function instantTarot(){const cards=['Aşıklar','Ay','Güneş','Kule','Yıldız','İmparatoriçe'];const c=cards[Math.floor(Math.random()*cards.length)];aiResult.textContent='Seçilen kart: '+c+' — Kısa yorum: Enerjin dönüşüm ve sezgi alanında yoğunlaşıyor.'}
tarotForm.addEventListener('submit',e=>{e.preventDefault();const fd=new FormData(tarotForm);formStatus.textContent='Gönderiliyor...';fetch('/api/tarot/request',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{formStatus.textContent=d.msg||JSON.stringify(d);if(d.ok)tarotForm.reset()}).catch(()=>formStatus.textContent='Gönderim hatası.')})
</script></body></html>"""
