...
MONOPOLY_ROOMS={}
MONOPOLY_CELLS=[("DÉPART","start",0,0,"corner"),("Sofia","property",60,10,"yellow"),("Belgrad","property",70,12,"yellow"),("Gare Grande Vitesse","transport",100,25,"transport"),("Istanbul","property",70,12,"yellow"),("Prague","property",80,14,"blue"),("Budapest","property",80,14,"blue"),("Lisbonne","property",90,16,"blue"),("Enchères","bonus",0,0,"special"),("Dubaï","property",160,32,"red"),("Pékin","property",160,32,"red"),("Compagnie Électrique","utility",150,20,"utility"),("Tokyo","property",180,36,"red"),("ALLEZ","start",0,0,"corner"),("Banque","bank",0,0,"corner"),("New York","property",220,44,"green"),("Monaco","property",200,40,"green"),("Aéroport International","transport",200,30,"transport"),("Londres","property",200,40,"green"),("Varsovie","property",100,18,"brown"),("Zurich","property",100,18,"brown"),("Prison","jail",0,0,"corner"),("Paris","property",160,30,"orange"),("Milan","property",140,26,"orange"),("Compagnies des Eaux","utility",100,20,"utility"),("Amsterdam","property",140,26,"orange"),("Moscou","property",120,22,"purple"),("Rome","property",100,18,"purple"),("Madrid","property",100,18,"purple"),("Impôts","tax",0,200,"tax")]
MONOPOLY_GROUPS={}
for i,c in enumerate(MONOPOLY_CELLS):
    if c[1]=="property": MONOPOLY_GROUPS.setdefault(c[4],[]).append(i)
def m_room(code,owner): return {"code":code,"owner":owner,"started":False,"turnIndex":0,"players":{},"owners":{},"houses":{},"lastLog":"Oda hazır.","lastDice":None}
def m_public(r): return {"code":r["code"],"started":r["started"],"turnIndex":r["turnIndex"],"players":r["players"],"owners":r["owners"],"houses":r["houses"],"lastLog":r["lastLog"],"lastDice":r["lastDice"]}
def m_code():
    while True:
        c="MON-"+"".join(random.choices(string.digits,k=4))
        if c not in MONOPOLY_ROOMS: return c
...
*{box-sizing:border-box}body{margin:0;background:#050301;color:white;font-family:Arial;overflow:hidden}.screen{height:100vh;display:grid;grid-template-columns:minmax(0,1fr) 270px;gap:12px;padding:10px}.main{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden}.boardWrap{position:relative;width:min(1260px,calc(100vw - 305px));aspect-ratio:16/10;filter:drop-shadow(0 30px 42px #000);transform:translateX(-6px)}.boardImg{width:100%;height:100%;object-fit:contain;object-position:center center;display:block}.token{position:absolute;width:42px;height:42px;border-radius:50%;display:grid;place-items:center;font-size:26px;background:radial-gradient(circle,#fff7,#0008);border:2px solid #d4af37;box-shadow:0 0 18px #00ff66;transition:left .34s,top .34s;z-index:5}.house{position:absolute;font-size:20px;z-index:4}.diceBox{position:absolute;left:50%;top:51%;transform:translate(-50%,-50%);z-index:20;perspective:700px}.diceCube{width:92px;height:92px;position:relative;transform-style:preserve-3d;transform:rotateX(-18deg) rotateY(25deg);transition:transform .45s ease;filter:drop-shadow(0 22px 18px rgba(0,0,0,.8))}.face{position:absolute;width:92px;height:92px;border-radius:18px;background:linear-gradient(145deg,#fff,#dfdfdf 55%,#8f8f8f);border:3px solid #fff;color:#1f1200;display:grid;place-items:center;font-size:55px;font-weight:900;box-shadow:inset -10px -10px 18px rgba(0,0,0,.22),inset 8px 8px 14px rgba(255,255,255,.75)}.front{transform:translateZ(46px)}.back{transform:rotateY(180deg) translateZ(46px)}.right{transform:rotateY(90deg) translateZ(46px)}.left{transform:rotateY(-90deg) translateZ(46px)}.top{transform:rotateX(90deg) translateZ(46px)}.bottom{transform:rotateX(-90deg) translateZ(46px)}.roll{animation:rollCube .85s linear infinite}@keyframes rollCube{0%{transform:rotateX(0deg) rotateY(0deg) rotateZ(0deg)}100%{transform:rotateX(360deg) rotateY(540deg) rotateZ(360deg)}}.side{background:#000b;border:1px solid #d4af37;border-radius:18px;padding:14px;overflow:auto}h2{color:#d4af37;margin:8px 0}input,select,button{width:100%;padding:11px;border-radius:12px;border:1px solid #d4af37;margin:6px 0}input,select{background:#050505;color:white}button{background:linear-gradient(#f5d47b,#8c5a0d);font-weight:900;cursor:pointer}button.dark{background:linear-gradient(#171717,#050505);color:white}.player{display:flex;gap:8px;border:1px solid #d4af3755;border-radius:12px;padding:9px;margin:7px 0}.turn{box-shadow:0 0 16px #00ff66;border-color:#00ff66}.picon{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;border:1px solid #d4af37}.log{font-family:Consolas;color:#00ff66;background:#020202;border:1px solid #0f63;border-radius:12px;padding:9px;height:120px;overflow:auto;font-size:12px}.loca{position:absolute;left:18px;top:18px;color:#d4af37;text-decoration:none;border:1px solid #d4af37;border-radius:12px;background:#000c;padding:10px 14px;font-weight:900;z-index:20}.buyPanel{position:absolute;left:50%;bottom:32px;transform:translateX(-50%);background:#000d;border:1px solid #d4af37;border-radius:18px;padding:12px;display:none;z-index:10}.houseBtn{width:auto;padding:8px 10px}
...
function renderHouses(st){housesPanel.innerHTML="";Object.entries(st.owners||{}).forEach(([cell,owner])=>{if(owner===myName){let b=document.createElement("button");b.className="houseBtn";b.textContent="🏠 "+cells[+cell];b.onclick=()=>socket.emit("monopoly_add_house",{code:roomCode,username:myName,cell:+cell});housesPanel.appendChild(b)}})}
socket.on("monopoly_room_state",render);socket.on("monopoly_error",d=>alert(d.msg));socket.on("monopoly_animate_move",d=>{diceCube.classList.add("roll");setTimeout(()=>{diceCube.classList.remove("roll");diceFace.textContent=faces[d.dice-1];diceCube.style.transform=rotations[d.dice]||rotations[1];render(d.state);if(d.pendingBuy&&d.state.players[myName]?.position===d.to)buyPanel.style.display="block"},900)});username.value=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"";
</script></body></html>"""
