...
const coords2d=[[88.6,64.0],[86.8,78.7],[77.9,79.3],[66.9,79.3],[56.1,79.3],[45.3,79.3],[34.7,79.3],[24.5,79.3],[12.4,78.6],[11,66.2],[11,56],[11,45.8],[11,35.5],[11,26.4],[11,19],[11,11.8],[16.2,7.6],[28.4,7.6],[39.4,7.6],[50.2,7.6],[59.9,7.6],[70.1,7.6],[80.2,7.6],[90,7.6],[90,20],[90,31.5],[90,43.4],[90,55.2]];
...
function init3D(){
 ...
 camera=new THREE.PerspectiveCamera(34,c.clientWidth/c.clientHeight,.1,100);
 camera.position.set(0,11.8,6.7);
 ...
 dice=createDice(); dice.position.set(0,.72,.28); scene.add(dice);
...
function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.36,(p[1]/100-.5)*boardH+off)}
...
function rollDiceAnim(n,done){
 let frame=0; const duration=46; const startPos=new THREE.Vector3(-1.45,1.9,-0.45); const endPos=new THREE.Vector3(0,.72,.28); dice.position.copy(startPos);
 const interval=setInterval(()=>{frame++; const t=frame/duration; dice.position.x=startPos.x+(endPos.x-startPos.x)*(1-Math.pow(1-t,2.4)); dice.position.z=startPos.z+(endPos.z-startPos.z)*(1-Math.pow(1-t,1.8)); const bounce=Math.abs(Math.sin(t*Math.PI*4.2))*(1-t)*0.72; dice.position.y=endPos.y+bounce; const speed=(1-t); dice.rotation.x+=.62*speed+.045; dice.rotation.y+=.78*speed+.035; dice.rotation.z+=.44*speed+.025; if(frame>=duration){clearInterval(interval); dice.position.copy(endPos); showDice(n); if(done)done();}},24);
}
...
function createRoom(){myName=name();socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}
function joinRoom(){myName=name();socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value})}
...
function moveToken(n,from,to,state,done){const mesh=players3d[n];if(!mesh){done();return}let steps=[],cur=from,total=coords2d.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let k=0;const id=setInterval(()=>{if(k>=steps.length){clearInterval(id);done();return}mesh.position.copy(pos3D(steps[k],idx*.08));mesh.position.y=.68;setTimeout(()=>mesh.position.y=.36,105);k++},220)}
...
socket.on("monopoly_animate_move",d=>{rollDiceAnim(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=d.state.players[myName].position;buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";buyPanel.style.display="block"}})}else render(d.state)})});
