...
body{
  margin:0;
  min-height:100vh;
  background:radial-gradient(circle at top,#2b1b04,#050505 52%,#000);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  overflow-x:hidden;
}
...
h1{text-align:center;color:#d4af37;text-shadow:0 0 20px rgba(212,175,55,.5);margin:6px 0 14px}
.game{
  display:grid;
...
.board3d{
  ...
  border:4px solid #d4af37;
  border-radius:30px;
  box-shadow:0 28px 54px rgba(0,0,0,.75),0 0 35px rgba(212,175,55,.35),inset 0 0 35px rgba(212,175,55,.08);
  transform:rotateX(22deg) rotateZ(-1deg);
  transform-style:preserve-3d;
  position:relative;
  padding:14px;
}
.center{
  ...
  border-radius:24px;
  background:
    radial-gradient(circle at center,rgba(212,175,55,.13),rgba(0,0,0,.74) 62%),
...
.center .brand{
  ...
  color:#d4af37;
  font-weight:900;
  text-shadow:0 0 22px rgba(212,175,55,.58);
  z-index:2;
}
.center .sub{font-size:18px;letter-spacing:4px;color:#fff;z-index:2}
.dice3d{
  margin-top:20px;
  width:92px;
  height:92px;
  border-radius:18px;
  background:linear-gradient(145deg,#fff,#d4af37 48%,#5d3906);
  color:#050505;
  display:grid;
  place-items:center;
  font-size:52px;
  ...
  box-shadow:0 18px 25px rgba(0,0,0,.55),0 0 20px rgba(212,175,55,.55);
  transform-style:preserve-3d;
  z-index:3;
}
.dice3d.rolling{animation:diceSpin .75s linear infinite}
@keyframes diceSpin{
...
.cell .bar{height:16px;border-radius:8px;margin:-2px -2px 4px}
.brown{background:#8b4513}.sky{background:#45b7ff}.pink{background:#ff4fb3}.orange{background:#ff9c2e}.red{background:#e33131}.yellow{background:#f7d348}.green{background:#33c46b}.blue{background:#3167ff}.gold{background:#d4af37}.utility{background:#87e6ff}.transport{background:#c9c9c9}.bank{background:#ffd700}
.cell .name{font-weight:900;line-height:1.04}.cell .price{color:#d4af37;font-weight:900;margin-top:4px}
.house{position:absolute;bottom:4px;left:5px;font-size:18px}
.token{
  position:absolute;
  right:5px;
  bottom:3px;
...
select,input{
  width:100%;padding:10px;border-radius:12px;border:1px solid #d4af37;
  background:#050505;color:#fff;margin:7px 0;
}
button{
  width:100%;
  border:1px solid #d4af37;
...
.legend{font-size:12px;color:#d7c28a;line-height:1.4;margin-top:8px}
@media(max-width:980px){
  .game{grid-template-columns:1fr}
  .board3d{transform:rotateX(14deg) rotateZ(0deg);width:94vw;height:94vw}
  .cell{width:76px;height:76px;font-size:8.6px}
  .center{inset:105px}.center .brand{font-size:38px}.skyline{font-size:38px}
}
</style>
</head>
<body>
...
<div class="game">
  <div class="board3dWrap">
    <div class="board3d" id="board">
      <div class="center">
        ...
        <div class="sub">WORLD LUXURY EDITION</div>
        <div class="dice3d" id="centerDice">🎲</div>
  ...
  <div class="panel">
    ...
    <div class="info">
      ...
      <div><b>Piyon:</b> <span id="currentToken">🎩</span></div>
    ...
    <button class="dark" onclick="payBank()">🏦 BANKAYA ÖDE</button>
    <button class="dark" onclick="endTurn()">🔄 TURU BİTİR</button>
    <div class="legend">🏦 Banka ödemesi özel banka karesinde veya borç durumunda kullanılabilir.</div>
...
<script>
const cells=[
 ["Başlangıç",0,"gold","start"],
 ["Sofia",60,"brown","city"],["Bucarest",80,"brown","city"],["İstanbul",100,"brown","city"],
 ["✈️ Aéroport International",200,"transport","transport"],
 ["Belgrade",120,"sky","city"],["Prague",140,"sky","city"],["Budapest",160,"sky","city"],
 ["🏦 Banque Centrale",0,"bank","bank"],
 ["Lisbonne",180,"pink","city"],["Athènes",200,"pink","city"],["Madrid",220,"pink","city"],
 ["🚢 Port International",220,"transport","transport"],
 ["Rome",240,"orange","city"],["Berlin",260,"orange","city"],["Moscou",280,"orange","city"],
 ["⚡ Compagnie Électrique",150,"utility","utility"],
 ["Amsterdam",300,"red","city"],["Milan",320,"red","city"],["Paris",350,"red","city"],
 ["🚄 Gare Grande Vitesse",240,"transport","transport"],
 ["Londres",380,"yellow","city"],["Singapour",420,"yellow","city"],["Tokyo",450,"green","city"],
 ["💧 Compagnie des Eaux",150,"utility","utility"],
 ["Pékin",480,"green","city"],["Dubaï",520,"blue","city"],["Toronto",560,"blue","city"],
 ["Hong Kong",600,"blue","city"],["Monaco",650,"blue","city"],["New York",700,"blue","city"]
];
let pos=0;
window.money=1500;
let owned=[];
...
function changeToken(){
 playerToken=document.getElementById("tokenSelect").value;
 document.getElementById("currentToken").textContent=playerToken;
 renderBoard();
}
function placeCell(i,el){
 ...
 if(i<=side){
   x=max-i*(size+gap);y=max;
 }else if(i<=side*2){
   x=margin;y=max-(i-side)*(size+gap);
 }else if(i<=side*3){
   x=margin+(i-side*2)*(size+gap);y=margin;
 }else{
   x=max;y=margin+(i-side*3)*(size+gap);
 }
...
function renderBoard(){
 ...
 cells.forEach((c,i)=>{
   ...
   div.className="cell";
   let house = owned.includes(i) && c[3]==="city" ? '<div class="house">🏠</div>' : "";
   div.innerHTML='<div class="bar '+c[2]+'"></div><div class="name">'+c[0]+'</div>'+(c[1]>0?'<div class="price">'+c[1]+' $</div>':'')+house+(i===pos?'<div class="token">'+playerToken+'</div>':'');
...
function rollDice(){
 const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];
 centerDice.classList.add("rolling");
 let count=0;
 const anim=setInterval(()=>{
  centerDice.textContent=faces[Math.floor(Math.random()*6)];
  count++;
  if(count>13){
    clearInterval(anim);
    const roll=Math.floor(Math.random()*6)+1;
    centerDice.classList.remove("rolling");
    centerDice.textContent=faces[roll-1]+" "+roll;
    move(roll);
  }
 },70);
}
function move(n){
 ...
 if(cell[3]==="bank"){addLog("🏦 Banka karesindesin. İstersen bankaya ödeme yapabilirsin.")}
 renderBoard();
}
function buyProperty(){
 const cell=cells[pos];
 if(!["city","transport","utility"].includes(cell[3])){addLog("Bu kare satın alınamaz.");return}
...
function payBank(){
 const amount=50;
 if(window.money<amount){addLog("Bankaya ödeme için yeterli paran yok.");return}
 window.money-=amount;
 addLog("🏦 Bankaya ödeme yapıldı: -"+amount+" $");
 renderBoard();
}
