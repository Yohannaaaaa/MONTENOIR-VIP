...
function updateProfileChip(){
    if(currentAccount){
        let avatarSrc = (currentProfile && currentProfile.avatarData) ? currentProfile.avatarData : '/static/' + ((currentProfile && currentProfile.avatar) ? currentProfile.avatar : 'woman.png');
        profileChip.innerHTML='<span class="avatarWrap frame-gold"><img src="'+avatarSrc+'" class="avatarImg"></span><span>'+currentAccount+' · 🪙 '+currentChips+'</span>';
    }else{
...
function restoreAccountFromStorage(){
    ...
    if(a && !currentAccount){
        ...
        if(!currentProfile){currentProfile={username:a,chips:getSavedChips(a),wins:0,games:0,avatar:'woman.png'};}
        currentChips=currentProfile.chips || getSavedChips(a);
        if(playerName) playerName.value=a;
        if(authStatus) authStatus.innerHTML='Connecté : '+a;
        updateProfileChip();
    }
    return !!currentAccount;
}

function rememberLoggedProfile(profile){
    currentAccount=profile.username;
    currentProfile=profile;
    currentChips=profile.chips || getSavedChips(profile.username);
...
function requireLogin(){
    ...
    if(!currentProfile){
        ...
        currentChips = currentProfile.chips || 1000;
        updateProfileChip();
    }
    return true;
}
...
socket.on('register_result',d=>{
    ...
    rememberLoggedProfile(d.profile);
    closeModals();
