...
def register_page():
    ...
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Devenir membre - Montenoir VIP</title>
    <script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
    <style>
        body{margin:0;min-height:100vh;background:radial-gradient(circle at top,#2b1a05,#050505 55%,#000);color:#d4af37;font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;padding:30px;box-sizing:border-box;}
        .box{width:min(420px,92vw);background:#0009;border:1px solid #d4af37;border-radius:18px;padding:30px;box-shadow:0 0 25px rgba(212,175,55,.4);}
        h1{text-align:center;margin-top:0;}
        input,select{width:100%;box-sizing:border-box;padding:12px;margin-bottom:14px;border-radius:10px;border:1px solid #d4af37;background:#111;color:#fff;font-size:15px;}
        button{width:100%;padding:13px;border:none;border-radius:10px;background:linear-gradient(145deg,#fff4b0,#d4af37);color:#120800;font-weight:bold;font-size:15px;cursor:pointer;}
        button:hover{opacity:.9;}
        #msg{text-align:center;margin-top:14px;min-height:20px;}
        .ok{color:#7CFC00;}
        .err{color:#ff5555;}
        a{color:#d4af37;text-decoration:none;}
        .bottom{text-align:center;margin-top:16px;font-size:14px;}
    </style>
    </head>
    <body>
    <div class="box">
        ...
        <input id="regPassword2" type="password" placeholder="Confirmer le mot de passe">
        <select id="regGender">
        ...
        <div id="msg"></div>
        <div class="bottom">Déjà membre ? <a href="/login">Se connecter</a> · <a href="/">← Retour accueil</a></div>
    </div>
    <script>
    const socket = io();
    function doRegister(){
        ...
        if(!username || !email || !password){
            msg.className='err'; msg.textContent="Remplis tous les champs obligatoires.";
            return;
        }
        if(password !== password2){
            msg.className='err'; msg.textContent="Les mots de passe ne correspondent pas.";
            return;
        }
        msg.textContent = "Création en cours...";
        socket.emit('register_account', {username, email, password, gender});
    }
    socket.on('register_result', (res)=>{
        const msg = document.getElementById('msg');
        if(res.ok){
            msg.className='ok'; msg.textContent="Compte créé avec succès ! Redirection...";
            try{
                localStorage.setItem('codenamesAccount', document.getElementById('regUsername').value.trim());
                localStorage.setItem('loggedUser', document.getElementById('regUsername').value.trim());
            }catch(e){}
            setTimeout(()=>{ window.location.href='/'; }, 1200);
        } else {
            msg.className='err'; msg.textContent = res.msg || "Erreur lors de l'inscription.";
        }
    });
    </script>
    </body>
...
def login_page():
    ...
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Connexion - Montenoir VIP</title>
    <script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
    <style>
        body{margin:0;min-height:100vh;background:radial-gradient(circle at top,#2b1a05,#050505 55%,#000);color:#d4af37;font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;padding:30px;box-sizing:border-box;}
        .box{width:min(420px,92vw);background:#0009;border:1px solid #d4af37;border-radius:18px;padding:30px;box-shadow:0 0 25px rgba(212,175,55,.4);}
        h1{text-align:center;margin-top:0;}
        input{width:100%;box-sizing:border-box;padding:12px;margin-bottom:14px;border-radius:10px;border:1px solid #d4af37;background:#111;color:#fff;font-size:15px;}
        button{width:100%;padding:13px;border:none;border-radius:10px;background:linear-gradient(145deg,#fff4b0,#d4af37);color:#120800;font-weight:bold;font-size:15px;cursor:pointer;}
        button:hover{opacity:.9;}
        #msg,#fmsg{text-align:center;margin-top:10px;min-height:20px;}
        .ok{color:#7CFC00;}
        .err{color:#ff5555;}
        a{color:#d4af37;text-decoration:none;cursor:pointer;}
        .bottom{text-align:center;margin-top:16px;font-size:14px;}
        #forgotBox{display:none;margin-top:22px;border-top:1px solid #d4af37;padding-top:18px;}
    </style>
    </head>
    <body>
    <div class="box">
        ...
        <div id="msg"></div>
        <div class="bottom">
            <a onclick="toggleForgot()">Mot de passe oublié ?</a><br><br>
            Pas encore membre ? <a href="/register">Devenir membre</a> · <a href="/">← Retour accueil</a>
        </div>

        <div id="forgotBox">
            <h2 style="text-align:center;font-size:18px;">🔑 Réinitialiser le mot de passe</h2>
            <input id="forgotEmail" type="email" placeholder="Ton email">
            <button onclick="doForgot()">Envoyer le lien</button>
            <div id="fmsg"></div>
            <input id="resetToken" placeholder="Code reçu par email" style="margin-top:14px;">
    ...
    function doLogin(){
        ...
        if(!username || !password){
            msg.className='err'; msg.textContent="Entre ton nom d'utilisateur et ton mot de passe.";
            return;
        }
        msg.textContent = "Connexion en cours...";
        socket.emit('login_account', {username, password});
    }
    socket.on('login_result', (res)=>{
        const msg = document.getElementById('msg');
        if(res.ok){
            msg.className='ok'; msg.textContent="Connecté ! Redirection...";
            try{
                localStorage.setItem('codenamesAccount', document.getElementById('loginUsername').value.trim());
                localStorage.setItem('loggedUser', document.getElementById('loginUsername').value.trim());
            }catch(e){}
            setTimeout(()=>{ window.location.href='/'; }, 900);
        } else {
            msg.className='err'; msg.textContent = res.msg || "Connexion impossible.";
        }
    });
    ...
    function doForgot(){
        ...
        fmsg.className=''; fmsg.textContent='';
        if(!email){ fmsg.className='err'; fmsg.textContent="Entre ton email."; return; }
        fmsg.textContent = "Envoi en cours...";
        socket.emit('request_password_reset', {email});
    }
    socket.on('password_reset_requested', (res)=>{
        const fmsg = document.getElementById('fmsg');
        fmsg.className='ok';
        fmsg.textContent = "Si ce compte existe, un lien/code de réinitialisation a été envoyé par email.";
        if(res.token){ document.getElementById('resetToken').value = res.token; }
    });
    ...
    function doReset(){
        ...
        if(!token || !newPassword){
            fmsg.className='err'; fmsg.textContent="Entre le code reçu et un nouveau mot de passe.";
            return;
        }
        socket.emit('confirm_password_reset', {token, newPassword});
    }
    socket.on('password_reset_confirmed', (res)=>{
        const fmsg = document.getElementById('fmsg');
        if(res.ok){
            fmsg.className='ok'; fmsg.textContent="Mot de passe changé ! Tu peux te connecter.";
        } else {
            fmsg.className='err'; fmsg.textContent = res.msg || "Code invalide ou expiré.";
        }
    });

    // Si on arrive ici via le lien envoyé par email (?reset=TOKEN)
    const params = new URLSearchParams(window.location.search);
    if(params.get('reset')){
        document.getElementById('forgotBox').style.display = 'block';
        document.getElementById('resetToken').value = params.get('reset');
    }
    </script>
    </body>
    </html>
...
@app.route("/premium", endpoint="force_premium_page_final")
def premium_page_final():
    return render_template_string("""
    <html>
    <head><meta charset="UTF-8"><title>Premium</title></head>
    <body style="margin:0;background:#050505;color:#d4af37;font-family:Arial,sans-serif;text-align:center;padding-top:120px;">
        <h1>💎 Premium</h1>
        <h2>MONTENOIR VIP</h2>
        <p>Espace Premium bientôt disponible.</p>
        <br>
