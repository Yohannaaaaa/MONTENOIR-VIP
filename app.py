...
<div class="home">
  ...
  <section id="gamesPanel" class="gamesPanel">
    ...
    <div class="gameGrid">
      ...
      <a href="/coming-soon/101">💎 101</a><a href="/metropoly">🏙️ Metropoly</a>
...
<div class="card"><h2>📜 Oyun Kuralları</h2><p>Codenames, Poker, Tavla, Okey, 101, Metropoly, Ludo ve Bowling kuralları.</p><a class="btn" href="/oyun-kurallari">Aç</a></div>
...
def metropoly_redirect():
    return redirect("/metropoly")
...
@socketio.on("metropoly_create_room")
def mc(data):
    ...
    emit("metropoly_room_state",m_public(r),room=c)

@socketio.on("metropoly_join_room")
def mj(data):
    u=(data or {}).get("username","Misafir").strip() or "Misafir"
    t=(data or {}).get("token","🎩")
    c=((data or {}).get("code","") or "").strip().upper()
    if c not in METROPOLY_ROOMS:
        emit("metropoly_error",{"msg":"Oda bulunamadı."}); return
    r=METROPOLY_ROOMS[c]
    join_room(c)
...
@socketio.on("metropoly_start_game")
def ms(data):
    c=((data or {}).get("code","") or "").strip().upper()
...
def metropoly_apply_community_effect(r,u):
    import random
    p=r["players"][u]
...
@socketio.on("metropoly_roll_dice")
def mr(data):
    c=((data or {}).get("code","") or "").strip().upper()
    u=(data or {}).get("username","").strip()
    r=METROPOLY_ROOMS.get(c)
    if not r or u not in r["players"]:
        emit("metropoly_error",{"msg":"Oda/oyuncu yok."}); return
    names=active_names(r)
    if not names:
        return
    if u!=names[r["turnIndex"]%len(names)]:
        emit("metropoly_error",{"msg":"Sıra sende değil."}); return
    p=r["players"][u]
    if p.get("hasRolled") and r.get("lastDice")!=6:
        emit("metropoly_error",{"msg":"Bu tur zaten zar attın. Sırayı bitir."}); return
    if p.get("jailed",0)>0:
        if p["money"]>=JAIL_FINE:
            ...
            next_turn(r)
            emit("metropoly_room_state",m_public(r),room=c); return
...
@socketio.on("metropoly_buy_property")
def mb(data):
    c=((data or {}).get("code","") or "").strip().upper()
    u=(data or {}).get("username","").strip()
    r=METROPOLY_ROOMS.get(c)
    ...
    if typ not in ["property","transport","utility"] or str(pos) in r["owners"] or p["money"]<price:
        emit("metropoly_error",{"msg":"Satın alınamaz."}); return
    p["money"]-=price
    r["owners"][str(pos)]=u
...
@socketio.on("metropoly_end_turn")
def me(data):
    c=((data or {}).get("code","") or "").strip().upper()
    r=METROPOLY_ROOMS.get(c)
    if not r:
...
def mc(data):
    ...
    r["lastLog"]=u+" odayı kurdu."; emit("metropoly_room_state",m_public(r),room=c)

@socketio.on("metropoly_join_room")
def mj(data):
    u=(data or {}).get("username","Misafir").strip() or "Misafir"; t=(data or {}).get("token","🎩"); c=((data or {}).get("code","") or "").strip().upper()
    if c not in METROPOLY_ROOMS: emit("metropoly_error",{"msg":"Oda bulunamadı."}); return
    r=METROPOLY_ROOMS[c]; join_room(c)
    if u not in r["players"]:
        r["players"][u]={"name":u,"token":t,"money":1000,"position":0,"jailed":0,"bankrupt":False,"hasRolled":False}
    r["lastLog"]=u+" odaya girdi."; emit("metropoly_room_state",m_public(r),room=c)

@socketio.on("metropoly_start_game")
def ms(data):
    c=((data or {}).get("code","") or "").strip().upper(); r=METROPOLY_ROOMS.get(c)
    if r: r["started"]=True; r["lastLog"]="Oyun başladı."; emit("metropoly_room_state",m_public(r),room=c)

@socketio.on("metropoly_roll_dice")
def mr(data):
    c=((data or {}).get("code","") or "").strip().upper(); u=(data or {}).get("username","").strip(); r=METROPOLY_ROOMS.get(c)
    if not r or u not in r["players"]: emit("metropoly_error",{"msg":"Oda/oyuncu yok."}); return
    names=[n for n,p in r["players"].items() if not p.get("bankrupt")]
    if u!=names[r["turnIndex"]%len(names)]: emit("metropoly_error",{"msg":"Sıra sende değil."}); return
    ...
    r["lastDice"]=d; r["lastLog"]=log; emit("metropoly_animate_move",{"username":u,"from":old,"to":new,"dice":d,"state":m_public(r),"pendingBuy":pending},room=c)
...
def mb(data):
    ...
    if typ not in ["property","transport","utility"] or str(pos) in r["owners"] or p["money"]<price: emit("metropoly_error",{"msg":"Satın alınamaz."}); return
    p["money"]-=price; r["owners"][str(pos)]=u; r["lastLog"]=f"{u}, {name} aldı."; emit("metropoly_room_state",m_public(r),room=c)
...
@socketio.on("metropoly_chat")
def metropoly_chat(data):
    c=((data or {}).get("code","") or "").strip().upper()
    u=(data or {}).get("username","").strip() or "Oyuncu"
    msg=(data or {}).get("message","").strip()
    if c and msg:
        emit("metropoly_chat_message",{"username":u,"message":msg},room=c)


@socketio.on("metropoly_add_house")
def metropoly_add_house(data):
    c=((data or {}).get("code","") or "").strip().upper()
    u=(data or {}).get("username","").strip()
    ...
    if r.get("owners",{}).get(cell)!=u:
        emit("metropoly_error",{"msg":"Bu mülk senin değil."})
        return
    if r["hotels"].get(cell):
        emit("metropoly_error",{"msg":"Bu mülkte zaten otel var."})
        return
    count=int(r["houses"].get(cell,0))
    if count>=4:
        emit("metropoly_error",{"msg":"4 evden sonra otel almalısın."})
        return
    if r["players"][u]["money"]<50:
        emit("metropoly_error",{"msg":"Ev için para yetmiyor."})
        return
...
@socketio.on("metropoly_add_hotel")
def metropoly_add_hotel(data):
    c=((data or {}).get("code","") or "").strip().upper()
    u=(data or {}).get("username","").strip()
    cell=str((data or {}).get("cell",""))
    r=METROPOLY_ROOMS.get(c)
    ...
    if int(r["houses"].get(cell,0))<4:
        emit("metropoly_error",{"msg":"Otel için önce 4 ev gerekir."})
        return
    if r["players"][u]["money"]<100:
        emit("metropoly_error",{"msg":"Otel için para yetmiyor."})
        return
...
@app.route("/metropoly")
def metropoly_page():
    return '<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>METROPOLY</title><script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script><style>\n*{box-sizing:border-box}body{margin:0;overflow:hidden;background:#030303;color:white;font-family:Arial,Helvetica,sans-serif}.top{position:fixed;left:12px;right:12px;top:10px;height:54px;z-index:120;display:flex;gap:8px;align-items:center;padding:7px 10px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(#171717,#030303);box-shadow:0 0 22px #000}.top input,.top select,.top button{height:38px;border-radius:12px;border:1px solid #d4af37;background:#070707;color:white;padding:8px 10px;font-weight:900}.top button{background:linear-gradient(#ffe08a,#b87913);color:#160800;cursor:pointer}.loca{color:#ffd989;text-decoration:none;font-weight:900;border:1px solid #d4af37;border-radius:12px;padding:9px 14px;background:#000}.room{color:#ffd989;font-weight:900}\n#stage{position:fixed;left:0;right:0;top:72px;bottom:0;background:radial-gradient(circle at 50% 15%,#1d1206,#000 68%);overflow:hidden}#boardWrap{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(94vw,calc((100vh - 78px)*1.48));z-index:1}#boardImg{display:block;width:100%;height:auto;filter:drop-shadow(0 25px 30px #000)}#overlay{position:absolute;left:0;top:0;width:100%;height:100%;z-index:20;pointer-events:none}\n.tokenImg{position:absolute;width:64px;height:64px;object-fit:contain;transform:translate(-50%,-78%);filter:drop-shadow(0 0 12px currentColor) drop-shadow(0 14px 12px #000);z-index:45;pointer-events:none;will-change:left,top,transform}.tokenImg.tap{animation:pawnTap .28s ease}@keyframes pawnTap{0%{transform:translate(-50%,-78%) scale(1)}45%{transform:translate(-50%,-95%) scale(1.08)}100%{transform:translate(-50%,-78%) scale(1)}}\n.ownerDot{position:absolute;width:21px;height:21px;border-radius:50%;transform:translate(-50%,-50%);border:3px solid #ffe7a6;box-shadow:0 0 13px currentColor,0 0 7px rgba(255,255,255,.65),inset 3px 3px 6px rgba(255,255,255,.55),inset -4px -4px 7px rgba(0,0,0,.45);z-index:34;pointer-events:auto}.ownerDot::after{content:"";position:absolute;left:50%;top:50%;width:7px;height:7px;border-radius:50%;transform:translate(-50%,-50%);background:rgba(255,255,255,.78)}\n.buildImg{position:absolute;object-fit:contain;transform:translate(-50%,-100%);filter:drop-shadow(0 8px 7px #000);z-index:36;pointer-events:none}.houseBuild{width:42px;height:34px}.hotelBuild{width:58px;height:56px}.diceArea{position:absolute;left:50%;top:49%;transform:translate(-50%,-50%);z-index:70;display:flex;gap:18px;pointer-events:none}.diceImg{width:68px;height:68px;object-fit:contain;filter:drop-shadow(0 14px 10px #000) drop-shadow(0 0 9px #d4af37)}.diceArea.rolling .diceImg:first-child{animation:diceA 1.05s ease both}.diceArea.rolling .diceImg:last-child{animation:diceB 1.05s ease both}@keyframes diceA{0%{transform:translate(-48px,-35px) rotate(0deg) scale(.82)}30%{transform:translate(20px,12px) rotate(320deg) scale(1.08)}60%{transform:translate(-10px,-8px) rotate(610deg) scale(.98)}100%{transform:translate(0,0) rotate(720deg) scale(1)}}@keyframes diceB{0%{transform:translate(48px,-35px) rotate(0deg) scale(.82)}30%{transform:translate(-20px,12px) rotate(-320deg) scale(1.08)}60%{transform:translate(10px,-8px) rotate(-610deg) scale(.98)}100%{transform:translate(0,0) rotate(-720deg) scale(1)}}.roll{position:absolute;left:50%;top:64%;transform:translate(-50%,-50%);z-index:90;width:210px;height:58px;border-radius:20px;border:2px solid #ffd989;background:linear-gradient(#f7c863,#b87913 55%,#4d2700);color:#180900;font-size:26px;font-weight:900;box-shadow:0 0 26px #d4af3788,0 12px 24px #000;cursor:pointer;pointer-events:auto}\n.buildControls{position:fixed;left:16px;top:92px;z-index:140;display:flex;flex-direction:column;gap:12px}.buildBtn{width:150px;height:56px;border-radius:16px;border:2px solid #ffd989;background:linear-gradient(145deg,#ffdb70,#a96b0b 58%,#3d2100);color:#1b0b00;font-size:18px;font-weight:900;box-shadow:0 0 20px rgba(212,175,55,.55),0 10px 18px #000;cursor:pointer}.buildBtn.hotel{background:linear-gradient(145deg,#fff0a8,#c78b16 45%,#5a2e00)}.buildBtn:disabled{filter:grayscale(.9) brightness(.55);cursor:not-allowed;opacity:.62}.buildHint{width:150px;color:#ffd989;background:#000c;border:1px solid #d4af37aa;border-radius:12px;font-size:11px;font-weight:900;padding:8px;line-height:1.25;text-align:center}\n.panel{position:fixed;left:50%;top:44%;transform:translate(-50%,-50%);z-index:160;width:410px;max-width:92vw;padding:22px;border-radius:22px;background:#000e;border:1px solid #d4af37;box-shadow:0 0 30px #d4af3777;text-align:center;display:none}.panel button{border:1px solid #d4af37;border-radius:12px;padding:12px 18px;margin:8px;font-weight:900;cursor:pointer;background:linear-gradient(#ffe08a,#b87913);color:#150800}.dark{background:linear-gradient(#cf4d42,#6d120b)!important;color:white!important}.players{position:fixed;right:12px;top:74px;width:270px;max-height:43vh;z-index:125;border:1px solid #d4af37aa;border-radius:16px;background:#000c;padding:10px;overflow:auto}.player{display:flex;align-items:center;gap:8px;border:1px solid #d4af3755;border-radius:12px;margin:6px 0;padding:8px;background:#111c}.turn{border-color:#00ff66!important;box-shadow:0 0 12px #00ff6670}.dot{width:16px;height:16px;border-radius:50%;border:2px solid #fff9}.money{color:#ffd989}.toast,.chatlog{position:fixed;left:16px;bottom:18px;z-index:170;width:340px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.show{opacity:1}textarea{width:100%;height:110px;border:1px solid #d4af37;border-radius:14px;background:#070707;color:white;padding:12px;font-size:15px}\n</style></head><body>\n<div class="top"><a class="loca" href="/">▮ LOCA</a><input id="username" placeholder="Kullanıcı adı"><select id="tokenSelect"><option value="horse">🐴 Cheval</option><option value="jackpot">🎰 Machine</option><option value="boat">🛥️ Bateau</option><option value="car">🚗 Voiture</option><option value="hat">🎩 Chapeau</option><option value="cat">🐈 Chat noir</option></select><button onclick="createRoom()">ODA OLUŞTUR</button><input id="roomCodeInput" placeholder="MON-1234"><button onclick="joinRoom()">ODAYA GİR</button><button onclick="startGame()">OYUNU BAŞLAT</button><button onclick="openChat()">CHAT</button><span id="topRoomLabel" class="room">Oda: -</span></div>\n<div id="stage"><div id="boardWrap"><img id="boardImg" src="/static/metropoly_board_lux_3d.png"><div id="overlay"><div id="diceArea" class="diceArea"><img class="diceImg" src="/static/assets/metropoly/dice_gold.png"><img class="diceImg" src="/static/assets/metropoly/dice_gold.png"></div><button class="roll" onclick="rollDice()">🎲 ZAR AT</button><div id="tokens"></div><div id="owners"></div><div id="builds"></div></div></div></div>\n<div class="buildControls"><button id="buildHouseBtn" class="buildBtn" onclick="buildHouse()">🏠 EV AL</button><button id="buildHotelBtn" class="buildBtn hotel" onclick="buildHotel()">🏨 OTEL AL</button><div id="buildHint" class="buildHint">Ev için aynı renk serisini tamamla.</div></div>\n<div class="players"><b>ONLINE OYUNCULAR</b><div id="playersList"></div></div><div id="buyPanel" class="panel"><div id="buyCity" style="font-size:27px;font-weight:900;color:#ffb000">PARIS</div><div style="font-size:21px;font-weight:900;margin:12px 0">satın almak ister misin?</div><div id="buyPrice">Fiyat: 160 €</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">VAZGEÇ</button></div><div id="chatPanel" class="panel"><h3>CHAT</h3><textarea id="chatInput" placeholder="Mesaj yaz..."></textarea><button onclick="sendChat()">GÖNDER</button><button onclick="chatPanel.style.display=\'none\'">KAPAT</button></div><div id="moneyToast" class="toast"></div><div id="chatLog" class="chatlog"></div>\n<script>\nconst socket=io();let roomCode="",myName="",currentState=null,lastCell=null,previousMoneyState={};\nconst cells=["ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Amsterdam","Eaux","Moscou","Caisse de Communauté","Rome","Madrid","Impôts","Sofia","Belgrad","Gare","Istanbul","Prague","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo"];\nconst prices=[0,0,420,400,380,360,260,260,0,300,280,0,260,0,260,250,0,180,190,0,200,220,240,0,420,420,0,450];\nconst coords=[[82.5,51.7],[79.3,68.9],[70.2,68.4],[60.9,68.1],[51.3,68.2],[42.1,68.3],[32.7,68.6],[23.5,68.7],[14.8,68.5],[18.3,57.4],[18.7,50.5],[19.1,43.4],[19.6,34.0],[19.9,29.2],[20.2,24.2],[20.4,17.7],[26.7,8.2],[34.8,8.2],[43.0,8.2],[51.1,8.1],[58.9,8.1],[66.8,8.1],[74.7,8.1],[82.7,8.1],[83.1,18.0],[83.3,28.0],[83.7,39.6],[84.2,50.2]];\nconst colorGroups={green:[2,3,5],brown:[6,7],orange:[9,10],purple:[12,14,15],yellow:[17,18,20],blue:[21,22],red:[24,25,27]};\nconst tokenAsset={horse:"pawn_horse.png",jackpot:"pawn_jackpot.png",boat:"pawn_boat.png",car:"pawn_car.png",hat:"pawn_hat.png",cat:"pawn_cat.png"};\nconst houseColorAsset={green:"house_green.png",brown:"house_brown.png",orange:"house_orange.png",red:"house_red.png",purple:"house_purple.png",yellow:"house_yellow.png",blue:"house_green.png"};\nfunction boardRect(){return boardImg.getBoundingClientRect()}function point(i,off=0){const r=boardRect(),p=coords[i]||coords[0];return{x:p[0]/100*r.width+off,y:p[1]/100*r.height+off}}\nfunction safeId(n){return "tok_"+String(n).replace(/[^a-zA-Z0-9_-]/g,"_")}function name(){return (username.value||localStorage.getItem("montenoirUser")||"Misafir").trim()}\nfunction createRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("metropoly_create_room",{username:myName,token:tokenSelect.value})}function joinRoom(){myName=name();localStorage.setItem("montenoirUser",myName);socket.emit("metropoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value.trim().toUpperCase()})}function startGame(){if(!roomCode){alert("Önce oda oluştur veya odaya gir.");return}socket.emit("metropoly_start_game",{code:roomCode})}function rollDice(){if(!roomCode||!myName){alert("Önce odaya gir veya oda oluştur.");return}hideBuy();socket.emit("metropoly_roll_dice",{code:roomCode,username:myName})}function buyProperty(){socket.emit("metropoly_buy_property",{code:roomCode,username:myName});hideBuy()}function hideBuy(){buyPanel.style.display="none"}function openChat(){chatPanel.style.display="block"}\nfunction getGroup(cell){cell=Number(cell);for(const [g,arr] of Object.entries(colorGroups)){if(arr.includes(cell))return g}return null}function ownsFullGroup(st,player,cell){const g=getGroup(cell);if(!g)return false;return colorGroups[g].every(c=>(st.owners||{})[String(c)]===player)}function canBuildHouseOn(cell){if(!currentState||!myName)return false;const c=String(cell);if((currentState.owners||{})[c]!==myName)return false;if(!ownsFullGroup(currentState,myName,Number(cell)))return false;if((currentState.hotels||{})[c])return false;return Number((currentState.houses||{})[c]||0)<4}function canBuildHotelOn(cell){if(!currentState||!myName)return false;const c=String(cell);if((currentState.owners||{})[c]!==myName)return false;if(!ownsFullGroup(currentState,myName,Number(cell)))return false;if((currentState.hotels||{})[c])return false;return Number((currentState.houses||{})[c]||0)>=4}function updateBuildButtons(){const p=currentState&&currentState.players?currentState.players[myName]:null;const cell=p?Number(p.position):null;const canH=cell!==null&&canBuildHouseOn(cell);const canO=cell!==null&&canBuildHotelOn(cell);buildHouseBtn.disabled=!canH;buildHotelBtn.disabled=!canO;if(!p)buildHint.textContent="Önce odaya gir.";else if((currentState.owners||{})[String(cell)]!==myName)buildHint.textContent="Kendi mülküne gel.";else if(!ownsFullGroup(currentState,myName,cell))buildHint.textContent="Ev için aynı renk serisini tamamla.";else if(canO)buildHint.textContent="4 ev tamam. Otel alabilirsin.";else if(canH)buildHint.textContent="Seri tamam. Ev alabilirsin.";else buildHint.textContent="Bu yerde inşa hakkı yok."}function buildHouse(){const c=getBuildCell();if(c==null||!canBuildHouseOn(c)){alert("Ev almak için aynı renk serisinin tamamı sende olmalı ve kendi mülkünde olmalısın.");return}socket.emit("metropoly_add_house",{code:roomCode,username:myName,cell:c})}function buildHotel(){const c=getBuildCell();if(c==null||!canBuildHotelOn(c)){alert("Otel almak için bu mülkte önce 4 ev olmalı.");return}socket.emit("metropoly_add_hotel",{code:roomCode,username:myName,cell:c})}function getBuildCell(){if(!currentState||!myName)return null;const p=currentState.players[myName];if(!p)return null;const c=String(p.position);if((currentState.owners||{})[c]===myName)return Number(p.position);return null}\nfunction animateDice(n,done){diceArea.classList.remove("rolling");void diceArea.offsetWidth;diceArea.classList.add("rolling");setTimeout(()=>{diceArea.classList.remove("rolling");if(done)done()},1100)}\nfunction render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;renderPlayers(st);renderOwners(st);renderBuilds(st);Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];let el=document.getElementById(safeId(n));if(!el){el=document.createElement("img");el.id=safeId(n);el.className="tokenImg";tokens.appendChild(el)}const key=tokenAsset[p.token]||tokenAsset.horse;el.src="/static/assets/metropoly/"+key;el.style.color=p.color;const pt=point(p.position,i*7);el.style.left=pt.x+"px";el.style.top=pt.y+"px"});detectMoneyChanges(st);updateBuildButtons()}\nfunction renderPlayers(st){playersList.innerHTML="";Object.keys(st.players||{}).forEach((n,i)=>{const p=st.players[n];const colors=["#34c759","#ff3b30","#007aff","#ffd60a","#bf5af2","#ff9500"];if(!p.color)p.color=colors[i%colors.length];const key=tokenAsset[p.token]||tokenAsset.horse;const d=document.createElement("div");d.className="player"+(i===st.turnIndex?" turn":"");d.innerHTML=`<div class="dot" style="background:${p.color}"></div><img src="/static/assets/metropoly/${key}" style="width:34px;height:34px;object-fit:contain"><div><b>${n}</b><div class="money">${p.money}€</div></div>`;playersList.appendChild(d)})}\nfunction renderOwners(st){owners.innerHTML="";Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const pt=point(Number(cell));const dot=document.createElement("div");dot.className="ownerDot";dot.style.background=p.color;dot.style.color=p.color;dot.title="Propriétaire : "+owner;dot.style.left=pt.x+"px";dot.style.top=(pt.y+34)+"px";owners.appendChild(dot)})}\nfunction renderBuilds(st){builds.innerHTML="";Object.entries(st.houses||{}).forEach(([cell,n])=>{if(!n)return;const g=getGroup(cell)||"yellow";const file=houseColorAsset[g]||"house_yellow.png";const pt=point(Number(cell));for(let k=0;k<Number(n);k++){const b=document.createElement("img");b.className="buildImg houseBuild";b.src="/static/assets/metropoly/"+file;b.style.left=(pt.x-18+k*12)+"px";b.style.top=(pt.y-18)+"px";builds.appendChild(b)}});Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(!n)return;const b=document.createElement("img");b.className="buildImg hotelBuild";b.src="/static/assets/metropoly/hotel_gold.png";const pt=point(Number(cell));b.style.left=pt.x+"px";b.style.top=(pt.y-18)+"px";builds.appendChild(b)})}\nfunction moveToken(n,from,to,state,done){let el=document.getElementById(safeId(n));if(!el){render(state);done();return}let steps=[],cur=Number(from),dest=Number(to),total=coords.length;while(cur!==dest){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Math.max(0,Object.keys(state.players).indexOf(n));let si=0;function one(){if(si>=steps.length){done();return}const step=steps[si];const a={x:parseFloat(el.style.left)||point(from).x,y:parseFloat(el.style.top)||point(from).y};const b=point(step,idx*7);let t0=null;const duration=285;el.classList.add("tap");function f(ts){if(!t0)t0=ts;const t=Math.min(1,(ts-t0)/duration);const e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;const arc=Math.sin(Math.PI*t)*36;el.style.left=(a.x+(b.x-a.x)*e)+"px";el.style.top=(a.y+(b.y-a.y)*e-arc)+"px";if(t<1)requestAnimationFrame(f);else{el.style.left=b.x+"px";el.style.top=b.y+"px";el.classList.remove("tap");si++;setTimeout(one,55)}}requestAnimationFrame(f)}one()}\nfunction sendChat(){const msg=(chatInput.value||"").trim();if(!msg)return;socket.emit("metropoly_chat",{code:roomCode,username:myName||name(),message:msg});chatInput.value="";chatPanel.style.display="none"}function showChatLog(u,msg){chatLog.textContent=u+": "+msg;chatLog.classList.add("show");clearTimeout(window.chatTimer);window.chatTimer=setTimeout(()=>chatLog.classList.remove("show"),5000)}function showMoneyToast(msg,pos=true){moneyToast.textContent=msg;moneyToast.style.color=pos?"#00ff66":"#ff4d4d";moneyToast.classList.add("show");clearTimeout(window.moneyToastTimer);window.moneyToastTimer=setTimeout(()=>moneyToast.classList.remove("show"),5000)}function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const d=p.money-previousMoneyState[n];if(d>0)showMoneyToast(n+" "+d+"€ aldı",true);if(d<0)showMoneyToast(n+" "+Math.abs(d)+"€ ödedi",false)}previousMoneyState[n]=p.money})}\nsocket.on("metropoly_room_state",render);socket.on("metropoly_error",d=>alert(d.msg));socket.on("metropoly_chat_message",d=>showChatLog(d.username,d.message));socket.on("metropoly_animate_move",d=>{animateDice(d.dice,()=>{if(currentState){moveToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=Number(d.state.players[myName].position);lastCell=landed;buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";setTimeout(()=>buyPanel.style.display="block",160)}})}else render(d.state)})});username.value=localStorage.getItem("montenoirUser")||"";\n</script></body></html>'
