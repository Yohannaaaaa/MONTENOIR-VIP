...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>METROPOLY 3D B1</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js"></script>
<style>
*{box-sizing:border-box}
body{margin:0;overflow:hidden;background:radial-gradient(circle at top,#261706,#050505 55%,#000);color:#fff;font-family:Arial}
.loca{position:fixed;top:12px;left:12px;z-index:20;padding:10px 16px;border:1px solid #d4af37;border-radius:12px;background:#000d;color:#ffd989;text-decoration:none;font-weight:900}
.topLobby{position:fixed;top:12px;left:130px;right:12px;z-index:20;height:56px;display:flex;gap:8px;align-items:center;padding:8px 10px;border:1px solid #d4af37aa;border-radius:16px;background:#000d}
...
#game3d{position:fixed;top:76px;left:0;right:0;bottom:86px;z-index:1}
.rollBtn{position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:30;width:300px;height:58px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(#ff5c4d,#b51512 60%,#5d0504);color:#fff;font-size:30px;font-weight:900;text-shadow:0 3px 5px #000;box-shadow:0 0 24px #ff2a2a80,0 12px 18px #000;cursor:pointer}
.roomLabel{color:#d4af37;font-weight:900;white-space:nowrap}
.buyPanel{position:fixed;left:50%;top:45%;transform:translate(-50%,-50%);z-index:50;width:410px;max-width:90%;padding:22px;border-radius:22px;background:#000c;border:1px solid #d4af37;text-align:center;display:none}
...
.toast{position:fixed;left:16px;bottom:16px;z-index:60;width:250px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;opacity:0;transition:.25s}.toast.show{opacity:1}
@media(max-width:850px){.topLobby{left:8px;right:8px;top:56px;height:auto;flex-wrap:wrap}.loca{top:8px}#game3d{top:150px}.rollBtn{width:230px;font-size:24px}}
</style>
</head>
<body>
...
function init3D(){
 ...
 camera=new THREE.PerspectiveCamera(43,c.clientWidth/c.clientHeight,.1,100);
 camera.position.set(0,6.8,9.4); camera.lookAt(0,0,0);
 renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});
 renderer.setPixelRatio(Math.min(devicePixelRatio,1.5)); renderer.setSize(c.clientWidth,c.clientHeight); renderer.shadowMap.enabled=true; c.appendChild(renderer.domElement);
 scene.add(new THREE.AmbientLight(0xffffff,.9));
 const dlight=new THREE.DirectionalLight(0xffe1a0,1.7); dlight.position.set(0,8,5); scene.add(dlight);
 const tex=new THREE.TextureLoader().load("/static/monopoly_board_lux_3d.png");
 tex.colorSpace=THREE.SRGBColorSpace;
 board=new THREE.Mesh(new THREE.BoxGeometry(boardW,.18,boardH),new THREE.MeshStandardMaterial({map:tex,roughness:.55,metalness:.04}));
 board.rotation.x=-.08; scene.add(board);
 const base=new THREE.Mesh(new THREE.BoxGeometry(boardW+.35,.16,boardH+.35),new THREE.MeshStandardMaterial({color:0x120c06,metalness:.45,roughness:.35}));
 base.position.y=-.19; scene.add(base);
 dice=createDice(); dice.position.set(0,.72,.35); scene.add(dice);
 addGridMarkers();
...
function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.35,(p[1]/100-.5)*boardH+off)}
function addGridMarkers(){/* invisible reference kept empty */}
function tokenMesh(icon,color){
 const g=new THREE.Group();
 const s=new THREE.Mesh(new THREE.SphereGeometry(.22,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.7,roughness:.25}));
 g.add(s);
 ...
 sp.position.y=.35;sp.scale.set(.55,.55,.55);g.add(sp);return g;
}
function createDice(){
 const g=new THREE.Group();
 const box=new THREE.Mesh(new THREE.BoxGeometry(.86,.86,.86),new THREE.MeshStandardMaterial({color:0xffffff,roughness:.32}));
 g.add(box);
 const dotMat=new THREE.MeshStandardMaterial({color:0x050505});
 const faces=[
  [[0,0,.44],[0,0,0],[[0,0]]],
 ...
 faces.forEach(f=>f[2].forEach(([a,b])=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.045,16,8),dotMat);dot.position.set(a,b,.49);const h=new THREE.Group();h.add(dot);h.position.set(...f[0]);h.rotation.set(...f[1]);g.add(h)}));
 return g;
}
...
function rollDiceAnim(n,done){let t=0;const id=setInterval(()=>{t++;dice.rotation.x+=.45;dice.rotation.y+=.62;dice.rotation.z+=.37;if(t>22){clearInterval(id);showDice(n);if(done)done()}},28)}
