...
def mc(data):
    ...
    MONOPOLY_ROOMS[c]=r
    join_room(c)
    colors=["#ff3b30","#007aff","#34c759","#ffd60a","#bf5af2","#ff9500"]; color=colors[(len(r["players"]))%len(colors)]; r["players"][u]={"name":u,"token":t,"color":color,"money":START_MONEY,"position":0,"jailed":0,"bankrupt":False}
  ...
  <div class="main">
    <div class="boardStage">
      ...
      <div class="boardWrap" id="boardWrap">
        ...
        <div class="imageMask diceMask"></div>
        <div class="imageMask buildMarketMask"></div>
...
let previousMoneyState={};
function showMoneyToast(message, positive=true){
  if(!moneyToast)return;
  moneyToast.className="moneyToast show "+(positive?"moneyPlus":"moneyMinus");
  moneyToast.textContent=message;
  clearTimeout(window.moneyToastTimer);
  window.moneyToastTimer=setTimeout(()=>{moneyToast.className="moneyToast";},5000);
}
function detectMoneyChanges(st){
  Object.entries(st.players||{}).forEach(([name,p])=>{
    if(previousMoneyState[name]!==undefined){
      const diff=p.money-previousMoneyState[name];
      if(diff>0) showMoneyToast(name+" bankadan "+diff+"€ aldı", true);
      if(diff<0) showMoneyToast(name+" "+Math.abs(diff)+"€ ödedi", false);
    }
    previousMoneyState[name]=p.money;
  });
}
...
function renderHouses(st){
 ...
 if(!hasButtons){
   housesPanel.innerHTML='<span style="font-size:12px;color:#aaa">Ev/otel için aynı renkteki tüm şehirleri almalısın.</span>';
 }
}
