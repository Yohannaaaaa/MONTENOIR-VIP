...
def upload_avatar(data):
    ...
    users[account]['avatarData'] = avatar_data
    users[account]['avatar'] = users[account].get('avatar', 'woman.png')
    save_users(users)
    ...
    if code in rooms:
        for p in rooms[code].get('players', []):
            if p.get('account') == account or p.get('sid') == request.sid:
                ...
                p['avatar'] = users[account].get('avatar', p.get('avatar', 'woman.png'))
    ...
    emit('avatar_upload_result', {'ok': True, 'profile': private_profile(account, users[account])})
