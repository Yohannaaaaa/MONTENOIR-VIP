...
<hr>
<h2>🪙 Paketler</h2>
<div>200 jeton · 4,99 £</div><div>500 jeton · 9,99 £</div><div>1200 jeton · 19,99 £</div><div>3000 jeton · 39,99 £</div><div>8000 jeton · 89,99 £</div>
</div>
</div>
...
<div class="vipCasinoMarks"><span class="m1">A♠</span><span class="m2">K♥</span><span class="m3">Q♣</span><span class="m4">J♦</span><span class="m5">♠♥♣♦</span><span class="m6">A K Q J</span></div><div id="profileChip" class="vipProfileChip" onclick="openProfile()">👤 Profil</div><button id="tarotTopBtn" class="tarotTopBtn" onclick="window.location.href='/tarot'">🔮 TAROT & RITUEL</button><div class="topLeftFixed"><button onclick="startTimer()">⏱ Süre</button><button onclick="newGame()">🎲 Yeni Oyun</button><button onclick="goLobby()">🚪 Lobi</button><button class="closeTableBtn" onclick="closeCurrentTable()">❌ Masayı Kapat</button><button onclick="openAuth()">👤 Üyelik</button><button onclick="openProfile()">🏆 Profil</button><button onclick="openRanking()">📊 Classement</button><button onclick="openSettings()">⚙️ Ayarlar</button><button onclick="openWords()">📚 Kelimeler</button><button onclick="openBet()">🎰 Bahis</button><button onclick="openShop()">🪙 Jeton Al</button></div>
...
function renderPlayers(players,locks){
    ...
    function micHtml(p){
        ...
        const nameClass = (st.speaking ? ' speakingName ' : ' ') + playerNameClass(p) + ' ' + playerGenderClass(p);
