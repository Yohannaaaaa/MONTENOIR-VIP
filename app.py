...
def tarot_lucky_wheel(data):
    ...
    if not key:
        ...
        return
    reward = random.choice([0, 10, 20, 30, 50, 100])
...
.tarotCardsBg{position:fixed;inset:0;z-index:0;pointer-events:none;opacity:.12;background:radial-gradient(circle at 20% 30%,rgba(212,175,55,.35),transparent 13%),radial-gradient(circle at 80% 20%,rgba(212,175,55,.25),transparent 12%),radial-gradient(circle at 40% 80%,rgba(212,175,55,.22),transparent 15%);}
.tarotCardsBg:after{content:"☾  THE MOON   ✦   THE STAR   ✦   THE LOVERS   ☽";position:absolute;top:38%;left:-5%;width:110%;font-size:70px;font-weight:900;letter-spacing:18px;color:#d4af37;opacity:.16;transform:rotate(-8deg);}
...
h1{font-size:52px;text-shadow:0 0 18px #d4af37,0 0 50px #8a6a00;margin:20px 0 5px}.sub{color:#f4df94;letter-spacing:3px}.topbar{display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin:20px 0}
...
<div class="panel"><h2>👑 Admin Talep Paneli</h2><div id="adminRequests">Yohanna / admin hesabıyla talepleri görebilirsin.</div></div>
</div>
...
function renderProfile(p){
    ...
    levelBadge.innerHTML=levelName(p.chips);
...
function spinWheel(){if(!account){alert('Giriş gerekli.');return}socket.emit('tarot_lucky_wheel',{account});}
...
socket.on('tarot_wheel_result',d=>{if(!d.ok){alert(d.msg);return}renderProfile(d.profile);alert('Şans çarkı: +'+d.reward+' jeton');});
