...
<style>
*{box-sizing:border-box}
body{margin:0;min-height:100vh;background:radial-gradient(circle at 50% 0%,#241506,#030303 46%,#000);color:white;font-family:Arial,Helvetica,sans-serif;overflow:hidden}
.loca{position:fixed;left:16px;top:14px;z-index:300;color:#ffd989;text-decoration:none;border:1px solid #d4af37;border-radius:13px;background:#000c;padding:11px 18px;font-weight:900;box-shadow:0 0 18px #d4af3750}
.topLobby{position:fixed;left:150px;right:16px;top:14px;height:58px;display:flex;align-items:center;gap:10px;padding:8px 12px;border:1px solid #d4af37aa;border-radius:16px;background:linear-gradient(180deg,#141414ee,#000e);z-index:250;box-shadow:0 0 22px #000}
.topLobby input,.topLobby select,.topLobby button{height:42px;margin:0;border-radius:12px;border:1px solid #d4af37;background:#050505;color:white;padding:8px 12px;font-weight:800}
.topLobby button{background:linear-gradient(180deg,#ffe08a,#a96d0d);color:#170900;cursor:pointer}
#username{width:170px}#tokenSelect{width:150px}#roomCodeInput{width:135px}.topLobby button{min-width:118px}.roomLabel{color:#d4af37;font-weight:900;white-space:nowrap}
.screen{height:100vh;padding:86px 16px 82px 16px;display:block}
.main{height:calc(100vh - 118px);display:flex;align-items:flex-start;justify-content:center;overflow:visible}
.boardWrap{position:relative;width:min(1260px,96vw);height:calc(100vh - 150px);min-height:540px;margin:0 auto;overflow:visible;filter:drop-shadow(0 35px 42px #000)}
.boardImg{width:100%;height:100%;object-fit:contain;object-position:center top;display:block;user-select:none;pointer-events:none}
.token{position:absolute;width:44px;height:44px;border-radius:50%;display:grid;place-items:center;font-size:26px;background:radial-gradient(circle at 30% 25%,#fff8,#d4af37 18%,#3b2103 55%,#030303);border:2px solid #ffe39b;box-shadow:0 0 16px rgba(0,255,102,.7),0 10px 14px #000;transition:left .22s ease,top .22s ease,transform .22s ease;z-index:90}
.token.hop{animation:pionHop .24s ease}
@keyframes pionHop{0%{transform:translateY(0) scale(1)}45%{transform:translateY(-15px) scale(1.14)}100%{transform:translateY(0) scale(1)}}
.ownerMark{position:absolute;width:46px;height:10px;border-radius:10px;border:1px solid #fff9;box-shadow:0 0 10px currentColor;z-index:75}
.ownerIcon{position:absolute;width:24px;height:24px;border-radius:50%;display:grid;place-items:center;font-size:15px;background:#000d;border:1px solid #ffe19a;z-index:76}
.house{position:absolute;font-size:22px;z-index:80}
.diceBox{position:absolute;left:50%;top:54%;transform:translate(-50%,-50%);z-index:85;width:112px;height:112px;perspective:900px;pointer-events:none}
.realDice{width:92px;height:92px;position:absolute;left:50%;top:50%;margin-left:-46px;margin-top:-46px;transform-style:preserve-3d;transform:rotateX(-22deg) rotateY(26deg);transition:transform .45s ease;filter:drop-shadow(0 18px 14px #000)}
.realDice.rolling{animation:realCubeRoll .6s linear infinite}
@keyframes realCubeRoll{0%{transform:rotateX(0deg) rotateY(0deg) rotateZ(0deg)}100%{transform:rotateX(720deg) rotateY(540deg) rotateZ(360deg)}}
.diceFace{position:absolute;width:92px;height:92px;border-radius:18px;background:radial-gradient(circle at 30% 22%,#fff,transparent 26%),linear-gradient(145deg,#fff,#e8e8e8 55%,#8a8a8a);border:3px solid white;box-shadow:inset -12px -12px 22px #0004,inset 9px 9px 18px #ffffffd0}
.front{transform:translateZ(46px)}.back{transform:rotateY(180deg) translateZ(46px)}.right{transform:rotateY(90deg) translateZ(46px)}.left{transform:rotateY(-90deg) translateZ(46px)}.top{transform:rotateX(90deg) translateZ(46px)}.bottom{transform:rotateX(-90deg) translateZ(46px)}
...
.show-1{transform:rotateX(-22deg) rotateY(26deg)}.show-2{transform:rotateX(68deg) rotateY(26deg)}.show-3{transform:rotateX(-22deg) rotateY(206deg)}.show-4{transform:rotateX(-22deg) rotateY(-64deg)}.show-5{transform:rotateX(-112deg) rotateY(26deg)}.show-6{transform:rotateX(68deg) rotateY(26deg)}
.buyPanel{position:absolute;left:50%;top:43%;transform:translate(-50%,-50%);width:420px;max-width:90%;min-height:170px;background:rgba(0,0,0,.62);border-radius:22px;display:none;z-index:130;text-align:center;padding:22px 26px}
.buyPanel .city{color:#ff9500;font-size:25px;font-weight:900;text-transform:uppercase;margin-bottom:8px}.buyPanel .question{font-size:22px;font-weight:800;margin-bottom:16px}.buyPanel .price{font-size:18px;margin-bottom:20px}.buyPanel button{width:140px;margin:0 8px;padding:12px;border-radius:12px;border:1px solid #d4af37;font-weight:900;cursor:pointer}.buyPanel button:first-of-type{background:#135d17;color:white}.buyPanel button.dark{background:#55120f;color:white}
.rollArea{position:absolute;left:50%;bottom:-66px;transform:translateX(-50%);width:320px;z-index:120}
.rollButtonOnBoard{width:300px;padding:17px 28px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(180deg,#ff5c4d,#b51512 58%,#5d0504);color:white;font-size:31px;font-weight:900;letter-spacing:1px;text-shadow:0 3px 5px #000;box-shadow:0 0 25px #ff2a2a80,0 14px 22px #000c;cursor:pointer}
.moneyToast{position:fixed;left:18px;bottom:22px;width:245px;padding:14px;border:1px solid #d4af37;border-radius:14px;background:#000d;color:#00ff66;font-weight:900;box-shadow:0 0 20px #d4af3760;z-index:200;opacity:0;transition:.25s}.moneyToast.show{opacity:1}
@media(max-width:900px){body{overflow:auto}.topLobby{position:relative;left:auto;right:auto;top:auto;height:auto;flex-wrap:wrap;margin:8px}.screen{height:auto;padding:8px 8px 120px}.main{height:auto}.boardWrap{width:98vw;height:64vw;min-height:0}.rollArea{bottom:-72px}.rollButtonOnBoard{width:230px;font-size:24px}}
</style>
</head>
<body>
...
const coords=[[88.6,64.0],[86.8,78.7],[77.9,79.3],[66.9,79.3],[56.1,79.3],[45.3,79.3],[34.7,79.3],[24.5,79.3],[12.4,78.6],[11.0,66.2],[11.0,56.0],[11.0,45.8],[11.0,35.5],[11.0,26.4],[11.0,19.0],[11.0,11.8],[16.2,7.6],[28.4,7.6],[39.4,7.6],[50.2,7.6],[59.9,7.6],[70.1,7.6],[80.2,7.6],[90.0,7.6],[90.0,20.0],[90.0,31.5],[90.0,43.4],[90.0,55.2]];
...
function detectMoneyChanges(st){Object.entries(st.players||{}).forEach(([n,p])=>{if(previousMoneyState[n]!==undefined){const diff=p.money-previousMoneyState[n];if(diff>0)showMoneyToast(n+" "+diff+"€ aldı",true);if(diff<0)showMoneyToast(n+" "+Math.abs(diff)+"€ ödedi",false)}previousMoneyState[n]=p.money})}
function addToken(p,i){const c=coords[p.position]||[88.6,64.0];const el=document.createElement("div");el.className="token";el.textContent=p.token||"🎩";el.style.left=`calc(${c[0]}% - 22px + ${i*4}px)`;el.style.top=`calc(${c[1]}% - 22px + ${i*4}px)`;boardWrap.appendChild(el)}
...
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;roomCode.textContent=st.code;topRoomLabel.textContent="Oda: "+st.code;document.querySelectorAll(".token,.house,.ownerMark,.ownerIcon").forEach(e=>e.remove());addOwnerMarks(st);Object.keys(st.players).forEach((n,i)=>addToken(st.players[n],i));if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st)}
function animateOneToken(username,from,to,state,done){const names=Object.keys(state.players);const idx=names.indexOf(username);let steps=[],cur=from,total=coords.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const el=[...document.querySelectorAll(".token")][idx];if(!el){done();return}let k=0;const timer=setInterval(()=>{if(k>=steps.length){clearInterval(timer);done();return}const c=coords[steps[k]]||[50,50];el.style.left=`calc(${c[0]}% - 22px + ${idx*4}px)`;el.style.top=`calc(${c[1]}% - 22px + ${idx*4}px)`;el.classList.remove("hop");void el.offsetWidth;el.classList.add("hop");k++},230)}
socket.on("monopoly_room_state",render);
socket.on("monopoly_error",d=>alert(d.msg));
socket.on("monopoly_animate_move",d=>{diceCube.classList.add("rolling");setTimeout(()=>{diceCube.classList.remove("rolling");showDice(d.dice);if(currentState){animateOneToken(d.username,d.from,d.to,d.state,()=>{render(d.state);if(d.pendingBuy&&d.state.players[myName]){const landed=d.state.players[myName].position;buyCity.textContent=cells[landed]||"Mülk";buyPrice.textContent="Fiyat: "+(prices[landed]||0)+" €";buyPanel.style.display="block"}})}else{render(d.state)}},650)});
