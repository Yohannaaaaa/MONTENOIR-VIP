...
function spinDailyWheel(){const u=currentMontenoirUser();if(!u){wheelResult.innerHTML="Önce giriş yap.";return;}wheelResult.innerHTML="Çark dönüyor...";fetch("/api/reward/daily",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u})}).then(r=>r.json()).then(d=>{if(!d.ok){wheelResult.innerHTML=d.msg||"Hata";return;}const prize=d.prize||30;const map={30:30,50:90,100:150,300:210,1000:270,2000:330};dailyWheel.style.transform="rotate("+(1440+(map[prize]||30))+"deg)";setTimeout(()=>{wheelResult.innerHTML="🎉 "+prize+" jeton kazandın!"},3300)}).catch(()=>wheelResult.innerHTML="Bağlantı hatası.")}
...
def add_loca_button_to_secondary_pages(response):
    try:
        ...
        if path != "/" and response.content_type and "text/html" in response.content_type:
            ...
            if "LOCA</a>" not in html and "</body>" in html:
                loca = """
<style>
#globalLocaBtn{position:fixed;top:14px;left:14px;z-index:99999;color:#d4af37;text-decoration:none;border:1px solid #d4af37;border-radius:12px;background:rgba(0,0,0,.70);padding:10px 14px;font-weight:900;font-family:Arial;box-shadow:0 0 14px rgba(212,175,55,.25)}
</style>
