...
#game3d{position:fixed;top:78px;left:0;right:320px;bottom:86px;z-index:1}
...
.playerCard{display:flex;align-items:center;gap:10px;border:1px solid #d4af3755;border-radius:13px;padding:10px;margin:8px 0;background:#ffffff08}
.playerCard.turn{border-color:#00ff66;box-shadow:0 0 16px #00ff6670}
.colorDot{width:14px;height:14px;border-radius:50%;box-shadow:0 0 10px currentColor}
...
.rollBtn{position:fixed;left:calc((100vw - 320px)/2);bottom:16px;transform:translateX(-50%);z-index:60;width:300px;height:58px;border-radius:22px;border:2px solid #ffd989;background:linear-gradient(#ff5c4d,#b51512 60%,#5d0504);color:#fff;font-size:30px;font-weight:900;text-shadow:0 3px 5px #000;box-shadow:0 0 24px #ff2a2a80,0 12px 18px #000;cursor:pointer}
.diceHelp{position:fixed;left:calc((100vw - 320px)/2 + 240px);bottom:20px;z-index:60;border:1px solid #d4af37aa;border-radius:14px;background:#000c;padding:10px 14px;color:#ffd989;font-weight:900}
...
@media(max-width:950px){
 body{overflow:auto}.topLobby{left:8px;right:8px;top:56px;height:auto;flex-wrap:wrap}.playersPanel{position:relative;top:auto;right:auto;width:auto;margin:8px}.loca{top:8px}#game3d{position:relative;top:auto;right:auto;height:65vw;min-height:420px;margin-top:150px}.rollBtn{left:50%;width:230px;font-size:24px}.diceHelp{display:none}
}
</style>
</head>
<body>
...
<div class="diceHelp">6 gelirse tekrar atarsın</div>
...
function init3D(){
 ...
 camera=new THREE.PerspectiveCamera(38,c.clientWidth/c.clientHeight,.1,100);
 camera.position.set(0,10.2,7.2);
 camera.lookAt(0,0,0);
...
function createDice(){
 const g=new THREE.Group();
 const mat=new THREE.MeshStandardMaterial({color:0xfafafa,roughness:.24,metalness:.02});
 const box=new THREE.Mesh(new THREE.BoxGeometry(.86,.86,.86),mat);
 box.castShadow=true;g.add(box);
 const dotMat=new THREE.MeshStandardMaterial({color:0x050505,roughness:.45});
 const faces=[
  [[0,0,.44],[0,0,0],[[0,0]]],
  [[0,0,-.44],[0,Math.PI,0],[[-.18,.18],[.18,-.18]]],
  [[.44,0,0],[0,Math.PI/2,0],[[-.2,.2],[0,0],[.2,-.2]]],
  [[-.44,0,0],[0,-Math.PI/2,0],[[-.2,.2],[.2,.2],[-.2,-.2],[.2,-.2]]],
  [[0,.44,0],[-Math.PI/2,0,0],[[-.22,.22],[.22,.22],[0,0],[-.22,-.22],[.22,-.22]]],
  [[0,-.44,0],[Math.PI/2,0,0],[[-.22,.24],[.22,.24],[-.22,0],[.22,0],[-.22,-.24],[.22,-.24]]]
 ];
 faces.forEach(f=>f[2].forEach(([a,b])=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.046,16,8),dotMat);dot.position.set(a,b,.495);const h=new THREE.Group();h.add(dot);h.position.set(...f[0]);h.rotation.set(...f[1]);g.add(h)}));
 return g;
}
function showDice(n){
 const r={1:[-.25,.45,0],2:[1.25,.45,0],3:[0,3.14,0],4:[0,-1.57,0],5:[-1.57,0,0],6:[1.57,0,0]}[n]||[-.25,.45,0];
 dice.rotation.set(r[0],r[1],r[2]);
}
function rollDiceAnim(n,done){
 let t=0;
 const start=dice.position.clone();
 dice.position.set(-1.2,1.25,.15);
 const id=setInterval(()=>{
   ...
   dice.rotation.z+=.42;
   dice.position.x += .055;
   dice.position.y = .72 + Math.abs(Math.sin(t/3))*0.55*(1-t/28);
   dice.position.z = .28 + Math.sin(t/5)*.18;
   if(t>28){
     clearInterval(id);
     dice.position.copy(start);
     showDice(n);
     if(done)done();
   }
 },28);
...
function render(st){
 currentState=st;roomCode=st.code;roomCodeInput.value=st.code;topRoomLabel.textContent="Oda: "+st.code;
 renderPlayersPanel(st);
 renderOwners(st);
 Object.keys(st.players).forEach((n,i)=>{const p=st.players[n];if(!players3d[n]){players3d[n]=tokenMesh(p.token,p.color);scene.add(players3d[n])}players3d[n].position.copy(pos3D(p.position,i*.08))});
 if(st.lastDice)showDice(st.lastDice);detectMoneyChanges(st);
}
function renderPlayersPanel(st){
 const names=Object.keys(st.players||{});playersList.innerHTML="";
 names.forEach((n,i)=>{
  const p=st.players[n];const d=document.createElement("div");
  d.className="playerCard"+(i===st.turnIndex?" turn":"");
  d.innerHTML=`<div class="colorDot" style="background:${p.color||"#d4af37"};color:${p.color||"#d4af37"}"></div><div class="pawnIcon">${p.token||"🎩"}</div><div><div class="playerName">${n}</div><div class="playerMoney">${p.money}€</div></div>`;
  playersList.appendChild(d);
 });
}
function renderOwners(st){Object.values(owners3d).forEach(o=>scene.remove(o));owners3d={};Object.entries(st.owners||{}).forEach(([cell,owner])=>{const p=st.players[owner];if(!p)return;const m=new THREE.Mesh(new THREE.BoxGeometry(.55,.05,.12),new THREE.MeshStandardMaterial({color:p.color||0xd4af37,metalness:.3,roughness:.35}));m.position.copy(pos3D(+cell));m.position.y=.48;m.position.z+=.33;scene.add(m);owners3d[cell]=m})}
