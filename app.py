...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Monopoly - MONTENOIR VIP</title>
<style>
*{box-sizing:border-box}
body{
  margin:0;
  background:radial-gradient(circle at top,#221704,#050505 48%,#000);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  min-height:100vh;
}
...
h1{color:#d4af37;text-align:center;margin:14px 0 22px;font-size:38px;text-shadow:0 0 18px rgba(212,175,55,.35)}
.grid{display:grid;grid-template-columns:1.4fr .8fr;gap:18px}
.board{
  border:2px solid #d4af37;border-radius:18px;padding:14px;
  background:rgba(0,0,0,.54);box-shadow:0 0 28px rgba(212,175,55,.18);
  display:grid;grid-template-columns:repeat(6,1fr);gap:8px;
}
.cell{
  min-height:74px;border:1px solid rgba(212,175,55,.38);border-radius:10px;
  background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(0,0,0,.45));
  padding:7px;font-size:11px;position:relative;overflow:hidden;
}
.cell.active{box-shadow:0 0 18px #00ff66;border-color:#00ff66}
.cell .price{color:#d4af37;font-weight:bold;margin-top:5px}
.playerToken{
  position:absolute;right:5px;bottom:5px;font-size:21px;filter:drop-shadow(0 0 6px #00ff66)
}
.panel{
  border:1px solid rgba(212,175,55,.55);border-radius:18px;background:rgba(0,0,0,.68);
  padding:18px;box-shadow:0 0 24px rgba(0,0,0,.6)
}
button{
  width:100%;border:1px solid #d4af37;border-radius:14px;
  background:linear-gradient(180deg,#f2d17a,#8d5b0d);
  color:#fff;text-shadow:0 2px 4px #000;font-weight:900;
  font-size:17px;padding:13px;margin:7px 0;cursor:pointer;
}
button.dark{background:linear-gradient(180deg,#171717,#050505)}
.dice{
  height:86px;border:1px solid rgba(212,175,55,.4);border-radius:16px;
  display:grid;place-items:center;font-size:48px;background:#070707;margin:12px 0;
  box-shadow:inset 0 0 20px rgba(212,175,55,.08)
}
.info{line-height:1.7;color:#ddd}.info b{color:#d4af37}
.players{margin-top:14px}.player{padding:9px;border-bottom:1px solid rgba(212,175,55,.18)}
.log{
  margin-top:14px;min-height:100px;max-height:190px;overflow:auto;
  background:#030303;border:1px solid rgba(0,255,102,.25);border-radius:12px;
  padding:10px;color:#00ff66;font-family:Consolas,monospace;font-size:12px
}
@media(max-width:850px){.grid{grid-template-columns:1fr}.board{grid-template-columns:repeat(3,1fr)}}
</style>
</head>
<body>
<div class="page">
  <a href="/" class="loca">🚪 LOCA</a>
  <h1>🎩 MONTENOIR MONOPOLY</h1>
  <div class="grid">
    ...
    <div class="panel">
      ...
      <div class="players">
        <h3>👥 Oyuncular</h3>
        <div class="player">👑 Yohanna — ∞ $ — OWNER</div>
        <div class="player" id="meLine">👤 Misafir — 1500 $</div>
      </div>
...
<script>
const cells=[
 ["Başlangıç",0],["Montenoir Sokak",100],["Şans",0],["Altın Cadde",140],["Vergi",-100],["Casino Bulvarı",180],
 ["Ziyaret",0],["Elmas Sokak",220],["Sandık",0],["VIP Plaza",260],["Hapishane",0],["Kraliyet Cadde",300],
 ["Ücretsiz Park",0],["Noir Avenue",340],["Şans",0],["Aurum Palace",400],["Lüks Vergi",-150],["MONTENOIR",500]
];
let pos=0,money=1500,owned=[];
const username=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Misafir";
playerName.textContent=username;meLine.textContent="👤 "+username+" — "+money+" $";
function renderBoard(){
 board.innerHTML="";
 cells.forEach((c,i)=>{
   const div=document.createElement("div");
   div.className="cell"+(i===pos?" active":"");
   div.innerHTML="<b>"+c[0]+"</b>"+(c[1]>0?'<div class=price>'+c[1]+' $</div>':"")+(i===pos?'<div class=playerToken>🎩</div>':"");
   board.appendChild(div);
 });
 positionName.textContent=cells[pos][0];moneyEl();
}
function moneyEl(){moneyElX=money;document.getElementById("money").textContent=money;meLine.textContent="👤 "+username+" — "+money+" $";}
function addLog(t){log.innerHTML=t+"<br>"+log.innerHTML}
function rollDice(){
 const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];let count=0;
 const anim=setInterval(()=>{dice.textContent=faces[Math.floor(Math.random()*6)];count++;if(count>10){clearInterval(anim);const roll=Math.floor(Math.random()*6)+1;dice.textContent=faces[roll-1]+" "+roll;move(roll)}},80);
}
function move(n){
 const old=pos;pos=(pos+n)%cells.length;if(pos<old){money+=200;addLog("Başlangıçtan geçtin: +200 $")}
 const cell=cells[pos];addLog(username+" "+n+" ilerledi: "+cell[0]);
 if(cell[1]<0){money+=cell[1];addLog("Vergi ödedin: "+cell[1]+" $")}
 if(cell[0]==="Şans"){const bonus=[50,100,-50,150][Math.floor(Math.random()*4)];money+=bonus;addLog("Şans kartı: "+bonus+" $")}
 if(cell[0]==="Sandık"){money+=100;addLog("Sandık açıldı: +100 $")}
 renderBoard();
}
function buyProperty(){
 ...
 if(money<cell[1]){addLog("Yeterli paran yok.");return}
 money-=cell[1];owned.push(pos);addLog(cell[0]+" satın alındı: -"+cell[1]+" $");renderBoard();
}
function endTurn(){addLog("Tur bitti. Sıradaki oyuncu sistemi yakında.");}
renderBoard();
</script>
</body>
</html>
