...
def mc(data):
    ...
    if not u:
        emit("monopoly_error", {"msg": "Nom requis"})
        return
    c = m_code()
    r = m_room(c, u)
...
def mr(data):
    ...
    new = (old + d) % 28
    p["position"] = new
    p["hasRolled"] = (d1 != 6 or d2 != 6)
    emit("monopoly_animate_move", {"username": u, "from": old, "to": new, "dice1": d1, "dice2": d2, "dice": d}, room=c)
...
// Add home button with language support
const homeTexts = {'en':'HOME','fr':'ACCUEIL','es':'INICIO','de':'HOME','it':'HOME','nl':'THUIS','ru':'ДОМА','ar':'الرئيسية','pt':'INICIO','pl':'STRONA GŁÓWNA','tr':'LOCA','ja':'ホーム','zh':'首页','ko':'홈'};
document.addEventListener('DOMContentLoaded', function() {
    ...
    homeBtn.textContent = homeTexts[currentLang] || 'ACCUEIL';
    document.body.appendChild(homeBtn);
    
    // Watch for language changes
    const obs = new MutationObserver(() => {
        const lang = localStorage.getItem('language') || 'fr';
        if (homeBtn && homeTexts[lang]) homeBtn.textContent = homeTexts[lang];
    });
