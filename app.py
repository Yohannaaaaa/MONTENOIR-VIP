...
.boardStage{
  position:relative;
  width:min(1280px,calc(100vw - 335px));
...
.diceCube{
  ...
  filter:drop-shadow(0 26px 20px rgba(0,0,0,.98)) drop-shadow(0 0 18px rgba(255,255,255,.22));
}
.face{
  position:absolute;
  width:96px;height:96px;
  ...
  background:
    ...
    linear-gradient(145deg,#ffffff 0%,#e9e9e9 52%,#888 100%);
  border:3px solid #fff;
  box-shadow:
    inset -12px -12px 22px rgba(0,0,0,.24),
    inset 9px 9px 16px rgba(255,255,255,.82),
...
.pip{
  ...
  box-shadow:inset 2px 2px 3px rgba(255,255,255,.12),0 1px 2px rgba(0,0,0,.5);
}
.tl{left:20px;top:20px}.tr{right:20px;top:20px}.bl{left:20px;bottom:20px}.br{right:20px;bottom:20px}.c{left:50%;top:50%;transform:translate(-50%,-50%)}.ml{left:20px;top:50%;transform:translateY(-50%)}.mr{right:20px;top:50%;transform:translateY(-50%)}
...
.roll{animation:rollCube .85s linear infinite}
@keyframes rollCube{
...
@media(max-width:900px){
  ...
  .diceCube{width:72px;height:72px}
  .face{width:72px;height:72px;border-radius:14px}
  .pip{width:12px;height:12px}.tl{left:15px;top:15px}.tr{right:15px;top:15px}.bl{left:15px;bottom:15px}.br{right:15px;bottom:15px}.ml{left:15px}.mr{right:15px}
  .front{transform:translateZ(36px)}.back{transform:rotateY(180deg) translateZ(36px)}
  .right{transform:rotateY(90deg) translateZ(36px)}.left{transform:rotateY(-90deg) translateZ(36px)}
  .top{transform:rotateX(90deg) translateZ(36px)}.bottom{transform:rotateX(-90deg) translateZ(36px)}
}
</style>
</head>
<body>
...
<div class="screen">
      ...
      <div class="boardWrap" id="boardWrap">
        ...
        <div class="diceBox">
          <div id="diceCube" class="diceCube">
            <div class="face front" id="diceFace"><span class="pip c"></span></div>
            <div class="face back"><span class="pip tl"></span><span class="pip c"></span><span class="pip br"></span></div>
            <div class="face right"><span class="pip tl"></span><span class="pip tr"></span><span class="pip bl"></span><span class="pip br"></span></div>
            <div class="face left"><span class="pip tl"></span><span class="pip tr"></span><span class="pip c"></span><span class="pip bl"></span><span class="pip br"></span></div>
            <div class="face top"><span class="pip tl"></span><span class="pip tr"></span><span class="pip ml"></span><span class="pip mr"></span><span class="pip bl"></span><span class="pip br"></span></div>
            <div class="face bottom"><span class="pip tl"></span><span class="pip br"></span></div>
          </div>
        </div>
  ...
  <div class="side">
    ...
    <h2>Log</h2>
    <div id="log" class="log">Hazır.</div>
  </div>
</div>
...
const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];
const rotations={1:"rotateX(-18deg) rotateY(25deg)",2:"rotateX(90deg) rotateY(25deg)",3:"rotateX(-18deg) rotateY(205deg)",4:"rotateX(-18deg) rotateY(-65deg)",5:"rotateX(-108deg) rotateY(25deg)",6:"rotateX(72deg) rotateY(25deg)"};

// Koordinatlar resmin üzerinde kartların merkezlerine göre ayarlandı.
const coords=[
[86,77],[77,77],[68,77],[59,77],[50,77],[41,77],[32,77],[23,77],[14,77],
[11,66],[11,55],[11,44],[11,33],[11,22],
[20,19],[30,19],[40,19],[50,19],[60,19],[70,19],[80,19],[88,19],
[89,31],[89,43],[89,55],[89,67],[88,77],
[79,77],[70,77],[61,77]
];
const cells=["DÉPART","Sofia","Belgrad","Gare","Istanbul","Prague","Budapest","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo","ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Milan","Eaux","Amsterdam","Moscou","Rome","Madrid","Impôts"];
...
function render(st){
 ...
 document.querySelectorAll(".token,.house").forEach(e=>e.remove());
 ...
 names.forEach((n,i)=>{
   ...
   d.innerHTML=`<div class="picon">${p.token}</div><div><b>${n}</b><br>${p.money}€ ${p.bankrupt?"— İFLAS":""}</div>`;
 ...
 Object.entries(st.houses||{}).forEach(([cell,n])=>{
   ...
   for(let i=0;i<n;i++){
     ...
     h.className="house";h.textContent="🏠";
 ...
 logMsg(st.lastLog||"");
 if(st.lastDice){
   diceCube.style.transform=rotations[st.lastDice]||rotations[1];
 }
 renderHouses(st);
}
function addToken(p,i){
 ...
 el.style.left=`calc(${c[0]}% - 27px + ${i*7}px)`;
 el.style.top=`calc(${c[1]}% - 27px + ${i*7}px)`;
...
function animateOneToken(username, from, to, state, done){
 ...
 const timer=setInterval(()=>{
   ...
   tokenEl.style.left=`calc(${c[0]}% - 27px + ${idx*7}px)`;
   tokenEl.style.top=`calc(${c[1]}% - 27px + ${idx*7}px)`;
   k++;
 },260);
}
