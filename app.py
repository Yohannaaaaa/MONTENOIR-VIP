...
function toggleCollapse(id){const el=document.getElementById(id);if(el)el.classList.toggle('hidden');}
...
function chipKey(n){return 'codenamesChips_'+(n||'guest')}function getSavedChips(n){let v=localStorage.getItem(chipKey(n));if(v===null)return 1000;let x=parseInt(v);return isNaN(x)?1000:x}function setSavedChips(n,a){localStorage.setItem(chipKey(n),String(a))}function saveLocalProfile(){localStorage.setItem('codenamesRoom',roomCode);localStorage.setItem('codenamesName',myName);localStorage.setItem('codenamesRole',myRole);localStorage.setItem('codenamesTeam',myTeam)}function restoreLocalFields(){let r=localStorage.getItem('codenamesRoom')||'',n=localStorage.getItem('codenamesName')||'',ro=localStorage.getItem('codenamesRole')||'',t=localStorage.getItem('codenamesTeam')||'';if(r)roomInput.value=r;if(n)playerName.value=n;if(ro)roleChoice.value=ro;if(t)teamChoice.value=t}function copyRoomLink(){if(!roomCode)return;const link=location.origin+'/codenames?room='+roomCode;const done=()=>{copyLinkBtn.innerHTML='✅ Kopyalandı!';setTimeout(()=>copyLinkBtn.innerHTML='🔗 Davet Linkini Kopyala',1800)};if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(link).then(done).catch(()=>prompt('Linki kopyala:',link))}else{prompt('Linki kopyala:',link)}}
...
function tryAutoJoinLinkRoom(){
    if(!linkRoom || roomCode || !currentAccount) return;
    pendingAutoSit=true;
    socket.emit('join_room_code',{room:linkRoom,account:currentAccount,token:mySessionToken()});
}
...
function demoBuyChips(amount, provider){
    ...
    socket.emit('buy_virtual_chips',{room:roomCode,amount:amount,type:'paid'});
}
...
socket.on('connect',()=>{
    ...
    linkRoom=(params.get('room')||'').trim().toUpperCase();
    ...
    if(linkRoom && currentAccount){
        tryAutoJoinLinkRoom();
    }else if(savedRoom && savedName){
        pendingAutoSit=true;
        if(currentAccount){socket.emit('join_room_code',{room:savedRoom,account:currentAccount,token:mySessionToken()});}
    }
});socket.on('room_created',d=>{roomCode=d.room;isAdmin=true;roomText.innerHTML='Oda: '+roomCode+' 👑 Admin sensin';copyLinkBtn.classList.remove('hidden');saveLocalProfile();if(pendingSit){pendingSit=false;setTimeout(()=>sitAtTable(),250)}});socket.on('room_joined',d=>{roomCode=d.room;isAdmin=false;roomText.innerHTML='Oda: '+roomCode;copyLinkBtn.classList.remove('hidden');saveLocalProfile();if(pendingAutoSit){pendingAutoSit=false;setTimeout(()=>sitAtTable(),250)}});socket.on('error_msg',d=>{soundError();alert(d.msg)});socket.on('players_update',d=>{if(d.micStates) currentMicStates=d.micStates;if(d.ready) currentReady=d.ready;renderReady(d.players,d.ready||{});renderPlayers(d.players,d.locks)});socket.on('game_update',d=>{
...
def create_room(data):
    ...
    code = room_code()
    rooms[code] = {'players': [], 'game': new_game('default'), 'stats': {'blueWins':0,'redWins':0,'history':[],'wordHistory':[],'betHistory':[],'gameNo':0}, 'locks': {'blue':False,'red':False}, 'adminSid': request.sid, 'bets': {}, 'micStates': {}, 'ready': {}, 'teamChat': {'blue': [], 'red': []}, 'dm': {}, 'category': 'default', 'vipRoom': vip_room}
...
def buy_virtual_chips(data):
    ...
    if player:
        current_chips = int(player.get('chips', 1000))
        if chip_type == 'bonus' and current_chips <= 0:
            emit('error_msg', {'msg': 'Jetonu bitti! Sadece paralı jeton alabilirsin.'})
            return
        player['chips'] = current_chips + amount
...
const TAROT_DECK=[
 {num:'0',name:'Deli',symbol:'🃏',meaning:'Yeni başlangıçlar, cesaret ve bilinmeyene adım atma zamanı.',image:'00_joker.webp'},
 {num:'I',name:'Büyücü',symbol:'🪄',meaning:'Elindeki güçleri fark et, iradenle istediğini yaratabilirsin.',image:'01_sihirbaz.webp'},
 {num:'II',name:'Azize',symbol:'🌙',meaning:'İçsel bilgeliğine güven, sezgilerin sana doğru yolu gösteriyor.',image:'02_bakireyiz.webp'},
 {num:'III',name:'İmparatoriçe',symbol:'👑',meaning:'Bolluk, bereket ve yaratıcılık enerjisi çevreni sarıyor.',image:'03_imparatorice.webp'},
 {num:'IV',name:'İmparator',symbol:'⚔️',meaning:'Disiplin ve yapı kurma zamanı, kontrolü eline al.',image:'04_imparator.webp'},
 {num:'V',name:'Aziz',symbol:'📿',meaning:'Gelenek, rehberlik ve manevi öğretilerden güç al.',image:'05_basrahip.webp'},
 {num:'VI',name:'Aşıklar',symbol:'❤️',meaning:'Önemli bir seçim ya da ilişki senin için dönüm noktası olacak.',image:'06_asiklar.webp'},
 {num:'VII',name:'Savaş Arabası',symbol:'🏇',meaning:'Kararlılıkla ilerle, zafer disiplinli irade ile gelir.',image:'07_savas_arabasi.webp'},
 {num:'VIII',name:'Güç',symbol:'🦁',meaning:'İç gücün ve sabrınla zorlukların üstesinden geliyorsun.',image:'08_guc.webp'},
 {num:'IX',name:'Ermiş',symbol:'🕯️',meaning:'Yalnız kalıp içine dönme, cevaplar sessizlikte saklı.',image:'09_munzevi.webp'},
 {num:'X',name:'Kader Çarkı',symbol:'☸️',meaning:'Değişim kapıda, akışa güven, döngü senin lehine dönüyor.',image:'10_kader_carki.webp'},
 {num:'XI',name:'Adalet',symbol:'⚖️',meaning:'Denge ve doğruluk zamanı, yaptıkların karşılığını buluyor.',image:'11_adalet.webp'},
 {num:'XII',name:'Asılan Adam',symbol:'🙃',meaning:'Bakış açını değiştir, teslimiyet yeni bir anlayış getirir.',image:'12_asili_adam.webp'},
 {num:'XIII',name:'Ölüm',symbol:'🦋',meaning:'Bir dönem kapanıyor, dönüşüm ve yeniden doğuş yakında.',image:'13_olum.webp'},
 {num:'XIV',name:'Denge',symbol:'🌊',meaning:'Uyum ve ölçülülük ara, aşırılıklardan kaçın.',image:'14_itidal.webp'},
 {num:'XV',name:'Şeytan',symbol:'⛓️',meaning:'Bağımlılıklar ya da korkular seni tutsak ediyor olabilir, farkındalık özgürleştirir.',image:'15_iblis.webp'},
 {num:'XVI',name:'Kule',symbol:'🌩️',meaning:'Ani bir değişim ya da sarsıntı, aslında seni gerçeğe uyandırıyor.',image:'16_kule.webp'},
 {num:'XVII',name:'Yıldız',symbol:'⭐',meaning:'Umut ve şifa zamanı, evren senin için bir yol açıyor.',image:'17_yildiz.webp'},
 {num:'XVIII',name:'Ay',symbol:'🌕',meaning:'Belirsizlik ve gizli duygular var, sezgilerine kulak ver.',image:'18_ay.webp'},
 {num:'XIX',name:'Güneş',symbol:'☀️',meaning:'Mutluluk, başarı ve netlik yakında hayatına giriyor.',image:'19_gunes.webp'},
...
def group_cells_for(group):
    return [i for i,c in enumerate(METROPOLY_CELLS) if c[1]=="property" and c[4]==group]

def owns_full_group(room, username, group):
    cells=group_cells_for(group)
...
def monopoly_add_house(data):
    ...
    try:
        ...
        return
    if idx<0 or idx>=len(METROPOLY_CELLS):
        return
...
@app.route("/", endpoint="force_accueil_couverture_luxe")
@app.route("/index", endpoint="force_accueil_couverture_luxe_index")
@app.route("/home", endpoint="force_accueil_couverture_luxe_home")
def force_accueil_couverture_luxe():
    return render_template("index.html")

@app.route("/games", endpoint="force_games_to_metropoly")
