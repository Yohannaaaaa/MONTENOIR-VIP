...
def tarot_get_profile(data):
    ...
    key = find_user_key(users, account)
    if not key:
        key = ensure_user_account(account)
        users = load_users()
...
<body><div class="tarotCardsBg"></div><div class="stars"></div><div class="fog"></div><div class="wrap">
<h1>🔮 Tarot & Rituel</h1><div class="sub">Siyah kadife · Altın sır · Mum ışığı</div><div class="candle">🕯️ 🕯️ 🕯️</div>
<div class="topbar"><button class="btn" onclick="location.href='/'">♠ Codenames'e Dön</button><button class="btn" onclick="loadProfile()">👤 Profili Yenile</button><button class="btn" onclick="spinWheel()">🎡 Şans Çarkı</button><button class="btn" id="tarotOwnerBtn" style="display:none" onclick="loadAdminRequests()">👑 Admin Talep Paneli</button></div>
...
<div id="tarotConnectBox" class="panel" style="text-align:center;display:none">
  <h2>👤 Profil Bağla</h2>
  <p style="color:#f4df94">Tarot ödülleri ve jetonlar için önce Codenames profilini bağla.</p>
  <input id="tarotLoginName" placeholder="Codenames kullanıcı adın">
  <button class="btn" onclick="connectTarotProfile()">Profili Bağla</button>
  <button class="btn" onclick="location.href='/'">Codenames'e Dönüp Giriş Yap</button>
</div>


<div id="tarotProfileCard" class="panel" style="margin-bottom:15px;border:2px solid #d4af37;">
<h2>👤 PROFİL</h2>
<div id="tarotProfileBox">
<div><b>Kullanıcı:</b> <span id="tpUser">Bağlanmadı</span></div>
<div><b>Jeton:</b> 🪙 <span id="tpChips">0</span></div>
<div><b>Üyelik:</b> <span id="tpMembership">Yok</span></div>
</div>
</div>
...
account=p.username;
tarotConnectBox.style.display='none';
localStorage.setItem('codenamesAccount',account);
localStorage.setItem('loggedUser',account);
localStorage.setItem('loggedIn','true');
if(document.getElementById('tpUser')){
tpUser.textContent=p.username||'';
tpChips.textContent=(p.chips||0);
if((p.username||'').toLowerCase()==='yohanna'){
tpUser.innerHTML='👑 YOHANNA';
tpMembership.innerHTML='💎 OWNER';
tpChips.innerHTML='∞';
}else{
tpMembership.textContent=(p.membershipLabel||p.membershipLevel||'Yok');
}
}
const owner=(p.username||'').toLowerCase()==='yohanna';userName.innerHTML=p.username;userName.className=owner?'ownerName':'';ownerBadge.innerHTML=owner?'<span class="ownerBadge">👑 OWNER</span>':'';chips.innerHTML=owner?'∞':p.chips;levelBadge.innerHTML=owner?'👑 OWNER':(p.membershipLabel||levelName(p.chips));avatarImg.src=p.avatarData||defaultAvatarData(p.avatar||'woman.png');tarotOwnerBtn.style.display=owner?'inline-block':'none';tarotAdminPanel.style.display=owner?'block':'none';saveSharedProfile(p);}
function resolveTarotAccount(){
    let a = localStorage.getItem('codenamesAccount') || localStorage.getItem('loggedUser') || account || '';
    if(!a){
        try{
            const p = JSON.parse(localStorage.getItem('codenamesProfile') || '{}');
            if(p && p.username) a = p.username;
        }catch(e){}
    }
    account = a || '';
    return account;
}
function connectTarotProfile(){
    const v = (tarotLoginName.value || '').trim();
    if(!v){alert('Kullanıcı adını yaz.');return}
    account = v;
    localStorage.setItem('codenamesAccount', account);
    localStorage.setItem('loggedUser', account);
    localStorage.setItem('loggedIn','true');
...
function loadProfile(){
    resolveTarotAccount();
    if(!account){
        tarotConnectBox.style.display='block';
        userName.innerHTML='Profil bağlı değil';
        chips.innerHTML='0';
        levelBadge.innerHTML='Profil gerekli';
        return;
    }
    tarotConnectBox.style.display='none';
    socket.emit('tarot_get_profile',{account});
...
function submitTarot(){resolveTarotAccount();if(!account){tarotConnectBox.style.display='block';alert('Giriş gerekli.');return}if(!selectedService){alert('Önce hizmet seç.');return}socket.emit('tarot_submit_request',{account,service:selectedService,name:clientName.value,motherName:motherName.value,birthDate:birthDate.value,question:question.value,photoNote:photoNote.value});}
function spinWheel(){resolveTarotAccount();if(!account){tarotConnectBox.style.display='block';alert('Ödül almak için önce profilini bağla.');return}const w=document.getElementById('luckyWheel');if(w){w.style.transform='rotate('+(1440+Math.floor(Math.random()*720))+'deg)';}setTimeout(()=>socket.emit('tarot_lucky_wheel',{account}),900);}
function loadAdminRequests(){socket.emit('tarot_get_requests',{account});}
function sendTarotChat(){resolveTarotAccount();if(!account){tarotConnectBox.style.display='block';alert('Giriş gerekli.');return}let msg=tarotChatInput.value.trim();if(!msg)return;socket.emit('tarot_chat',{account,msg});tarotChatInput.value='';}
...
function ownerSetMembership(){
    const target = ownerTarget();
    if(!isOwnerAccount()){alert('Bu işlem sadece Yohanna içindir.');return}
    if(!target){alert('Önce kullanıcı adını yaz veya kullanıcı listesinden seç.');return}
    if(ownerPanelResult) ownerPanelResult.innerHTML='Üyelik uygulanıyor...';
    socket.emit('owner_set_membership',{
        owner:currentAccount,
        target:target,
        level:ownerMembershipLevel.value,
        duration:ownerMembershipDuration.value
    });
}
...
socket.on('owner_panel_data',d=>{if(!d.ok){alert(d.msg);return}ownerStats.innerHTML='Toplam kullanıcı: '+d.stats.totalUsers+'<br>Online kullanıcı: '+d.stats.onlineUsers+'<br>Bugünkü kayıt: '+d.stats.todayUsers+'<br>Satılan jeton: '+d.stats.soldChips+'<br>Tamamlanan tarot: '+d.stats.completedTarot+'<br>Tamamlanan ritüel: '+d.stats.completedRitual;ownerUserList.innerHTML='<h3>👥 Kullanıcılar</h3>'+d.users.map(u=>'<div class="profileCard" style="cursor:pointer" data-user="'+u.username+'"><b>'+u.username+'</b> · 🪙 '+u.chips+' · '+(u.membershipLabel||'Üyelik yok')+'</div>').join('');document.querySelectorAll('#ownerUserList .profileCard').forEach(el=>el.onclick=()=>{ownerTargetUser.value=el.dataset.user;});if(d.settings){setMaintenance.checked=!!d.settings.maintenance;setRegistrations.checked=!!d.settings.registrations;setWheel.checked=!!d.settings.wheel;setTarot.checked=!!d.settings.tarot;setRitual.checked=!!d.settings.ritual;ownerWheelLimit.value=String(d.settings.wheel?d.settings.wheelDailyLimit||1:0);}});
...
/* === HARD BUTTON CLICK FIX MAIN === */
function hardBindButtonsMain(){
    const bind = (id, fn) => {
...
document.addEventListener('DOMContentLoaded', hardBindButtonsMain);
setTimeout(hardBindButtonsMain,300);
setTimeout(hardBindButtonsMain,1200);


socket.on('owner_set_membership_result',d=>{
    if(ownerPanelResult) ownerPanelResult.innerHTML=d.msg;
    if(!d.ok) alert(d.msg);
    ownerLoadPanel();
});
</script></body></html>
