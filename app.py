...
<title>🏛️ LOCA - MONTENOIR VIP</title>
<script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script>
<style>
...
fr:["🎮 JEUX","🔮 TAROT & RITUEL","👤 ABONNEMENT / CONNEXION","🏆 PROFIL / XP","🪙 CAISSE MONTENOIR","☰ MENU","⚙️ PARAMÈTRES","🎟️ Tournois","🎨 Personnalisation profil","🎁 Coffres","🏆 Succès","👥 Amis","🤖 Tarot IA Premium","📜 Règles des jeux","Acheter","Ouvrir","🚪 LOCA"],
...
# ===== MONTENOIR OWNER / REWARDS / AVATAR =====
...
@app.route("/kasa")
def kasa_page():
    return """<html><head><meta charset='utf-8'><style>body{background:#050505;color:#fff;font-family:Arial;padding:30px}.locaBtn{display:inline-flex;align-items:center;gap:8px;margin-bottom:12px;color:#d4af37;border:1px solid #d4af37;padding:10px 14px;border-radius:12px;text-decoration:none;background:rgba(0,0,0,.70)}</style></head><body><a class='locaBtn' href='/'>🚪 LOCA</a><h1>🪙 MONTENOIR Kasası</h1><p>Jeton paketleri ve ödeme sistemi burada olacak.</p></body></html>"""
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>MONTENOIR Monopoly 3D</title>
<style>
*{box-sizing:border-box}
body{
  margin:0;
  min-height:100vh;
  background:radial-gradient(circle at top,#2b1b04,#050505 48%,#000);
  color:#fff;
  font-family:Arial,Helvetica,sans-serif;
  overflow-x:hidden;
}
.page{max-width:1250px;margin:0 auto;padding:22px}
.loca{
  display:inline-flex;align-items:center;gap:8px;
  color:#d4af37;text-decoration:none;border:1px solid #d4af37;
  background:rgba(0,0,0,.72);border-radius:14px;padding:10px 16px;
  font-weight:900;box-shadow:0 0 18px rgba(212,175,55,.25)
}
h1{text-align:center;color:#d4af37;text-shadow:0 0 20px rgba(212,175,55,.5);margin:8px 0 18px}
.game{
  display:grid;
...
.board3d{
  ...
  border:4px solid #d4af37;
  border-radius:28px;
  box-shadow:0 35px 60px rgba(0,0,0,.75),0 0 35px rgba(212,175,55,.35),inset 0 0 35px rgba(212,175,55,.08);
...
.center{
  ...
  border-radius:22px;
  background:
    radial-gradient(circle at center,rgba(212,175,55,.18),rgba(0,0,0,.7) 58%),
    linear-gradient(135deg,#151515,#040404);
  display:flex;
  flex-direction:column;
  align-items:center;
  justify-content:center;
...
.center .brand{font-size:46px;color:#d4af37;font-weight:900;text-shadow:0 0 20px rgba(212,175,55,.55)}
.center .sub{font-size:20px;letter-spacing:3px;color:#fff}
.cell{
  position:absolute;
  width:92px;
  height:92px;
  ...
  font-size:10px;
  color:#fff;
  box-shadow:inset 0 0 12px rgba(0,0,0,.7),0 6px 10px rgba(0,0,0,.45);
  transform:translateZ(12px);
  overflow:hidden;
}
.cell .bar{height:14px;border-radius:8px;margin:-2px -2px 4px}
.brown{background:#8b4513}.sky{background:#45b7ff}.pink{background:#ff4fb3}.orange{background:#ff9c2e}.red{background:#e33131}.yellow{background:#f7d348}.green{background:#33c46b}.blue{background:#3167ff}.gold{background:#d4af37}
.cell .name{font-weight:900;line-height:1.05}.cell .price{color:#d4af37;font-weight:900;margin-top:4px}
.house{position:absolute;bottom:4px;left:5px;font-size:15px}
.token{
  position:absolute;
  right:4px;
  bottom:2px;
...
button{
  ...
  border:1px solid #d4af37;
  border-radius:16px;
  background:linear-gradient(180deg,#f5d47b,#8b580c);
  color:#fff;
  font-weight:900;
  padding:13px;
  margin:7px 0;
...
button.dark{background:linear-gradient(180deg,#171717,#050505)}
.info{line-height:1.7}.info b{color:#d4af37}
.log{
  margin-top:12px;
  min-height:140px;
  max-height:220px;
...
@media(max-width:950px){
  ...
  .board3d{transform:rotateX(38deg) rotateZ(-2deg)}
}
</style>
</head>
<body>
...
<h1>🎩 MONTENOIR MONOPOLY 3D</h1>

<div class="game">
  <div class="board3dWrap">
    <div class="board3d" id="board">
      <div class="center">
        <div class="brand">MONTENOIR</div>
        <div class="sub">VIP MONOPOLY</div>
  ...
  <div class="panel">
    <h2>🎲 Partie test</h2>
    <div class="info">
      <div><b>Oyuncu:</b> <span id="playerName">Misafir</span></div>
      <div><b>Para:</b> <span id="money">1500</span> $</div>
      <div><b>Pozisyon:</b> <span id="positionName">Başlangıç</span></div>
      <div><b>Mülk:</b> <span id="ownedCount">0</span></div>
    </div>
    <div class="dice" id="dice">🎲</div>
    <button onclick="rollDice()">🎲 LANCER LE DÉ</button>
    <button class="dark" onclick="buyProperty()">🏠 SATIN AL</button>
    <button class="dark" onclick="endTurn()">🔄 TURU BİTİR</button>
    <div class="log" id="log">Sistem hazır. Zar at.</div>
  </div>
</div>
</div>
...
<script>
const cells=[
 ["Başlangıç",0,"gold"],["Montenoir Sokak",100,"brown"],["Şans",0,"gold"],["Altın Cadde",140,"brown"],["Vergi",-100,"red"],["Casino Bulvarı",180,"sky"],
 ["Ziyaret",0,"gold"],["Elmas Sokak",220,"sky"],["Sandık",0,"gold"],["VIP Plaza",260,"pink"],["Hapishane",0,"red"],["Kraliyet Cadde",300,"pink"],
 ["Ücretsiz Park",0,"gold"],["Noir Avenue",340,"orange"],["Şans",0,"gold"],["Aurum Palace",400,"orange"],["Lüks Vergi",-150,"red"],["Saray Yolu",420,"red"],
 ["Montenoir Garı",200,"yellow"],["Safir Cadde",460,"yellow"],["Crown Plaza",500,"green"],["Diamond Tower",550,"green"],["MONTENOIR",650,"blue"],["Final",0,"gold"]
];
let pos=0,money=1500,owned=[];
const username=localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Misafir";
playerName.textContent=username;

function placeCell(i,el){
 const n=cells.length;
 const size=92,gap=6;
 const boardSize=document.getElementById("board").clientWidth;
 const margin=18;
 const max=boardSize-margin-size;
 let x=margin,y=margin;
 if(i<7){x=max-i*(size+gap);y=max}
 else if(i<13){x=margin;y=max-(i-6)*(size+gap)}
 else if(i<19){x=margin+(i-12)*(size+gap);y=margin}
 else{x=max;y=margin+(i-18)*(size+gap)}
...
function renderBoard(){
 ...
 cells.forEach((c,i)=>{
   ...
   div.className="cell";
   div.innerHTML='<div class="bar '+c[2]+'"></div><div class="name">'+c[0]+'</div>'+(c[1]>0?'<div class="price">'+c[1]+' $</div>':'')+(owned.includes(i)?'<div class="house">🏠</div>':'')+(i===pos?'<div class="token">🎩</div>':'');
   board.appendChild(div);placeCell(i,div);
 });
 positionName.textContent=cells[pos][0];
 money.textContent=window.money;
 ownedCount.textContent=owned.length;
}
function addLog(t){log.innerHTML=t+"<br>"+log.innerHTML}
function rollDice(){
 const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];let count=0;
 const anim=setInterval(()=>{
  dice.textContent=faces[Math.floor(Math.random()*6)];
  count++;
  if(count>12){clearInterval(anim);const roll=Math.floor(Math.random()*6)+1;dice.textContent=faces[roll-1]+" "+roll;move(roll)}
 },70);
}
function move(n){
 const old=pos;pos=(pos+n)%cells.length;
 if(pos<old){window.money+=200;addLog("Başlangıçtan geçtin: +200 $")}
 const cell=cells[pos];addLog(username+" "+n+" ilerledi: "+cell[0]);
 if(cell[1]<0){window.money+=cell[1];addLog("Vergi: "+cell[1]+" $")}
 if(cell[0]==="Şans"){const b=[50,100,-50,150][Math.floor(Math.random()*4)];window.money+=b;addLog("Şans kartı: "+b+" $")}
 if(cell[0]==="Sandık"){window.money+=100;addLog("Sandık: +100 $")}
 renderBoard();
}
function buyProperty(){
 const cell=cells[pos];
 if(cell[1]<=0){addLog("Bu kare satın alınamaz.");return}
 if(owned.includes(pos)){addLog("Bu mülk zaten sende.");return}
 if(window.money<cell[1]){addLog("Yeterli paran yok.");return}
 window.money-=cell[1];owned.push(pos);addLog("🏠 "+cell[0]+" satın alındı: -"+cell[1]+" $");renderBoard();
}
function endTurn(){addLog("Tur bitti. Online çok oyunculu sistem sonraki adım.");}
window.money=1500;
renderBoard();
