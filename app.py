...
def mc(data):
    ...
    MONOPOLY_ROOMS[c]=r
    join_room(c)
    r["players"][u]={"name":u,"token":t,"money":START_MONEY,"position":0,"jailed":0,"bankrupt":False}
...
@app.route("/monopoly")
def monopoly_page():
    return r"""<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Monopoly Online</title><script src="https://cdn.socket.io/4.7.5/socket.io.min.js"></script><style>
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 50% 0%,#2a1805 0%,#050301 42%,#000 100%);color:white;font-family:Arial;overflow:hidden}.screen{height:100vh;display:grid;grid-template-columns:minmax(0,1fr) 290px;gap:14px;padding:14px}.main{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden}.boardWrap{position:relative;width:min(1320px,calc(100vw - 330px));aspect-ratio:16/10;filter:drop-shadow(0 38px 48px rgba(0,0,0,.95));transform:translateX(-6px) perspective(1100px) rotateX(1.5deg);border-radius:28px}.boardImg{width:100%;height:100%;object-fit:contain;object-position:center center;display:block;border-radius:26px;box-shadow:inset 0 0 40px rgba(212,175,55,.2),0 0 45px rgba(212,175,55,.22)}.token{position:absolute;width:48px;height:48px;border-radius:50%;display:grid;place-items:center;font-size:29px;background:radial-gradient(circle at 30% 25%,#fff8,#d4af37 12%,#4b2d05 45%,#050505 85%);border:2px solid #ffe39b;box-shadow:0 0 18px rgba(0,255,102,.75),0 12px 18px rgba(0,0,0,.85),inset 0 0 10px rgba(255,255,255,.22);transition:left .28s ease,top .28s ease,transform .28s ease;z-index:6}.token.moving{transform:scale(1.28) translateY(-8px)}.house{position:absolute;font-size:23px;z-index:5;filter:drop-shadow(0 0 8px rgba(212,175,55,.8)) drop-shadow(0 8px 8px #000)}.diceBox{position:absolute;left:50%;top:52%;transform:translate(-50%,-50%);z-index:20;perspective:850px}.diceCube{width:104px;height:104px;position:relative;transform-style:preserve-3d;transform:rotateX(-18deg) rotateY(25deg);transition:transform .45s ease;filter:drop-shadow(0 26px 18px rgba(0,0,0,.95)) drop-shadow(0 0 18px rgba(255,255,255,.22))}.face{position:absolute;width:104px;height:104px;border-radius:20px;background:linear-gradient(145deg,#ffffff,#e9e9e9 52%,#777);border:3px solid #fff;color:#1f1200;display:grid;place-items:center;font-size:63px;font-weight:900;box-shadow:inset -13px -13px 20px rgba(0,0,0,.24),inset 9px 9px 16px rgba(255,255,255,.82),0 0 16px rgba(255,255,255,.16)}.front{transform:translateZ(52px)}.back{transform:rotateY(180deg) translateZ(52px)}.right{transform:rotateY(90deg) translateZ(52px)}.left{transform:rotateY(-90deg) translateZ(52px)}.top{transform:rotateX(90deg) translateZ(52px)}.bottom{transform:rotateX(-90deg) translateZ(52px)}.roll{animation:rollCube .85s linear infinite}@keyframes rollCube{0%{transform:rotateX(0deg) rotateY(0deg) rotateZ(0deg)}100%{transform:rotateX(360deg) rotateY(540deg) rotateZ(360deg)}}.side{background:linear-gradient(180deg,rgba(12,12,12,.92),rgba(0,0,0,.88));border:1px solid rgba(212,175,55,.72);border-radius:20px;padding:14px;overflow:auto;box-shadow:0 0 35px rgba(0,0,0,.85),inset 0 0 18px rgba(212,175,55,.08)}h2{color:#d4af37;margin:8px 0}input,select,button{width:100%;padding:12px;border-radius:14px;border:1px solid #d4af37;margin:6px 0;box-shadow:0 0 16px rgba(212,175,55,.18)}input,select{background:#050505;color:white}button{background:linear-gradient(180deg,#f9df8c,#94600e);font-weight:900;cursor:pointer;color:#120900;text-shadow:0 1px 0 rgba(255,255,255,.35)}button.dark{background:linear-gradient(#171717,#050505);color:white}.player{display:flex;gap:10px;align-items:center;border:1px solid rgba(212,175,55,.38);border-radius:14px;padding:10px;margin:7px 0;background:linear-gradient(180deg,rgba(255,255,255,.055),rgba(0,0,0,.45));box-shadow:inset 0 0 10px rgba(212,175,55,.05)}.turn{box-shadow:0 0 16px #00ff66;border-color:#00ff66}.picon{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;border:1px solid #ffe39b;background:radial-gradient(circle,#d4af37,#100800);font-size:22px;box-shadow:0 0 12px rgba(212,175,55,.35)}.log{font-family:Consolas;color:#00ff66;background:#020202;border:1px solid #0f63;border-radius:12px;padding:9px;height:120px;overflow:auto;font-size:12px}.loca{position:absolute;left:18px;top:18px;color:#d4af37;text-decoration:none;border:1px solid #d4af37;border-radius:12px;background:#000c;padding:10px 14px;font-weight:900;z-index:20}.buyPanel{position:absolute;left:50%;bottom:32px;transform:translateX(-50%);background:#000d;border:1px solid #d4af37;border-radius:18px;padding:12px;display:none;z-index:10}.houseBtn{width:auto;padding:8px 10px}
@media(max-width:900px){body{overflow:auto}.screen{height:auto;grid-template-columns:1fr}.boardWrap{width:98vw;transform:none}.token{width:34px;height:34px;font-size:22px}.diceCube{width:70px;height:70px}.face{width:70px;height:70px;font-size:42px}.front{transform:translateZ(35px)}.back{transform:rotateY(180deg) translateZ(35px)}.right{transform:rotateY(90deg) translateZ(35px)}.left{transform:rotateY(-90deg) translateZ(35px)}.top{transform:rotateX(90deg) translateZ(35px)}.bottom{transform:rotateX(-90deg) translateZ(35px)}}
</style></head><body><a class="loca" href="/">🚪 LOCA</a><div class="screen"><div class="main"><div class="boardWrap" id="boardWrap"><img class="boardImg" src="/static/monopoly_board_lux_3d.png"><div class="diceBox">
  <div id="diceCube" class="diceCube">
    <div class="face front" id="diceFace">⚀</div>
...
</div><div id="buyPanel" class="buyPanel"><div id="buyText">Satın al?</div><button onclick="buyProperty()">SATIN AL</button><button class="dark" onclick="hideBuy()">PAS GEÇ</button></div></div></div><div class="side"><h2>🎩 Monopoly Online</h2><input id="username" placeholder="Kullanıcı adı"><select id="tokenSelect"><option>✈️</option><option>🎰</option><option>🛥️</option><option>🚗</option><option>🎩</option><option>🐈</option></select><button onclick="createRoom()">ODA OLUŞTUR</button><input id="roomCodeInput" placeholder="MON-1234"><button class="dark" onclick="joinRoom()">ODAYA GİR</button><button onclick="startGame()">OYUNU BAŞLAT</button><button onclick="rollDice()">🎲 ZAR AT</button><button class="dark" onclick="endTurn()">TURU BİTİR</button><h2>Oda: <span id="roomCode">-</span></h2><div id="players"></div><h2>🏠 Evler</h2><div id="housesPanel"></div><h2>Log</h2><div id="log" class="log">Hazır.</div></div></div><script>
const socket=io();let roomCode="",myName="",currentState=null;const faces=["⚀","⚁","⚂","⚃","⚄","⚅"];const rotations={1:"rotateX(-18deg) rotateY(25deg)",2:"rotateX(90deg) rotateY(25deg)",3:"rotateX(-18deg) rotateY(205deg)",4:"rotateX(-18deg) rotateY(-65deg)",5:"rotateX(-108deg) rotateY(25deg)",6:"rotateX(72deg) rotateY(25deg)"};const coords=[[89,80],[79,80],[69,80],[59,80],[49,80],[39,80],[29,80],[19,80],[9,80],[8,67],[8,56],[8,45],[8,34],[8,22],[18,19],[28,19],[38,19],[48,19],[58,19],[68,19],[78,19],[88,19],[89,32],[89,44],[89,56],[89,68],[89,79],[79,79],[69,79],[59,79]];const cells=["DÉPART","Sofia","Belgrad","Gare","Istanbul","Prague","Budapest","Lisbonne","Enchères","Dubaï","Pékin","Électricité","Tokyo","ALLEZ","Banque","New York","Monaco","Aéroport","Londres","Varsovie","Zurich","Prison","Paris","Milan","Eaux","Amsterdam","Moscou","Rome","Madrid","Impôts"];
function name(){return (username.value||localStorage.getItem("montenoirUser")||localStorage.getItem("loggedUser")||"Misafir").trim()}function createRoom(){myName=name();socket.emit("monopoly_create_room",{username:myName,token:tokenSelect.value})}function joinRoom(){myName=name();socket.emit("monopoly_join_room",{username:myName,token:tokenSelect.value,code:roomCodeInput.value})}function startGame(){socket.emit("monopoly_start_game",{code:roomCode})}function rollDice(){socket.emit("monopoly_roll_dice",{code:roomCode,username:myName})}function buyProperty(){socket.emit("monopoly_buy_property",{code:roomCode,username:myName});hideBuy()}function endTurn(){socket.emit("monopoly_end_turn",{code:roomCode,username:myName})}function hideBuy(){buyPanel.style.display="none"}function logMsg(t){log.innerHTML=t+"<br>"+log.innerHTML}
function render(st){currentState=st;roomCode=st.code;roomCodeInput.value=st.code;roomCode.textContent=st.code;document.querySelectorAll(".token,.house").forEach(e=>e.remove());let names=Object.keys(st.players);players.innerHTML="";names.forEach((n,i)=>{let p=st.players[n],turn=i===st.turnIndex;players.innerHTML+=`<div class="player ${turn?'turn':''}"><div class="picon">${p.token}</div><div><b>${n}</b><br>${p.money}€</div></div>`;addToken(p,i)});Object.entries(st.houses||{}).forEach(([cell,n])=>{let c=coords[+cell]||[50,50];for(let i=0;i<n;i++){let h=document.createElement("div");h.className="house";h.textContent="🏠";h.style.left=`calc(${c[0]}% - 16px + ${i*12}px)`;h.style.top=`calc(${c[1]}% - 42px)`;boardWrap.appendChild(h)}});Object.entries(st.hotels||{}).forEach(([cell,n])=>{if(n){let c=coords[+cell]||[50,50];let h=document.createElement("div");h.className="house";h.textContent="🏨";h.style.left=`calc(${c[0]}% - 18px)`;h.style.top=`calc(${c[1]}% - 45px)`;boardWrap.appendChild(h)}});logMsg(st.lastLog||"");diceFace.textContent=st.lastDice?faces[st.lastDice-1]:"⚀"; if(st.lastDice){diceCube.style.transform=rotations[st.lastDice]||rotations[1];}renderHouses(st)}
function addToken(p,i){let c=coords[p.position]||[86,80],el=document.createElement("div");el.className="token";el.textContent=p.token;el.style.left=`calc(${c[0]}% - 21px + ${i*6}px)`;el.style.top=`calc(${c[1]}% - 21px + ${i*6}px)`;boardWrap.appendChild(el)}
...
function animateOneToken(username, from, to, state, done){
  ...
  let tokenEl=[...document.querySelectorAll(".token")][idx];
  ...
  const timer=setInterval(()=>{
    ...
    tokenEl.style.left=`calc(${c[0]}% - 21px + ${idx*6}px)`;
    tokenEl.style.top=`calc(${c[1]}% - 21px + ${idx*6}px)`;
    k++;
  },260);
}
socket.on("monopoly_animate_move",d=>{
  diceCube.classList.add("roll");
  setTimeout(()=>{
    diceCube.classList.remove("roll");
    diceFace.textContent=faces[d.dice-1];
    diceCube.style.transform=rotations[d.dice]||rotations[1];
    if(currentState){animateOneToken(d.username,d.from,d.to,d.state,()=>{
      render(d.state);
      if(d.pendingBuy&&d.state.players[myName]?.position===d.to)buyPanel.style.display="block";
    })}else{
      render(d.state);
      if(d.pendingBuy&&d.state.players[myName]?.position===d.to)buyPanel.style.display="block";
    }
