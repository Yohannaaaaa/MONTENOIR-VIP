...
def clean_register_user(username, email, password):
...
def api_clean_register():
    ...
    return clean_register_user(data.get("username", ""), data.get("email", ""), data.get("password", ""))
...
def compat_register_account_clean(data):
    ...
    if p1 != p2:
        ...
        return
    emit("register_result", clean_register_user(data.get("username", ""), data.get("email", ""), p1))
...
function renderUserTop(profile){if(profile&&profile.username){miniUser.innerText=profile.username;userBlock.classList.add("show")}else{miniUser.innerText="Üye";userBlock.classList.remove("show")}}
