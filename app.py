...
def mr(data):
    ...
    d1=random.randint(1,6); d2=random.randint(1,6); d=d1+d2
    is_double_six=(d1==6 and d2==6)
    old=p["position"]; new=(old+d)%len(METROPOLY_CELLS)
    if new<old: p["money"]+=200
    p["position"]=new; name,typ,price,rent,grp=METROPOLY_CELLS[new]; log=f"{u} a fait {d1} et {d2} ({d}). {name}."
    pending=False
    if typ in ["property","transport","utility"]:
        owner=r["owners"].get(str(new))
        if owner and owner!=u:
