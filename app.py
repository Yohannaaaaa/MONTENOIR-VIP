...
function init3D(){
 ...
 camera.position.set(0,13.2,5.8);
 camera.lookAt(0,0,0);
 ...
 dice=createDice(); dice.position.set(0,.72,.28); scene.add(dice); initPhysics();
...
function pos3D(i,off=0){const p=coords2d[i]||coords2d[0];return new THREE.Vector3((p[0]/100-.5)*boardW+off,.39,(p[1]/100-.5)*boardH+off)}
function tokenMesh(icon,color){
 const g=new THREE.Group();
 const s=new THREE.Mesh(new THREE.SphereGeometry(.23,32,18),new THREE.MeshStandardMaterial({color:color||0xd4af37,metalness:.72,roughness:.25}));
 ...
 sp.position.y=.36;sp.scale.set(.56,.56,.56);g.add(sp);
 return g;
}
...
function initPhysics(){
 ...
 groundBody.position.set(0,0.27,0);
...
function createDice(){
 const g=new THREE.Group();
 const mat=new THREE.MeshStandardMaterial({color:0xfdfdfd,roughness:.18,metalness:.01});
 const box=new THREE.Mesh(new THREE.BoxGeometry(.92,.92,.92,4,4,4),mat); box.castShadow=true; g.add(box);
 const edge=new THREE.Mesh(new THREE.BoxGeometry(.935,.935,.935),new THREE.MeshStandardMaterial({color:0xd8d8d8,roughness:.35,transparent:true,opacity:.22})); g.add(edge);
 const dotMat=new THREE.MeshStandardMaterial({color:0x030303,roughness:.4});
 const faces=[
  [[0,0,.47],[0,0,0],[[0,0]]],
  [[0,0,-.47],[0,Math.PI,0],[[-.19,.19],[.19,-.19]]],
  [[.47,0,0],[0,Math.PI/2,0],[[-.22,.22],[0,0],[.22,-.22]]],
  [[-.47,0,0],[0,-Math.PI/2,0],[[-.22,.22],[.22,.22],[-.22,-.22],[.22,-.22]]],
  [[0,.47,0],[-Math.PI/2,0,0],[[-.23,.23],[.23,.23],[0,0],[-.23,-.23],[.23,-.23]]],
  [[0,-.47,0],[Math.PI/2,0,0],[[-.23,.25],[.23,.25],[-.23,0],[.23,0],[-.23,-.25],[.23,-.25]]]
 ];
 faces.forEach(f=>f[2].forEach(([a,b])=>{const dot=new THREE.Mesh(new THREE.SphereGeometry(.06,20,10),dotMat);dot.position.set(a,b,.505);const h=new THREE.Group();h.add(dot);h.position.set(...f[0]);h.rotation.set(...f[1]);g.add(h)}));
 return g;
}
...
function rollDiceAnim(n,done){
 ...
 // Spawn the die above the black center and throw it forward.
 diceBody.position.set(-1.35,2.25,-0.55);
 diceBody.velocity.set(2.25,-1.8,1.05);
 ...
 const watcher=setInterval(()=>{
   ...
   // Keep dice on the black board area, never outside.
   diceBody.position.x=Math.max(-2.1,Math.min(2.1,diceBody.position.x));
   diceBody.position.z=Math.max(-1.25,Math.min(1.25,diceBody.position.z));

   if((ticks>70 && v<0.22 && av<0.32) || ticks>155){
     clearInterval(watcher);
     physicsRunning=false;

     // Stop cleanly and orient to the server result so gameplay stays exact.
     dice.position.set(0,.72,.28);
     forceDiceFace(n);
...
function makeHouse(color){const g=new THREE.Group();const base=new THREE.Mesh(new THREE.BoxGeometry(.16,.13,.16),new THREE.MeshStandardMaterial({color:color||0x34c759,metalness:.25,roughness:.35}));base.position.y=.065;const roof=new THREE.Mesh(new THREE.ConeGeometry(.14,.12,4),new THREE.MeshStandardMaterial({color:0x111111,metalness:.25,roughness:.32}));roof.rotation.y=Math.PI/4;roof.position.y=.19;g.add(base);g.add(roof);return g}
function makeHotel(color){const g=new THREE.Group();const base=new THREE.Mesh(new THREE.BoxGeometry(.24,.22,.20),new THREE.MeshStandardMaterial({color:color||0xff3b30,metalness:.28,roughness:.35}));base.position.y=.11;const roof=new THREE.Mesh(new THREE.BoxGeometry(.28,.06,.24),new THREE.MeshStandardMaterial({color:0xffd989,metalness:.45,roughness:.25}));roof.position.y=.25;g.add(base);g.add(roof);return g}
...
function moveToken(n,from,to,state,done){const mesh=players3d[n];if(!mesh){done();return}let steps=[],cur=from,total=coords2d.length;while(cur!==to){cur=(cur+1)%total;steps.push(cur);if(steps.length>total+2)break}const idx=Object.keys(state.players).indexOf(n);let k=0;const id=setInterval(()=>{if(k>=steps.length){clearInterval(id);done();return}mesh.position.copy(pos3D(steps[k],idx*.045));mesh.position.y=.68;setTimeout(()=>mesh.position.y=.36,105);k++},220)}
