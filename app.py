...
function soundCorrect(){
    playTone(660,.12,'sine',.09);
    setTimeout(()=>playTone(880,.10,'sine',.07),90);
    setTimeout(()=>playTone(1040,.08,'triangle',.05),170);
}
function soundWrong(){
    playTone(220,.18,'sawtooth',.07);
    setTimeout(()=>playTone(160,.16,'sawtooth',.05),110);
}
function soundAssassin(){
    playTone(80,.35,'sawtooth',.13);
    setTimeout(()=>playTone(45,.45,'sawtooth',.11),130);
    setTimeout(()=>playTone(120,.18,'square',.06),260);
}
function soundWin(){
    [523,659,784,1046].forEach((f,i)=>setTimeout(()=>playTone(f,.18,'triangle',.09),i*120));
}
function soundLose(){
    [330,260,196,130].forEach((f,i)=>setTimeout(()=>playTone(f,.22,'sawtooth',.07),i*130));
}
...
function formatClueLog(logs){
    ...
    return '📜 Oyun bandı:<br>' + logs.slice(-8).reverse().map(x=>'<span class="clueNeon">💚 '+x+'</span>').join('');
}
